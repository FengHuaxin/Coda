...
```bash
npx vitest run test/domains/Coda-classic/Coda-scripts.test.ts   # Coda 脚本契约 + 行为测试
npx vitest run                                   # 全量测试
```
...
- `domains/`：业务领域模块。每个子目录是一个可独立维护的领域模块，例如 `domains/bundle/`、`domains/Coda-classic/`、`domains/dashboard/`、`domains/skill/`。
...
脚本位于 `assets/skills/Coda/scripts/`，全部是 **Node.js 启动器（`.mjs`）**，只依赖 Node.js（用户用 Coda 必有 Node.js），**不依赖 bash / Git Bash / WSL**。

- 每个启动器（`Coda-state.mjs`、`Coda-guard.mjs`、`Coda-handoff.mjs`、`Coda-archive.mjs`、`Coda-yaml-validate.mjs`、`Coda-hook-guard.mjs`）是薄封装：`import { main } from './Coda-runtime.mjs'` 后以 `main(['<command>', ...process.argv.slice(2)])` 分发。
- 所有真实逻辑在 `domains/Coda-classic/*.ts`（TypeScript），由 `scripts/build/build-classic-runtime.mjs`（esbuild）打包成单个 `Coda-runtime.mjs`。**修改 `domains/Coda-classic/*` 后必须 `pnpm build` 重新生成 runtime**，否则测试用的是旧 bundle。
- `Coda-env.mjs` 打印自身所在目录（scripts dir），供 skill 样板代码解析同级启动器路径。
...
- 新增/重命名脚本必须同步：`assets/manifest.json` 的 `skills[]`（以及 `hooks` 里的 `Coda-hook-guard.mjs`）、`test/domains/Coda-classic/Coda-scripts.test.ts` 的 `beforeEach` 拷贝列表、`.codex/skills/Coda/scripts/` 镜像（该目录 gitignored，由 install 重新生成，本地保持一致即可）。
- skill 样板（boilerplate，当前版本 `v3`）在所有 SKILL.md / reference 中重复，改动需全量同步；样板通过 `find` 定位 `Coda-env.mjs`，再用 `node "$Coda_ENV"` 解析路径，命令统一为 `node "$Coda_STATE" ...` 形式。

## 脚本依赖关系

```
Coda-runtime.mjs  ←  所有 Coda-*.mjs 启动器 import 它
  └─ domains/Coda-classic/classic-cli.ts 分发：state / validate / guard / handoff / archive / hook-guard
Coda-hook-guard.mjs ← PreToolUse hook（由 install 写入各平台 settings，命令形如 node <skillsDir>/.../Coda-hook-guard.mjs）
```

打包入口 `domains/Coda-classic/classic-cli.ts` 导出 `main` / `runClassicCli` / `CLASSIC_COMMANDS`；esbuild ESM bundle 保留这些 export，启动器直接 import 调用，单进程、无 bash、无二次 node 派生。

## .Coda.yaml 状态机

每个 change 的状态文件，字段变更需要同步三处（全在 TypeScript 中）：

1. `domains/Coda-classic/classic-state-command.ts` — `set` 白名单 + enum 验证（`SETTABLE_FIELDS` / `MACHINE_OWNED_FIELDS`）
2. `domains/Coda-classic/classic-validate-command.ts` — schema 校验 + 已知字段
3. `test/domains/Coda-classic/Coda-scripts.test.ts` — 测试中的 yaml 字符串

改完 1/2 后 `pnpm build` 重新生成 `Coda-runtime.mjs`，否则 `classic-runtime.test.ts` 的新鲜度检查会失败。
