...
/** Write only run_id onto a yaml document (the rest of Run state lives in .coda/run-state.json). */
...
export const RUN_STATE_FILE = '.Coda/run-state.json';
...
export async function writeRunState(changeDir: string, state: RunState): Promise<void> {
  ...
  const file = path.join(changeDir, RUN_STATE_FILE);
  const temporary = path.join(changeDir, '.Coda', `run-state.${randomUUID()}.tmp`);
