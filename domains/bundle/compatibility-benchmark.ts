...
export interface CodaBundleCompatibilityResult {
  platforms: number;
...
async function createCurrentCodaBundle(repoRoot: string, root: string): Promise<CurrentManifest> {
  ...
  if (hookScript && hookScriptId) {
    ...
    await fs.writeFile(
      path.join(root, 'hooks', 'coda-phase-guard.yaml'),
      stringify({
        event: 'before_write',
  ...
  const bundleManifest = {
    ...
    metadata: {
      ...
      description: 'Current Coda managed asset compatibility fixture',
      defaultLocale: 'en',
      locales: ['en', 'zh'],
    },
    skills: [
    ...
    resources: {
      ...
      hooks:
        hookScript && hookScriptId
          ? [{ id: 'coda-phase-guard', path: 'hooks/coda-phase-guard.yaml' }]
          : [],
...
export async function runCodaBundleCompatibilityBenchmark(options: {
  repoRoot: string;
}): Promise<CodaBundleCompatibilityResult> {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-compat-'));
  const bundleRoot = path.join(temporary, 'bundle');
  const targetRoot = path.join(temporary, 'target');
  await fs.mkdir(bundleRoot, { recursive: true });
  try {
    const current = await createCurrentCodaBundle(options.repoRoot, bundleRoot);
    ...
    for (const target of targets) {
      for (const locale of ['en', 'zh'] as const) {
        ...
        for (const relative of current.skills) {
          ...
          const pathOk = Boolean(actual);
          record(paths, pathOk);
          record(skill, pathOk && (await sameFile(actual!.source, expectedSource)));
        }
        for (const relative of referencePaths) {
          ...
          record(
            reference,
            Boolean(actual) && (await sameFile(actual!.source, expectedSource)),
          );
        }
      }
