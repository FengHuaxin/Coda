...
import { CODA_FIVE_PHASE_NODES } from '../workflow-contract/builtins.js';
...
const WORKFLOW_CONTRACT_BUILTIN_SKILLS = new Set(
  CODA_FIVE_PHASE_NODES.map((node) => node.implementation.skill),
);
...
function bundleManifest(
  ...
  return {
    ...
    metadata: {
      ...
      description: state.factory?.goal ?? `Generated Coda Skill Factory bundle for ${state.name}.`,
    ...
    // currently embeds Coda-native runtime files inside the generated entry Skill.
...
This Bundle is generated by the Coda Skill Factory and uses a portable control plane.

- Entry Skill: \`skills/${skillId}/SKILL.md\`
- Plan script: \`skills/${skillId}/scripts/coda-plan.mjs\`
- Check script: \`skills/${skillId}/scripts/coda-check.mjs\`
- Hook guard script: \`skills/${skillId}/scripts/coda-hook-guard.mjs\`
      ...
      sources: [
        {
          ...
          root: path.join(projectRoot, '.coda', '__bundle_builtin__', candidate.name),
          description: `Built-in Coda workflow step: ${candidate.name}.`,
