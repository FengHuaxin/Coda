...
export interface BundleEvalResult {
  schemaVersion: 1;
  provider: 'native-skill-creator' | 'coda-fallback';
  level: 'quick' | 'full';
  bundleHash: string;
...
function parseEvalResult(value: unknown): BundleEvalResult {
  ...
  if (!['native-skill-creator', 'coda-fallback'].includes(String(value.provider))) {
...
async function validateFactoryManifestResources(options: {
  ...
  const expectedScripts = [
    {
      id: 'coda-plan',
      path: `skills/${entrySkill}/scripts/coda-plan.mjs`,
    },
    {
      id: 'coda-check',
      path: `skills/${entrySkill}/scripts/coda-check.mjs`,
    },
    {
      id: 'coda-hook-guard',
      path: `skills/${entrySkill}/scripts/coda-hook-guard.mjs`,
    },
    {
      id: 'workflow-state',
  ...
  for (const hook of expectedHooks) {
    ...
    try {
      ...
      if (normalized.script !== 'coda-hook-guard') {
        errors.push(`hook ${hook.id} must reference coda-hook-guard`);
      }
      if (
...
async function validateGeneratedScriptContracts(
  ...
  const checkScriptPath = path.join(packageRoot, 'scripts', 'coda-check.mjs');
  let source: string;
  try {
  ...
  const requiredFragments = [
    ...
    'control-plane-ok',
    'scripts/coda-hook-guard.mjs',
    ...(engineMode === 'none' ? [] : ['coda/skill.yaml']),
  ];
  ...
  if (workflowContractFragments.every((fragment) => source.includes(fragment))) {
    evidence.push('script-contract:coda-check workflow protocol verify');
    return;
  }
  const missing = requiredFragments.filter((fragment) => !source.includes(fragment));
  if (missing.length > 0) {
    errors.push(`scripts/coda-check.mjs verify contract missing: ${missing.join(', ')}`);
  } else {
    evidence.push('script-contract:coda-check verify');
  }
}
...
async function writeEvidence(
  ...
  const directory = path.resolve(projectRoot, '.coda', 'bundle-evals', name, result.bundleHash);
