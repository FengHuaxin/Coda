...
本目录包含 `Coda dashboard` 的只读本地仪表盘：后端采集当前仓库的
...
```powershell
pnpm build
node .\bin\Coda.js dashboard . --port 4399 --no-open
```
...
```powershell
node .\bin\Coda.js dashboard . --port 4399
node .\bin\Coda.js dashboard . --port 4399 --no-open
node .\bin\Coda.js dashboard . --json
```
...
| **Superpowers** | designDoc, plan, verifyReport                                  | 通过 `.Coda.yaml` 路径指针引用的 `docs/superpowers/` 文件 |
| **Coda**       | .Coda.yaml, handoff, checkpoint, brainstorm, subagentProgress | `.Coda/` 中间产物                                         |
...
修改 `domains/dashboard/web/` 后，如果通过 `bin/Coda.js` 启动，需要先运行
