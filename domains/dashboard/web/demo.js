...
function addCodaArtifacts(change) {
  ...
  const Coda = [
    {
      ...
      label: '.Coda.yaml',
      source: 'coda',
      exists: change.artifacts?.CodaYaml ?? true,
      path: `${dir}/.Coda.yaml`,
    },
    {
      key: 'handoff',
      ...
      source: 'coda',
      exists: false,
      path: `${dir}/.Coda/handoff/design-context.json`,
    },
    {
      key: 'checkpoint',
      label: 'Checkpoint',
    ...
    },
    {
      ...
      source: 'coda',
      exists: false,
      path: `${dir}/.Coda/handoff/brainstorm-summary.md`,
    },
    {
      ...
      source: 'coda',
      exists: false,
      path: `${dir}/.Coda/subagent-progress.md`,
    },
  ];
  if (change.artifacts?.grouped) {
    change.artifacts.grouped.push(...superpowers, ...Coda);
  }
  return change;
}
  ...
  risks: [
    {
      ...
      code: 'verify-failed',
      message: '1 个变更验证失败',
      suggestion: '打开 fix-webhook-retries 并重新运行 Coda verify',
    },
    {
      level: 'warning',
    ...
    active: [
        ...
        risks: [
          {
            ...
            suggestion: '完成「测试覆盖」分组后运行 Coda verify',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        next: {
          command: 'Coda verify',
          reason: '验证失败需修复',
          description: 'verify-result.md 显示 2 项断言失败，修复后重新运行 Coda verify。',
        },
        risks: [
          {
            level: 'error',
...
// Enrich all changes with Coda intermediate artifacts
DEMO_SNAPSHOT.changes.active.forEach(addCodaArtifacts);
DEMO_SNAPSHOT.changes.archived.forEach(addCodaArtifacts);
...
export const DEMO_SKILL_VISUALS = {
  compose: {
    ...
    bundles: [
      {
        name: 'customize-Coda-release-checks',
        status: 'review-approved',
        currentStep: 'publish',
        mode: 'optimize',
        goal: '定制 /Coda：在验证前插入发布准备、README 同步和 Changelog 检查。',
        engineMode: 'deterministic',
        runnerMode: 'change',
        reusedSkills: [
          { skill: 'coda', status: 'available', sourceCount: 1 },
    ...
    results: [
      {
        ...
        entries: [
          ...
          },
          {
            ...
            passed: true,
            passRate: 0.92,
            evidence: ['Coda/eval.yaml', 'scripts/Coda-check.mjs'],
          },
          {
