/**
 * Lightweight `.coda.yaml` parser.
 *
 * Mirrors the format produced by `assets/skills/coda/scripts/coda-state.sh`
...
export type CodaYaml = Record<string, string>;

export async function readCodaYaml(yamlPath: string): Promise<CodaYaml | null> {
  if (!(await fileExists(yamlPath))) return null;
  const raw = await fs.readFile(yamlPath, 'utf-8');
  return parseCodaYaml(raw);
}

export function parseCodaYaml(raw: string): CodaYaml {
  const out: CodaYaml = {};
