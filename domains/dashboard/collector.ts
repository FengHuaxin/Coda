...
import { readCodaYaml, type CodaYaml } from './yaml.js';
...
async function buildChangeItem(input: BuildChangeInput): Promise<ChangeDashboardItem> {
  const yamlPath = path.join(input.dir, '.coda.yaml');
  ...
  const yaml: CodaYaml = (await readCodaYaml(yamlPath)) ?? {};
  ...
  const resolvedVerifyPath = yamlVerifyPath
    ...
    : path.join(input.dir, '.coda', 'verify-result.md');
  ...
  // Coda intermediate artifacts
  const handoffPath = path.join(input.dir, '.coda', 'handoff', 'design-context.json');
  const checkpointPath = path.join(input.dir, '.coda', 'checkpoint.json');
  const brainstormPath = path.join(input.dir, '.coda', 'handoff', 'brainstorm-summary.md');
  const subagentProgressPath = path.join(input.dir, '.coda', 'subagent-progress.md');
  ...
  const artifactPreviews = await readArtifactPreviews([
    ...
    [
      'verifyReport',
      '验证报告',
      verify.reportExists ? resolvedVerifyPath : path.join(input.dir, '.coda', 'verify-result.md'),
    ],
    ['codaYaml', '变更配置', yamlPath],
  ...
  const risks: DashboardRisk[] = buildChangeRisks({
    status: input.status,
    phase,
    hasCodaYaml: artifacts.codaYaml,
    tasks,
    verify,
    artifacts,
...
function buildGroupedArtifacts(input: GroupedInput): GroupedArtifact[] {
  ...
  return [
    ...
    },
    {
      ...
      label: '.coda.yaml',
      source: 'coda',
      exists: input.codaYamlExists,
      path: input.codaYamlPath,
    },
    {
      key: 'handoff',
      label: 'Handoff 上下文',
