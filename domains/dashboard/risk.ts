...
export function buildChangeRisks(input: ChangeRiskInput): DashboardRisk[] {
  ...
  if (!input.hasCodaYaml) {
    risks.push({
      level: 'warning',
      code: 'MISSING_Coda_YAML',
      message: '该 change 缺少 .Coda.yaml，无法判定准确状态。',
      suggestion: '运行 /Coda-open 或手动补齐 .Coda.yaml。',
    });
  }
  ...
  if (input.phase === 'unknown') {
    risks.push({
      ...
      suggestion: '检查 .Coda.yaml 的 phase 字段，或重新运行对应阶段命令。',
    });
  }
  ...
  if (!input.artifacts.tasks) {
    ...
    if (remaining > 0) {
      risks.push({
        ...
        suggestion: '继续执行 /Coda-build 完成剩余任务后再进入 verify。',
      });
    }
  }
  ...
  if (input.verify.result === 'fail') {
    risks.push({
      ...
      suggestion: '打开 verify-result.md 修复失败项后重新运行 /Coda-verify。',
    });
    ...
    risks.push({
      ...
      suggestion: '完成 build 任务后运行 /Coda-verify。',
    });
  }
  ...
  if (input.status === 'archived' && input.archiveMetadataKnown === false) {
    risks.push({
      ...
      suggestion: '后续归档时遵守标准命名，或在 .Coda.yaml 中记录 archived_at。',
    });
  }
