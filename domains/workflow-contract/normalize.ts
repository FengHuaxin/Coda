import { BUILTIN_CODA_OUTPUT_SCHEMAS, CODA_FIVE_PHASE_NODES } from './builtins.js';
import type {
...
function templatesFor(
  ...
  if (kind === 'coda-five-phase-overlay') {
    return [...CODA_FIVE_PHASE_NODES.map(cloneTemplate), ...customNodes.map(cloneTemplate)];
  }
  return customNodes.map(cloneTemplate);
}

function outputSchemasFor(input: WorkflowDefinitionInput): WorkflowOutputSchema[] {
  const schemas = input.kind === 'coda-five-phase-overlay' ? BUILTIN_CODA_OUTPUT_SCHEMAS : [];
...
export function normalizeWorkflowDefinition(
  ...
  const protocol: WorkflowProtocol = {
    ...
    outputSchemas,
    state:
      input.kind === 'coda-five-phase-overlay'
        ? {
            statePath: 'openspec/changes/*/.coda.yaml',
            compatibilityStatePath: `.coda/runs/${input.name}/state.json`,
            currentNodeField: 'phase',
            completedNodesField: 'completedNodes',
            evidenceField: 'evidence',
          }
        : {
            statePath: `.coda/runs/${input.name}/state.json`,
