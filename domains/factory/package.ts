...
function nodeAuthoringMode(
  ...
  return protocol.kind === 'coda-five-phase-overlay' ? 'delegates' : 'substance';
}
...
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const packageRoot = path.resolve(__dirname, '..');
const runRoot = process.env.CODA_RUN_ROOT ? path.resolve(process.env.CODA_RUN_ROOT) : process.cwd();
...
function statePath(protocol) {
  ...
  const fallback = String(protocol.state?.compatibilityStatePath ?? '.coda/runs/' + protocol.name + '/state.json');
...
function renderSkillReviewMarkdown(plan: FactorySkillPackagePlan): string {
  ...
  return '# Skill Review\n\nEvidence source: deterministic-check-only.\n\nNo LLM authoring review has been recorded for this Bundle yet. This file is an honest placeholder, not a review approval. Run the `/coda-any` skill-review lane and record its verdict via `coda bundle authoring-record <name> --lane skill-review --file <review.json>` to replace this with a real multi-vote review summary.\n';
}
...
function workflowContractArtifacts(plan: FactorySkillPackagePlan): FactoryPackageDraft {
  ...
  const requiredFiles = [
    ...
    ...(plan.engineMode === 'none'
      ? []
      : ['coda/skill.yaml', 'coda/guardrails.yaml', 'coda/checks.yaml', 'coda/eval.yaml']),
  ];
  const artifacts: FactoryPackageArtifact[] = [
    ...
    artifact('scripts/coda-plan.mjs', 'script', workflowContractPlanScript(), true),
    artifact('scripts/coda-check.mjs', 'script', workflowContractCheckScript(requiredFiles), true),
    artifact('scripts/coda-hook-guard.mjs', 'script', workflowContractHookGuardScript(), true),
    ...
    ...(plan.engineMode === 'none'
      ...
      : [
          ...
          artifact('coda/checks.yaml', 'engine', stringify(runtimeEvals())),
          artifact(
            'coda/eval.yaml',
            'engine',
...
export async function generateFactorySkillPackage(
  ...
  return {
    ...
    controlPlane: {
      checksPath: plan.engineMode === 'none' ? null : path.join(codaRoot, 'checks.yaml'),
      evalManifestPath: plan.engineMode === 'none' ? null : path.join(codaRoot, 'eval.yaml'),
