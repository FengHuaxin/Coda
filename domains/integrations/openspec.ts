...
function createOpenSpecAllWorkflowsEnv(): { env: NodeJS.ProcessEnv; configHome: string } {
  const configHome = fs.mkdtempSync(path.join(os.tmpdir(), 'Coda-openspec-profile-'));
  try {
...
function writeAllWorkflowsToDefaultConfig(): ConfigBackup | null {
  ...
  const backupPath = configPath + '.Coda-backup';
...
function migrateOpenCodeOpenSpecPaths(homeDir: string): void {
  ...
  // ~/.config/opencode/ (Coda's globalSkillsDir). Move the files over.
...
async function installOpenSpec(
  ...
  // Track the actual tool ids used (may be filtered if OpenSpec rejects some)
  ...
  try {
    ...
    try {
      ...
      } else if (stderrText.includes('Invalid tool(s)')) {
        // OpenSpec does not support some tool ids (e.g., kimicode, trae-cn).
        // Parse the invalid tools from the error message and retry without them.
        const invalidMatch = stderrText.match(/Invalid tool\(s\):\s*([^.]+)\./);
        if (invalidMatch) {
          const invalidTools = invalidMatch[1].split(',').map((t) => t.trim());
          const filteredToolIds = toolIds.filter((id) => !invalidTools.includes(id));
          if (filteredToolIds.length === 0) {
            console.warn('    OpenSpec does not support any of the requested tools.');
            return 'skipped';
          }
          console.warn(
            `    OpenSpec does not support: ${invalidTools.join(', ')}. Retrying with: ${filteredToolIds.join(', ')}`,
          );
          const retryInvocation = buildOpenSpecInitInvocation(
            projectPath,
            filteredToolIds,
            scope,
          );
          const retryArgs = useShell
            ? quoteArgsForShell(retryInvocation.args)
            : retryInvocation.args;
          execFileSync(retryInvocation.command, retryArgs, {
            cwd: projectPath,
            env: openspecEnv.env,
            stdio: 'inherit',
            timeout: 120_000,
          ...
          // Update effectiveToolIds for the mirror/migrate logic below
    ...
    if (mirrorOpenCodePlatformIds.length > 0 && effectiveToolIds.includes('opencode')) {
      mirrorOpenCodeCompatibleOpenSpecPaths(openspecTargetBase, scope, mirrorOpenCodePlatformIds);
    }

    if (openspecWritesGlobal && effectiveToolIds.includes('opencode')) {
