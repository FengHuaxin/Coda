...
async function readField(changeDir: string, field: string): Promise<string> {
  ...
  for (const candidate of candidates) {
    const candidatePath = path.join(changeDir, candidate);
...
// Windows, /bin/sh elsewhere) via `shell: true`, so Coda no longer requires a
...
async function handoffSourceFiles(changeDir: string): Promise<string[]> {
  ...
  // shell + coda-handoff provenance byte-for-byte. changeDir is a relative forward-slash
...
async function preflight(changeDir: string, name: string): Promise<void> {
  ...
  if (!(await findClassicYamlFile(changeDir))) {
    throw new GuardFailure(red(`FATAL: .Coda.yaml not found in ${changeDir}`));
  }
  const result = await classicValidateCommand([name], { json: false });
  if (result.exitCode !== 0) {
    if (result.stderr)
      process.stderr.write(result.stderr.endsWith('\n') ? result.stderr : `${result.stderr}\n`);
    throw new GuardFailure(red('FATAL: .Coda.yaml schema validation failed'));
  }
}
...
async function guardDesignChecks(
  ...
  if (designDoc && designDoc !== 'null') {
    blocked =
      (await runChecks(output, [
        ...
        check('Design Doc frontmatter links current change', async () => {
          ...
          return (await designDocFrontmatterHas(designDoc, 'Coda_change', change))
            ? pass()
            : fail('');
        }),
    ...
    output.stderr.push(
      yellow('  [WARN] No design_doc recorded in .coda.yaml (optional for hotfix/tweak)'),
    );
  }
  return blocked;
}
