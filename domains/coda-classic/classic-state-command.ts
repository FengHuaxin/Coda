...
import { openSpecChangeNameError, resolveClassicChangeDirectory, findClassicYamlFile } from './classic-paths.js';
...
async function readDocument(file: string): Promise<Document> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        fail(
          `ERROR: .Coda.yaml not found at ${path.relative(process.cwd(), file).replaceAll('\\', '/')}`,
        );
    }
    throw error;
  }
  const document = parseDocument(source, { uniqueKeys: false });
  if (document.errors.length > 0) fail(`ERROR: Invalid .Coda.yaml: ${document.errors[0].message}`);
  return document;
}
...
async function projectConfigValue(
  ...
  const file = path.resolve('.coda', 'config.yaml');
...
async function contextCompression(): Promise<string> {
  const value =
    process.env.CODA_CONTEXT_COMPRESSION ??
...
async function autoTransition(): Promise<string> {
  const value =
    process.env.CODA_AUTO_TRANSITION ?? (await projectConfigValue('auto_transition')) ?? 'true';
...
async function reviewModeDefault(): Promise<string | null> {
  const value =
    process.env.CODA_REVIEW_MODE ?? (await projectConfigValue('review_mode')) ?? 'standard';
...
async function stateFile(
  ...
  const file = (await findClassicYamlFile(change.directory)) ?? path.join(change.directory, '.Coda.yaml');
  return {
    ...change,
    file,
  };
...
async function setField(
  ...
  if (field === 'phase' && !options.internal && process.env.CODA_FORCE_PHASE !== '1') {
    fail(
  ...
  if (field === 'phase' && !options.internal) {
    output.stderr.push(
      ...
      yellow('  Consider using: coda-state.mjs transition <change-name> <event>'),
    );
  }
...
async function init(output: CommandOutput, name: string, workflow: string): Promise<void> {
  ...
  if (await exists(file)) fail(`ERROR: .Coda.yaml already exists at ${label}/.Coda.yaml`);
  ...
  await atomicWrite(file, document.toString());
  output.stderr.push(green(`Initialized: ${label}/.Coda.yaml (workflow=${workflow})`));
}
...
async function next(output: CommandOutput, name: string): Promise<void> {
  ...
  if (!(await exists(file))) fail(`ERROR: .Coda.yaml not found at ${label}/.Coda.yaml`);
...
async function recoverOpen(output: CommandOutput, directory: string): Promise<void> {
  ...
  output.stdout.push(
    '',
    complete === 3
      ? 'Recovery action: All artifacts complete. Run /Coda-open user confirmation, then guard to transition.'
      : complete === 0
        ? 'Recovery action: No artifacts created yet. Start from /Coda-open Step 1 (explore and clarify).'
        : 'Recovery action: Some artifacts incomplete. Resume /Coda-open from the first missing artifact.',
  );
}
...
async function recoverVerify(output: CommandOutput, name: string): Promise<void> {
  ...
  output.stdout.push(
    ...
    result === 'pass' && branch === 'handled'
      ...
      : result === 'fail'
        ? 'Recovery action: Verification failed and rolled back to build. Resume from /Coda-build.'
