...
export const classicValidateCommand: ClassicCommandHandler = async (args) => {
  ...
  const yamlFile = (await findClassicYamlFile(directory)) ?? path.join(directory, '.Coda.yaml');
  const lines = [`[VALIDATE] ${label}/${path.basename(yamlFile)}`];
  let errors = 0;
  let warnings = 0;
