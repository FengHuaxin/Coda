...
async function activeChanges(projectRoot: string): Promise<GoverningChange[]> {
  ...
  for (const entry of (await fs.readdir(changesDir, { withFileTypes: true })).sort((left, right) =>
    ...
    if (!existsSync(path.join(changeDir, '.Coda.yaml'))) continue;
...
function isCodaConfig(relativePath: string): boolean {
  return (
    relativePath === '.Coda.yaml' ||
    relativePath === 'Coda.yaml' ||
    relativePath === '.Coda.yml' ||
    relativePath === 'Coda.yml' ||
    relativePath.startsWith('.Coda/') ||
    relativePath.includes('/.Coda/')
  );
}
...
function openSpecAllowed(relativePath: string, phase: ClassicPhase): string | null {
  ...
  const stateFile =
    relativePath.endsWith('/.Coda.yaml') || relativePath.endsWith('/.openspec.yaml');
  const proposal =
    relativePath.endsWith('/proposal.md') ||
    relativePath.endsWith('/design.md') ||
    relativePath.endsWith('/tasks.md');
  const handoff = relativePath.includes('/.Coda/');
...
export const classicHookGuardCommand: ClassicCommandHandler = async (args) => {
  ...
  if (isCodaConfig(relativePath)) {
    return allowed(`${relativePath} (whitelist: Coda config)`);
  }
