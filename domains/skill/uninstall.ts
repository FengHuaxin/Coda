...
async function removeCodaSkillsForPlatform(
  baseDir: string,
  platform: Platform,
  ...
  if (platform.id === 'pi') {
    ...
    if (await removeFile(path.join(extensionsDir, 'coda-commands.ts'))) {
      removed++;
    }
...
async function removeCodaRulesForPlatform(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
): Promise<RemovalResult> {
...
async function removeCodaHooksForPlatform(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
): Promise<RemovalResult> {
...
async function removeCopilotHooks(platformBase: string): Promise<RemovalResult> {
  const hookFilePath = path.join(platformBase, 'hooks', 'coda-guard.json');
...
async function removeKiroHooks(
  ...
  for (const entry of entries) {
    ...
    const isCodaHook = scriptRelPaths.some((scriptPath) => {
