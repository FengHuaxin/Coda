...
description: Run the {skillName} Coda workflow
---
`;

const PI_COMMAND_EXTENSION_FILE = 'coda-commands.ts';
 ...
 * Project scope: <project>/.coda/skills/
 * Global scope: ~/.coda/skills/
 */
function getCentralSkillsDir(baseDir: string, scope: InstallScope): string {
  return path.join(baseDir, '.coda', 'skills');
}
 ...
 * 1. Copy skills to central store (.coda/skills/)
...
async function copyCodaSkillsForPlatform(
  baseDir: string,
  platform: Platform,
  overwrite: boolean,
  languageSkillsDir: string = 'skills',
  ...
  for (const skillRelPath of getManagedSkillPaths(manifest)) {
    ...
    try {
      ...
      // coda-hook-guard.mjs) is visible in the summary rather than silently
...
export default function registerCodaCommands(pi: ExtensionAPI) {
  for (const name of commands) {
    pi.registerCommand(name, {
      description: \`Coda: /\${name}\`,
      handler: async (args) => {
...
Equivalent Coda skill: \`${skillName}\`
...
/**
 * Copy Coda rule files to a platform's rules directory.
 * Formats:
 ...
 */
async function copyCodaRulesForPlatform(
  baseDir: string,
  platform: Platform,
  overwrite: boolean,
  scope: InstallScope = 'project',
...
function computeRuleDestPath(
  ...
  if (rulesFormat === 'copilot') {
    // GitHub Copilot: coda-phase-guard.md → coda-phase-guard.instructions.md
...
/**
 * Install Coda hooks for platforms that support them.
 ...
 */
async function installCodaHooksForPlatform(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
...
function isManagedHookCommand(command: unknown, scriptRelPaths: string[]): boolean {
  ...
  // Match both the current `node .../coda-hook-guard.mjs` form and the legacy
  // `bash .../coda-hook-guard.sh` form so uninstall also cleans up hooks
  // written by older Coda releases. Compare basenames without extension.
 ...
 * Writes to .github/hooks/coda-guard.json with preToolUse hooks config.
 */
...
/**
 * Kiro format:
 * Writes to .kiro/hooks/coda-phase-guard.kiro.hook as a JSON file.
 */
...
async function mergeProjectConfig(projectPath: string): Promise<void> {
  const configPath = path.join(projectPath, '.coda', 'config.yaml');
