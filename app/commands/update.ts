...
const PACKAGE_NAME = '@huaxin/coda';
...
function getInstalledCodaSkillsDirs(
  baseDir: string,
  platform: Platform,
...
async function hasLocalCodaSkills(
  baseDir: string,
  platform: Platform,
  scope: InstallScope,
): Promise<boolean> {
  for (const skillsDir of getInstalledCodaSkillsDirs(baseDir, platform, scope)) {
    if (!(await fileExists(skillsDir))) continue;
    const entries = await readDir(skillsDir);
    if (entries.some((entry) => entry.startsWith('Coda'))) return true;
  }
  return false;
}

async function detectInstalledCodaLanguage(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
  ...
  for (const skillsDir of getInstalledCodaSkillsDirs(baseDir, platform, scope)) {
    if (!(await fileExists(skillsDir))) continue;
    const entries = (await readDir(skillsDir)).filter((entry) => entry.startsWith('Coda'));
...
async function detectInstalledCodaTargets(
  projectPath: string,
  options: DetectTargetsOptions = {},
): Promise<InstalledCodaTarget[]> {
  const scopes = options.scopes ?? (['project', 'global'] as InstallScope[]);
  const targets: InstalledCodaTarget[] = [];

  for (const scope of scopes) {
    const baseDir = getScopedBaseDir(scope, projectPath, options.globalBaseDir);

    for (const platform of PLATFORMS) {
      if (!(await hasLocalCodaSkills(baseDir, platform, scope))) continue;

      targets.push({
        scope,
        platform,
        language: await detectInstalledCodaLanguage(baseDir, platform, scope),
      });
    }
  }
...
async function detectCodaPackageScope(
  projectPath: string,
  packageRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..'),
): Promise<InstallScope> {
  const localPackageRoot = path.join(projectPath, 'node_modules', '@huaxin', 'coda');
...
function formatSkillUpdateCommand(
  ...
  if (installMode === 'symlink') {
    return `symlink .coda/skills/ -> ${destPrefix}${getPlatformSkillsDir(platform, scope)}/skills/ (${scope})`;
  }
...
async function updateCodaNpmPackage(
  scope: InstallScope,
  projectPath: string,
...
export async function updateCommand(
  ...
  const packageScope = options.scope ?? (await detectCodaPackageScope(projectPath));
  ...
  if (!options.skipNpm) {
    ...
    const npmUpdated = await updateCodaNpmPackage(
      packageScope,
      projectPath,
      log,
      options.json === true,
  ...
  const targets = await detectInstalledCodaTargets(projectPath, {
  ...
  for (const target of targets) {
    ...
    const languageSkillsDir = languageToSkillsDir(options.language, target.language);
    const { copied, skipped } = await copyCodaSkillsForPlatform(
      baseDir,
      target.platform,
      true,
      languageSkillsDir,
    ...
    try {
      const { copied: ruleCopied } = await copyCodaRulesForPlatform(
        baseDir,
        target.platform,
        true,
        target.scope,
      ...
      if (ruleCopied > 0) {
        log(`  Coda rules -> ${target.platform.name}: ${ruleCopied} ${t(lang, 'rulesUpdated')}`);
      }
    } catch (err) {
      log(
        `  Coda rules -> ${target.platform.name}: ${t(lang, 'rulesFailed')} (${(err as Error).message})`,
      );
    }

    if (target.platform.supportsHooks) {
      try {
        const { installed, reason } = await installCodaHooksForPlatform(
          baseDir,
          target.platform,
          target.scope,
        );
        if (installed) {
          totalHooksInstalled++;
          log(`  Coda hooks -> ${target.platform.name}: ${t(lang, 'hooksUpdated')}`);
        } else if (reason) {
          log(`  Coda hooks -> ${target.platform.name}: ${t(lang, 'hooksSkipped')} (${reason})`);
        }
      } catch (err) {
        log(
          `  Coda hooks -> ${target.platform.name}: ${t(lang, 'hooksFailed')} (${(err as Error).message})`,
        );
      }
    }
  }
...
export type { InstalledCodaTarget, SkillLanguage, TranslationKey };
