...
function isManifestTarget(target: string): boolean {
  ...
  return normalized.endsWith('coda/eval.yaml') || normalized.endsWith('coda/eval.yml');
}
...
async function resolveReportConfig(options: EvalCommandOptions): Promise<string | null> {
  ...
  const file = path.join(os.tmpdir(), `coda-eval-report-${Date.now()}.json`);
  await fs.writeFile(
    file,
    JSON.stringify(
      {
...
async function buildLaunchDetails(
  ...
  return {
    ...
    experimentId: `coda-eval-${Date.now()}`,
