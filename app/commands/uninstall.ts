...
import { detectInstalledCodaTargets } from './update.js';
...
export async function uninstallCommand(
  ...
  if (targets.length === 0) {
    ...
    log('  No Coda installations found. Nothing to uninstall.\n');
    return;
  }
  ...
  log('  Found Coda installations on the following targets:\n');
  ...
  if (!options.force && !options.json) {
    if (targets.length === 1) {
      const confirmed = await select({
        message: `Uninstall Coda from ${targets[0].platform.name} (${targets[0].scope})?`,
        choices: [
  ...
  for (const target of selectedTargets) {
    ...
    const skillsResult = await removeCodaSkillsForPlatform(baseDir, target.platform, target.scope);
    totalSkills += skillsResult.removed;

    const rulesResult = await removeCodaRulesForPlatform(baseDir, target.platform, target.scope);
    totalRules += rulesResult.removed;

    let hooksRemoved = 0;
    if (target.platform.supportsHooks) {
      const hooksResult = await removeCodaHooksForPlatform(baseDir, target.platform, target.scope);
