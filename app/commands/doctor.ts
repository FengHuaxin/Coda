...
function checkCodaCli(): CheckResult {
  return {
    check: 'Coda CLI',
    status: 'pass',
...
async function checkWorkingDirs(projectPath: string): Promise<CheckResult> {
  ...
  if (!specsExist && !plansExist) {
    return { check: 'working directories', status: 'fail', message: 'missing — run: coda init' };
  }
  const missing = [];
...
async function checkSkillCompleteness(
      ...
      results.push(
        missing.length === 0
          ...
          : {
              ...
              message: `partial (${present.length}/${total} files; missing ${missing.length}) — run: coda update --scope ${base.scope}`,
            },
      );
    }
  }
  ...
  if (!anyCodaInstall) {
    results.push({
      ...
      message:
        scope === 'auto'
          ? 'not installed in project or global scope — run: coda init'
          : `not installed in ${scope} scope — run: coda init --scope ${scope}`,
    });
  }
...
async function checkScriptsPresent(): Promise<CheckResult> {
  ...
  const scriptsDir = path.join(assetsDir, 'skills', 'coda', 'scripts');
...
function formatRuntimeEvalRecovery(
  ...
  if (nextCommand) {
    return `run ${nextCommand} or restore missing evidence (${missing}), then rerun coda doctor`;
  }
  return `restore missing evidence (${missing}) and rerun coda doctor`;
}

async function checkCodaYamlValidity(projectPath: string): Promise<CheckResult[]> {
  ...
  for (const entry of entries) {
    ...
    const yamlPath = path.join(changeDir, '.coda.yaml');
    ...
    results.push({
      check: `next: ${entry}`,
      status: 'warn',
      message: 'inspect .coda.yaml and rerun coda doctor',
    });
  }
...
async function collectResults(projectPath: string, scope: DoctorScope): Promise<CheckResult[]> {
  ...
  results.push(...(await checkCodaYamlValidity(projectPath)));
  return results;
}
...
export async function doctorCommand(
  ...
  console.log(`Coda Doctor (scope: ${scope})\n`);
