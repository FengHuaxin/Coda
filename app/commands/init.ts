...
function displaySummary(results: PlatformResult[], scope: InstallScope, lang: string): void {
  ...
  console.log(`\n  ${t(lang, 'getStarted')}`);
  console.log(`    ${t(lang, 'getStartedCoda')}`);
...
export async function initCommand(targetPath: string, options: InitOptions = {}): Promise<void> {
  ...
  for (const platform of selectedPlatforms) {
    ...
    const hasCM = await hasSkills(baseDir, platform, 'coda', selectedPlatforms, scope);
    ...
    if (!options.yes) {
      const existingComponents = [
        ...
        hasCM && cmAction === 'install' ? 'coda' : null,
      ...
      if (cmAction === 'install' && hasCM) {
        cmAction = await promptOverwriteChoice('Coda', platform.name, lang);
      }
    }
  ...
  // Deduplicate tool ids: zcode and mimocode reuse opencode's openspecToolId,
  // and trae-cn reuses trae's. Without dedup, openspec init would receive
  // duplicate entries and fail.
  const osToolIds = [
  ...
  for (const plan of plans) {
    ...
    const skillsPath =
      installMode === 'symlink'
        ? `.Coda/skills/ -> ${platformSkillsDir}/skills/`
        : `${scope === 'global' ? '~/' : ''}${platformSkillsDir}/skills/`;

    let cmStatus: InstallStatus = 'skipped';
    if (cmAction !== 'skip') {
      const { copied, failed } = await copyCodaSkillsForPlatform(
        baseDir,
        platform,
        cmAction === 'overwrite',
        language.skillsDir,
      ...
      log(
        `  Coda -> ${platform.name}: ${cmStatus} (${copied} files${
          failed > 0 ? `, ${failed} failed` : ''
        }) -> ${skillsPath}`,
      );
    } else {
      log(`  Coda -> ${platform.name}: skipped (${t(lang, 'alreadyExists')})`);
    }
    ...
    if (cmAction !== 'skip') {
      ...
      if (ruleCopied > 0) {
        log(`  Coda rules -> ${platform.name}: ${ruleCopied} ${t(lang, 'rulesInstalled')}`);
      }
    }

    if (cmAction !== 'skip' && platform.supportsHooks) {
      const { installed, reason } = await installCodaHooksForPlatform(baseDir, platform, scope);
      if (installed) {
        log(`  Coda hooks -> ${platform.name}: ${t(lang, 'hooksInstalled')}`);
      } else if (reason) {
        log(`  Coda hooks -> ${platform.name}: ${t(lang, 'hooksSkipped')} (${reason})`);
      }
    }
