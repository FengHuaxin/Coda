...
program
  .command('init [path]')
  .description('Initialize Coda workflow in your project')
...
program
  ...
  .description('Launch the local Coda dashboard in your browser')
...
program
  .command('doctor [path]')
  .description('Diagnose Coda installation health')
  .option('--json', 'Output as JSON')
  .addOption(
...
program
  .command('update [path]')
  .description('Update coda skill files to latest version')
  .option('--json', 'Output as JSON')
  .addOption(new Option('--language <lang>', 'Language for skills').choices(['en', 'zh']))
  .addOption(new Option('--scope <scope>', 'Install scope').choices(['global', 'project']))
...
program
  ...
  .description('Remove Coda skills, rules, and hooks from your project or global scope')
  .option('--json', 'Output as JSON')
...
program
  ...
  .argument('[target]', 'Local Skill directory, SKILL.md, or coda/eval.yaml')
  .option('--project <dir>', 'Repository root that contains eval/', '.')
  .option('--manifest <path>', 'Path to coda/eval.yaml')
...
skill
  .command('add <path>')
  .description('Install a Coda Skill into the project Skill pool')
...
skill
  .command('run <skill>')
  .description('Start a deterministic Coda Skill Run')
  .option('--change <dir>', 'Change directory that owns the Run')
  .option('--run-id <id>', 'Standalone Run id stored under .coda/runs/<id>')
  .option('--project <dir>', 'Project root used for Skill discovery', '.')
...
skill
  .command('continue')
  .description('Resume a Coda Skill Run or submit its pending action outcome')
  .option('--change <dir>', 'Change directory that owns the Run')
  .option('--run-id <id>', 'Standalone Run id stored under .coda/runs/<id>')
  .option('--project <dir>', 'Project root used for Skill discovery', '.')
  .addOption(
...
skill
  .command('check')
  .description(
    'Check deterministic Engine Run runtime checks. Use coda eval for benchmark reports',
  )
  .option('--change <dir>', 'Change directory that owns the Run')
  .option('--run-id <id>', 'Standalone Run id stored under .coda/runs/<id>')
...
const publish = program
  .command('publish')
  .description('Skill publish candidates for the /coda-any user-facing release path');
...
publish
  .command('run <name>')
  .description('Generate an install candidate into .coda/bundles')
  .option('--project <dir>', 'Project root', '.')
  .requiredOption('--platform <id>', 'Reference platform id')
...
const bundle = program
  .command('bundle')
  .description('Advanced Bundle backend for /coda-any Skill Maker state and audits');
...
bundle
  ...
  .description('Plan the /coda-any authoring pipeline for a Bundle')
  .option('--project <dir>', 'Project root', '.')
  .addOption(
...
bundle
  ...
  .description('Validate and record a /coda-any authoring lane output')
  .option('--project <dir>', 'Project root', '.')
...
bundle
  ...
  .description('Publish an approved Bundle into .coda/bundles')
  .option('--project <dir>', 'Project root', '.')
  .requiredOption('--platform <id>', 'Reference platform id')
  .option('--overwrite', 'Replace an existing published Bundle')
  .option('--json', 'Output as JSON')
