# Coda Any Agent Workflow Contract Implementation Plan

> **For agentic workers:** implement task by task and keep verification evidence fresh. This plan is the current architecture direction for `/Coda-any`.

**Goal:** Replace `/Coda-any` internals with a single Agent workflow contract model shared by customization based on existing Coda five-phase Skills, arbitrary Skill workflows, manual orchestration, runtime scripts, eval, review, and publish readiness.

**Architecture:** Add `domains/workflow-contract` as the deep module for WorkflowDefinition, Nodes, Skill Bindings, Output Schemas, Guardrails, normalization, validation, hashing, and built-in Coda five-phase templates. Keep `domains/bundle` as orchestration/state, `domains/factory` as package rendering, and eval/readiness as consumers of the same protocol.

## Spec Source

Implement [docs/superpowers/specs/2026-06-27-Coda-any-agent-workflow-contract-design.md](../specs/2026-06-27-Coda-any-agent-workflow-contract-design.md).
...
- [x] Generate protocol-aware scripts: `Coda-plan.mjs`, `Coda-check.mjs`, `Coda-hook-guard.mjs`, `workflow-state.mjs`, `workflow-guard.mjs`, and `workflow-handoff.mjs`.
...
- [x] Update `/Coda-any` Chinese Skill instructions first, then sync English instructions.
...
- `eval/**`
- `app/commands/**`
- `assets/skills-zh/Coda-any/**`
- `assets/skills/Coda-any/**`
- `docs/operations/**`
...
- Coda five-phase customization is described as “基于 Coda 现有 Skill 的五阶段定制”.
