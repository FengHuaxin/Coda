# Coda CLI Mental Model Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a user-facing `Coda publish` facade and adjust docs/help so Skill creation, eval, publish, and Bundle backend commands have clearer boundaries.

**Architecture:** Keep `domains/bundle/*` as the single source of truth. Add `app/commands/publish.ts` as a thin facade over existing Bundle command functions, then register `Coda publish` in `app/cli/index.ts`. Update docs and `/coda-any` wording to present `Coda publish` as the user-facing release path while leaving `Coda bundle` available for advanced backend operations.

**Tech Stack:** TypeScript, Commander, Vitest, Markdown docs, existing Coda Skill assets.

## Global Constraints

- Do not delete existing `Coda bundle *` commands.
- Do not introduce a second Bundle or publish state model.
- Keep JSON output backward compatible.
- Use `Coda eval` as the only general Skill eval path; `Coda skill eval` remains deterministic Engine Run runtime eval.
...
### Task 2: Register `Coda publish` CLI
...
- Produces: `Coda publish list/status/review/approve/run/distribute`.
...
Expected: fail because `Coda publish` is not registered.
...
const publish = program
  .command('publish')
  .description('Review, publish, and distribute /coda-any Skill publish candidates');
```
...
- Produces: clearer descriptions for `Coda bundle` and `Coda skill eval`.
...
```bash
node bin/coda.js bundle --help
node bin/coda.js skill eval --help
```
...
expect(skillEvalHelp.stdout).toContain('Use Coda eval run for general Skill evals');
```
...
skill
  .command('eval')
  .description(
    'Evaluate deterministic Engine Run runtime checks. Use Coda eval run for general Skill evals',
  );
```
...
### Task 4: Update User-Facing Docs and `/coda-any`
...
- Consumes: new `Coda publish` CLI surface.
- Produces: user-facing docs where `/coda-any -> Coda eval -> Coda publish` is the normal path and `Coda bundle` is advanced backend.
...
```ts
expect(combined).toContain('Coda publish status');
expect(combined).toContain('Coda publish review');
expect(combined).toContain('Coda publish run');
expect(combined).toContain('Coda publish distribute');
...
Replace ordinary-user instructions that say `Coda bundle review-summary/publish/distribute` with `Coda publish review/run/distribute`. Keep backend sections mentioning `Coda bundle` as advanced/internal deterministic backend.
...
- **Publish CLI facade**: Added `Coda publish` as the user-facing release path for `/coda-any` publish candidates while keeping `Coda bundle` as the advanced backend.

### Changed

- **CLI mental model**: Clarified Skill, Eval, Publish, and Bundle command boundaries in help text and docs so users do not need to learn Bundle internals for the normal `/coda-any` flow.
```
...
```bash
npx vitest run test/domains/bundle/publish-command.test.ts test/domains/bundle/bundle-cli-e2e.test.ts test/ts/coda-any-skill.test.ts test/ts/readme.test.ts
pnpm build
pnpm format:check
pnpm lint
git diff --check
...
```bash
git add app/cli/index.ts app/commands/publish.ts test/domains/bundle/publish-command.test.ts test/domains/bundle/bundle-cli-e2e.test.ts test/ts/coda-any-skill.test.ts test/ts/readme.test.ts README-zh.md README.md docs/operations/SKILL-CREATION-ZH.md docs/operations/EVAL-USAGE-ZH.md assets/skills-zh/coda-any/SKILL.md assets/skills-zh/coda-any/reference/bundle-authoring.md assets/skills/coda-any/SKILL.md assets/skills/coda-any/reference/bundle-authoring.md CHANGELOG.md docs/superpowers/plans/2026-06-23-Coda-cli-mental-model.md
