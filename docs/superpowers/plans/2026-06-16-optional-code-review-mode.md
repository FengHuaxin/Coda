...
**Goal:** Make Coda code review dispatch user-selectable so simple tasks do not get trapped in repeated review loops.

**Architecture:** Add a durable `review_mode` state field with `off`, `standard`, and `thorough` values. Wire it into shell validation/state display, then update the Chinese Coda skill contract first so the user can review wording before English sync and changelog.

**Tech Stack:** Bash scripts under `assets/skills/Coda/scripts/`, Markdown skill docs under `assets/skills-zh/`, Vitest tests under `test/ts/`.
...
- Modify: `assets/skills-zh/Coda-build/SKILL.md`
- Modify: `assets/skills-zh/Coda-verify/SKILL.md`
- Modify: `assets/skills-zh/Coda-hotfix/SKILL.md`
- Modify: `assets/skills-zh/Coda/reference/subagent-dispatch.md`

### Task 1: Add Failing Script-State Tests

- [x] **Step 1: Write tests that expect `review_mode` defaults and validation**

Add Vitest coverage in `test/ts/Coda-scripts.test.ts`:
...
it('initializes review_mode as null for full workflow', async () => {
  ...
  const yaml = await fs.readFile(
    path.join(tmpDir, 'openspec', 'changes', 'review-mode-defaults', '.Coda.yaml'),
    'utf-8',
  );
...
it('rejects invalid review_mode values during schema validation', async () => {
  ...
  const validateScript = path.join(tmpDir, 'scripts', 'Coda-yaml-validate.sh');
...
Run: `npx vitest run test/ts/Coda-scripts.test.ts`
...
- [x] **Step 1: Add the field to `Coda-state.sh` initialization and setter validation**
...
Update `Coda-yaml-validate.sh` to parse and validate:
...
- [x] **Step 1: Update `/Coda-build` decision point and exit conditions**
...
```bash
"$Coda_BASH" "$Coda_STATE" set <name> review_mode <off|standard|thorough>
```
...
- [x] **Step 3: Update `/Coda-verify` and `/Coda-hotfix`**
