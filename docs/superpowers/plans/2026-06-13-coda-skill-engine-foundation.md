# Coda Skill Engine Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立不改变 Coda 0.3.8 用户行为的 Skill Engine 基础层，包括 Skill Package、Run 持久化、统一循环、Guardrails、Runtime Evals 和 Runtime Adapter 契约。

**Architecture:** `src/skill/` 负责 Skill Package 加载、校验和快照；`src/engine/` 负责纯函数决策与 Run 数据；`src/runtime/` 只定义平台执行契约。`.Coda.yaml` 保存 Run 的权威状态投影，其他文件通过引用保存追加式 Trajectory、Context、Artifacts 和 Checkpoint。
...
本计划交付内部 Foundation，不切换现役 `/Coda` Skill，不创建 `/Coda-any`，不把 shell
...
src/
  ...
  engine/
    ...
    state.ts          # .Coda.yaml 投影读写，保留 legacy 字段
...
iteration: 0
pending: null
pending_ref: .Coda/pending-action.json
trajectory_ref: .Coda/trajectory.jsonl
context_ref: .Coda/context.md
artifacts_ref: .Coda/artifacts.json
checkpoint_ref: .Coda/checkpoint.json
run_status: running
run_retries: "{}"
```
...
describe('loadSkillPackage', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-skill-load-'));
    await fs.mkdir(path.join(root, 'Coda'), { recursive: true });
    await fs.writeFile(path.join(root, 'SKILL.md'), '# Demo\n');
    await fs.writeFile(
      path.join(root, 'Coda', 'skill.yaml'),
      [
        'apiVersion: Coda/v1alpha1',
        'kind: Skill',
        'metadata:',
  ...
  it('loads explicit guardrails and runtime evals', async () => {
    await fs.writeFile(
      path.join(root, 'Coda', 'guardrails.yaml'),
      'allowedSkills: [writing-plans]\nallowedAgents: []\nallowedTools: []\nmaxIterations: 8\nmaxRetriesPerAction: 2\nconfirmationRequiredFor: []\n',
    );
    await fs.writeFile(
      path.join(root, 'Coda', 'evals.yaml'),
...
git commit -m "feat(engine): load Coda Skill packages"
```
...
export function validateSkillPackage(pkg: SkillPackage): string[] {
  ...
  if (definition.apiVersion !== 'Coda/v1alpha1') errors.push('unsupported apiVersion');
...
describe('Skill snapshots', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-snapshot-'));
...
export async function createSkillSnapshot(
  ...
  const snapshotDir = path.join(changeDir, '.Coda', 'skill-snapshot');
...
### Task 5: 扩展 `.Coda.yaml` schema 并实现保留式读写

**Files:**
- Create: `src/engine/state.ts`
- Create: `test/ts/engine-state.test.ts`
- Modify: `assets/skills/Coda/scripts/Coda-state.sh`
- Modify: `assets/skills/Coda/scripts/Coda-yaml-validate.sh`
...
describe('engine state projection', () => {
  ...
  beforeEach(async () => {
    changeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-engine-state-'));
    await fs.writeFile(
  ...
  it('round-trips run fields and preserves legacy and unknown fields', async () => {
    ...
    const raw = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
...
export async function readRunState(changeDir: string): Promise<RunState | null> {
  const file = path.join(changeDir, '.Coda.yaml');
...
export async function writeRunState(changeDir: string, state: RunState): Promise<void> {
  ...
  const temporary = path.join(changeDir, `.Coda.yaml.${randomUUID()}.tmp`);
...
在 `Coda-state.sh` 的 `cmd_set` 白名单加入：
...
在 `Coda-yaml-validate.sh` 的 `KNOWN_KEYS` 和 `src/commands/doctor.ts` 的
...
const sourceScripts = path.resolve('assets', 'skills', 'Coda', 'scripts');

function bash(): string | null {
  const candidates = [
    process.env.Coda_TEST_BASH,
    process.env.Coda_BASH,
    'bash',
...
describe.skipIf(!bash())('Skill Engine shell schema compatibility', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-engine-shell-'));
    await fs.mkdir(path.join(root, 'assets'), { recursive: true });
    for (const name of ['Coda-state.sh', 'Coda-yaml-validate.sh']) {
      await fs.copyFile(path.join(sourceScripts, name), path.join(root, 'assets', name));
    }
    stateScript = bashPath(path.join(root, 'assets', 'Coda-state.sh'));
    validateScript = bashPath(path.join(root, 'assets', 'Coda-yaml-validate.sh'));
  });
...
在 `test/ts/Coda-scripts.test.ts` 中所有代表完整 `.Coda.yaml` 的 fixture 增加
...
```bash
npx vitest run test/ts/engine-state.test.ts test/ts/engine-schema-compat.test.ts test/ts/Coda-scripts.test.ts test/ts/doctor.test.ts
```
...
```bash
git add src/engine/state.ts test/ts/engine-state.test.ts test/ts/engine-schema-compat.test.ts assets/skills/Coda/scripts/Coda-state.sh assets/skills/Coda/scripts/Coda-yaml-validate.sh src/commands/doctor.ts test/ts/Coda-scripts.test.ts test/ts/doctor.test.ts
git commit -m "feat(engine): persist Run state in .Coda.yaml"
```
...
describe('run store', () => {
  ...
  it('appends trajectory events and atomically round-trips run files', async () => {
    ...
    expect(await readArtifacts(changeDir, '.Coda/artifacts.json')).toEqual({ report: 'report.md' });
    expect(await readPendingAction(changeDir, '.Coda/pending-action.json')).toEqual(action);
    expect((await fs.readFile(path.join(changeDir, '.Coda/trajectory.jsonl'), 'utf8')).trim()).toBe(JSON.stringify(event));
  });
...
describe('Skill Engine Foundation integration', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-foundation-'));
    skillDir = path.join(root, 'skill');
    changeDir = path.join(root, 'change');
    await fs.mkdir(path.join(skillDir, 'Coda'), { recursive: true });
    await fs.mkdir(changeDir, { recursive: true });
    await fs.writeFile(path.join(skillDir, 'SKILL.md'), '# Demo\n');
    await fs.writeFile(
      path.join(skillDir, 'Coda', 'skill.yaml'),
      [
        'apiVersion: Coda/v1alpha1',
        'kind: Skill',
        'metadata: { name: demo, version: "1", description: Demo }',
    ...
    await fs.writeFile(path.join(changeDir, '.Coda.yaml'), 'workflow: full\nphase: build\n');
  });
...
- **Coda Skill Engine Foundation**: 新增 Skill Package 加载、边界校验、稳定快照、Run 状态、Trajectory、Context、Artifacts、Checkpoints、Guardrails、Runtime Evals 和 Runtime Adapter 契约，为后续 classic 迁移及 Agentic Skill 编排提供单一运行时基础。

### Changed

- **`.Coda.yaml` 运行投影**: 在保留 0.3.8 字段和行为的同时接受 Skill、Orchestration 与 Run 引用字段，使旧 change 和新引擎状态可以渐进共存。
...
- Spec §7：`.Coda.yaml` 是唯一 State；其他文件由引用连接；Checkpoint 不复制可修改 State。
- Spec §11：classic 尚未迁移，0.3.8 行为不得改变。
- Spec §12：禁止内联 shell、限制 script Tool 路径、动作先过 Guardrails。
- Memory Provider、CLI、classic 自动迁移、Skill Eval benchmark、`/Coda-any` 明确不在本计划。
- 无 `.Coda.flow.yaml`、Capability、Policy、Strategy、Evaluator 等废弃模型。

## Execution Handoff

Plan 1 完成后不要直接开始 Plan 2。先验证真实接口和 0.3.8 回归，再编写
`Coda-classic-migration` 的详细计划。
