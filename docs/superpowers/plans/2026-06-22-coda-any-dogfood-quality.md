# Coda Any Dogfood Quality Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `/Coda-any` dogfoodable end to end: real local Skill preferences resolve through CLI-backed recovery, generated Coda-native Skills carry useful workflow guidance and eval metadata, and review/publish summaries expose evidence and blockers.

**Architecture:** Keep `/Coda-any` as the user entry and keep Bundle Factory state as the authoring source of truth. Extend current `domains/bundle/*` and `domains/factory/*` modules rather than introducing a second state model; generated entry Skills embed Engine files under `skills/<entry>/Coda/`, while Bundle-level `engine.enabled` remains outside the Factory main path.
...
- `.Coda/skills.txt` 偏好必须解析为本地真实 Skill 内容。
- `.Coda/skills.txt` 的行顺序是推荐调用顺序；偏离时必须说明原因。
...
- `domains/factory/package.ts`: enrich generated `SKILL.md`, write `Coda/eval.yaml`, and keep `reference/resolved-skills.json` as structured evidence.
...
- `assets/skills-zh/Coda-any/SKILL.md` and references: update Chinese user-facing workflow after behavior lands.
- `assets/skills/Coda-any/SKILL.md` and references: English parity after Chinese approval.
...
describe('Bundle review summary readiness', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-review-summary-'));
  });
...
const risks = [
  ...
  '- 偏离 `.Coda/skills.txt` 顺序会降低用户偏好可预测性，必须在 review summary 中解释。',
].join('\n');
...
**Interfaces:**
- Produces: `skills/<entry>/Coda/eval.yaml` for Engine-enabled generated packages.
...
it('writes an authoring-skill eval manifest for Engine-enabled generated packages', async () => {
  ...
  expect(manifest).toContain('apiVersion: Coda.eval/v1alpha1');
...
```ts
const evalManifestPath = path.join(CodaRoot, 'eval.yaml');
...
```ts
evalManifestPath: plan.engineMode === 'none' ? null : path.join(CodaRoot, 'eval.yaml'),
```
...
### Task 6: Bilingual `/Coda-any` Skill Guidance
...
In `test/ts/Coda-any-skill.test.ts`, add Chinese contract phrases:
...
Add to `assets/skills-zh/Coda-any/SKILL.md`:
...
生成 Engine-enabled Skill 时，Factory 会写入 `Coda/eval.yaml`，默认使用 `authoring-skill`
...
Add equivalent backend detail to `assets/skills-zh/Coda-any/reference/bundle-authoring.md`.
...
For Engine-enabled generated Skills, Factory writes `Coda/eval.yaml` using the `authoring-skill`
...
```bash
git add assets/skills-zh/Coda-any assets/skills/Coda-any test/ts/Coda-any-skill.test.ts
git commit -m "docs(skill): document Coda-any readiness gates"
```
...
- Spec coverage: Tasks cover Dogfood E2E, CLI-backed candidate recovery, generated Skill quality, `resolved-skills.json`, eval manifest, review summary readiness, hash-bound publish gates, and bilingual `/Coda-any` behavior.
...
Plan complete and saved to `docs/superpowers/plans/2026-06-22-Coda-any-dogfood-quality.md`. Two execution options:
