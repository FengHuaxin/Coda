...
- 不新增第二套发布状态；Bundle authoring state 仍是 `/Coda-any` 生成、评估、审核、发布的事实源。
- 不改 Classic Coda `.Coda.yaml` 状态机。
- 不把 `hooks/*.yaml` 当作平台原生配置；它只是 Coda portable hook descriptor。
...
- 新生成物写 `Coda/checks.yaml`；迁移期 loader 继续读取旧 `Coda/evals.yaml`。
- 组合展开在创建阶段完成；运行时只执行最终 `Coda/skill.yaml`。
...
- Modify: `domains/skill/load.ts`
  - Load `Coda/checks.yaml` first.
  - Fall back to `Coda/evals.yaml`.
...
- Create: `domains/bundle/factory-compose.ts`
  - Parse source Skill `Coda/flow.yaml`.
  - Expand `use` steps.
...
- Modify: `domains/factory/package.ts`
  ...
  - Generate `scripts/Coda-plan.mjs`.
  - Generate `scripts/Coda-check.mjs`.
  - Generate `scripts/Coda-hook-guard.mjs`.
...
- Modify: `README-zh.md`
- Modify: `README.md`
- Modify: `test/ts/Coda-any-skill.test.ts`
...
- `loadSkillPackage(root)` must prefer `Coda/checks.yaml`.
- Old packages with only `Coda/evals.yaml` must still load.
- New factory packages must write `Coda/checks.yaml` and `Coda/eval.yaml`.
...
```ts
it('loads runtime checks from Coda/checks.yaml', async () => {
...
it('keeps loading legacy Coda/evals.yaml during migration', async () => {
  const root = await writeSkillPackage({
...
it('rejects packages that define both checks.yaml and evals.yaml', async () => {
  ...
  await fs.writeFile(path.join(root, 'Coda', 'evals.yaml'), 'runtime: []\n');
...
async function writeSkillPackage(options: {
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-skill-load-'));
  const CodaRoot = path.join(root, 'Coda');
  await fs.mkdir(CodaRoot, { recursive: true });
  await fs.writeFile(path.join(root, 'SKILL.md'), '# Demo\n');
  await fs.writeFile(path.join(CodaRoot, 'skill.yaml'), baseSkillYaml(), 'utf8');
  await fs.writeFile(path.join(CodaRoot, 'guardrails.yaml'), 'allowedSkills: []\n', 'utf8');
  if (options.runtimeChecksFile && options.runtimeChecks) {
    await fs.writeFile(
      path.join(CodaRoot, options.runtimeChecksFile),
...
```ts
async function readRuntimeChecks(CodaRoot: string): Promise<{
  document: { runtime?: RuntimeEvalDefinition[] } | null;
  sourcePath: string | null;
}> {
  const checksPath = path.join(CodaRoot, 'checks.yaml');
  const evalsPath = path.join(CodaRoot, 'evals.yaml');
...
```ts
await fs.writeFile(path.join(CodaRoot, 'checks.yaml'), stringify(runtimeEvals()), 'utf8');
await fs.writeFile(path.join(CodaRoot, 'eval.yaml'), stringify(evalManifest(plan)), 'utf8');
```
...
```ts
await expect(fs.access(path.join(output.packageRoot, 'Coda', 'checks.yaml'))).resolves.toBeUndefined();
await expect(fs.access(path.join(output.packageRoot, 'Coda', 'evals.yaml'))).rejects.toMatchObject({
  code: 'ENOENT',
});
expect(output.evalManifestPath).toBe(path.join(output.packageRoot, 'Coda', 'eval.yaml'));
```
...
async function writeSkill(root: string, name: string, flow?: string): Promise<BundleCandidateSource> {
  ...
  await fs.mkdir(path.join(skillRoot, 'Coda'), { recursive: true });
  await fs.writeFile(path.join(skillRoot, 'SKILL.md'), `---\nname: ${name}\ndescription: ${name}\n---\n`);
  if (flow) await fs.writeFile(path.join(skillRoot, 'Coda', 'flow.yaml'), flow);
  return {
    name,
...
async function readFlowSteps(root: string): Promise<Array<{ use: string } | { choose: { id: string; options: string[] } }> | null> {
  const flowPath = path.join(root, 'Coda', 'flow.yaml');
  let source: string;
  try {
...
await fs.mkdir(path.join(skillRoot, 'Coda'), { recursive: true });
await fs.writeFile(
  path.join(skillRoot, 'Coda', 'flow.yaml'),
  `steps:
  - use: brainstorming
  - use: writing-plans
`,
...
skills/<entry>/Coda/skill.yaml
skills/<entry>/Coda/guardrails.yaml
skills/<entry>/Coda/checks.yaml
skills/<entry>/Coda/eval.yaml
skills/<entry>/reference/resolved-skills.json
skills/<entry>/reference/composition-report.md
skills/<entry>/scripts/Coda-plan.mjs
skills/<entry>/scripts/Coda-check.mjs
skills/<entry>/scripts/Coda-hook-guard.mjs
...
it('generates the required stable composition control plane files', async () => {
  ...
  await expect(fs.access(path.join(output.packageRoot, 'scripts', 'Coda-plan.mjs'))).resolves.toBeUndefined();
  await expect(fs.access(path.join(output.packageRoot, 'scripts', 'Coda-check.mjs'))).resolves.toBeUndefined();
  await expect(fs.access(path.join(output.packageRoot, 'scripts', 'Coda-hook-guard.mjs'))).resolves.toBeUndefined();
  expect(output.controlPlane).toMatchObject({
    scripts: expect.arrayContaining([
      expect.stringContaining('Coda-plan.mjs'),
      expect.stringContaining('Coda-check.mjs'),
      expect.stringContaining('Coda-hook-guard.mjs'),
    ]),
  });
});
```
...
const statePath = path.resolve(process.cwd(), '.Coda', 'runs', 'state.json');
...
async function main() {
  ...
  if (command === 'init') {
    const skillYaml = await fs.readFile(path.resolve(process.cwd(), 'Coda', 'skill.yaml'), 'utf8');
...
resources: {
  ...
  scripts: [
    {
      id: 'Coda-plan',
      path: `skills/${skillId}/scripts/Coda-plan.mjs`,
      sideEffect: 'write',
      runtime: 'node',
    },
    {
      id: 'Coda-check',
      path: `skills/${skillId}/scripts/Coda-check.mjs`,
      sideEffect: 'read',
      runtime: 'node',
    },
    {
      id: 'Coda-hook-guard',
      path: `skills/${skillId}/scripts/Coda-hook-guard.mjs`,
      sideEffect: 'read',
      runtime: 'node',
    },
  ],
...
- `Coda-check.mjs` must exit 0 when required files exist.
- `Coda-check.mjs` must exit non-zero when a required file is missing.
- `Coda-plan.mjs init/status/complete-step` must write and read deterministic state.
- `Coda-hook-guard.mjs` must fail closed when state is absent or invalid.
...
describe('generated factory control-plane scripts', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-control-plane-scripts-'));
  });
  ...
  it('validates required generated files with Coda-check.mjs', async () => {
    const output = await generateFactorySkillPackage(basePlan(root));
    const script = path.join(output.packageRoot, 'scripts', 'Coda-check.mjs');
  ...
  it('fails closed when a required file is missing', async () => {
    const output = await generateFactorySkillPackage(basePlan(root));
    await fs.rm(path.join(output.packageRoot, 'Coda', 'skill.yaml'));

    const result = node(path.join(output.packageRoot, 'scripts', 'Coda-check.mjs'), ['verify'], output.packageRoot);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('Coda/skill.yaml');
  });

  it('supports init, status, and complete-step state operations', async () => {
    const output = await generateFactorySkillPackage(basePlan(root));
    const script = path.join(output.packageRoot, 'scripts', 'Coda-plan.mjs');
  ...
  it('blocks hook execution when state is missing', async () => {
    const output = await generateFactorySkillPackage(basePlan(root));
    const result = node(path.join(output.packageRoot, 'scripts', 'Coda-hook-guard.mjs'), ['before_write'], output.packageRoot);
...
- `Coda-check.mjs verify`:
  - reads required relative paths.
  - prints `control-plane-ok` on success.
  - prints missing path to stderr and exits 1 on failure.
- `Coda-plan.mjs init/status/complete-step`:
  - stores state at `.Coda/runs/state.json` under the Skill package cwd.
  - includes `schemaVersion`, `status`, `planHash`, `currentStep`, `completedSteps`.
- `Coda-hook-guard.mjs before_write`:
  - reads `.Coda/runs/state.json`.
...
it('rejects passing eval evidence when a factory Bundle lacks required control-plane files', async () => {
  ...
  await fs.rm(path.join(state.draftPath, 'skills', 'stable-missing-control', 'scripts', 'Coda-check.mjs'));
...
### Task 8: Update `/Coda-any`, Eval Docs, README, and CLI Help
...
- Ordinary user docs must say `/Coda-any -> Coda eval -> Coda publish -> distribute`.
...
- CLI help must describe `Coda skill` and `Coda bundle` as low-level/advanced surfaces.
...
expect(zhSkill).toContain('Coda/checks.yaml');
expect(zhSkill).toContain('Coda/eval.yaml');
expect(zhSkill).toContain('Coda publish distribute');
...
```ts
expect(readmeZh).toContain('/Coda-any');
expect(readmeZh).toContain('`Coda eval`');
expect(readmeZh).toContain('`Coda publish`');
...
```bash
npx vitest run test/ts/Coda-any-skill.test.ts test/ts/readme.test.ts test/app/cli-help.test.ts
```
...
```text
/Coda-any 生成的是稳定组合 Skill Bundle，不只是 SKILL.md。
生成物包含 SKILL.md、Coda/skill.yaml、Coda/guardrails.yaml、Coda/checks.yaml、Coda/eval.yaml、scripts、rules、hooks、reference 和 bundle.yaml。
hooks/*.yaml 是 Coda portable hook descriptor，只有通过 Coda publish distribute 编译到目标平台后才会生效。
```
...
```text
/Coda-any generates a stable composed Skill Bundle, not just a SKILL.md file.
The Bundle includes SKILL.md, Coda/skill.yaml, Coda/guardrails.yaml, Coda/checks.yaml, Coda/eval.yaml, scripts, rules, hooks, references, and bundle.yaml.
hooks/*.yaml files are Coda portable hook descriptors and become active only after Coda publish distribute compiles them into a target platform configuration.
```
...
```md
- `/Coda-any` 现在按稳定组合 Skill Bundle 路径工作：创建后通过 `Coda eval` 验证，再通过 `Coda publish` 审核、发布和分发。完整说明见 `docs/operations/SKILL-CREATION-ZH.md`。
```
...
Keep `Coda publish` as the user-facing publishing path.
...
```bash
git add assets/skills-zh/Coda-any/SKILL.md assets/skills-zh/Coda-any/reference/bundle-authoring.md assets/skills-zh/Coda-any/reference/eval-provider.md assets/skills/Coda-any/SKILL.md assets/skills/Coda-any/reference/bundle-authoring.md assets/skills/Coda-any/reference/eval-provider.md docs/operations/SKILL-CREATION-ZH.md docs/operations/EVAL-USAGE-ZH.md README-zh.md README.md app/cli/index.ts test/ts/Coda-any-skill.test.ts test/ts/readme.test.ts test/app/cli-help.test.ts
...
- Modify: `CHANGELOG.md`
- Check: `package.json`
- Check: `docs/superpowers/plans/2026-06-23-Coda-stable-composed-skill-control-plane.md`
...
- **Stable composed Skill control plane**: Generated `/Coda-any` Skill Bundles now include the required scripts, rules, hooks, runtime checks, Eval manifest, and composition evidence needed for resumable and guarded execution.
- **Factory Skill composition**: Added creation-time expansion for source Skill `Coda/flow.yaml` files, including choice resolution and cycle detection before compiling the final Plan.

### Changed

- **Runtime checks naming**: New Engine-native packages use `Coda/checks.yaml` for runtime checks while retaining legacy `Coda/evals.yaml` loading during migration.
...
```bash
git add CHANGELOG.md package.json docs/superpowers/plans/2026-06-23-Coda-stable-composed-skill-control-plane.md
