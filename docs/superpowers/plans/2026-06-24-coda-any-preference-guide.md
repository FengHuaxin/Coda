# Coda Any Preference Guide Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the project-level Skill preference guide for `/Coda-any`, replacing the unpublished `.Coda/skills.txt` user path with `.Coda/skill-preferences.yaml`, inventory-driven recommendations, proposal confirmation, and preference-aware readiness.
...
This plan implements one vertical feature: project-level Skill preferences for `/Coda-any`. It touches several modules, but each task produces a working, testable slice:

1. Parse `.Coda/skill-preferences.yaml`.
2. Build user-facing Skill inventory.
3. Replace `.Coda/skills.txt` default preference flow.
4. Add Factory proposal and metadata support.
5. Enforce advisory/strict policies in generation and review.
6. Update generated evidence, docs, and bilingual `/Coda-any` guidance.
...
- Create `domains/skill/preferences.ts`
  - Parses and normalizes `.Coda/skill-preferences.yaml`.
...
- Modify `domains/skill/find.ts`
  - Remove default `.Coda/skills.txt` reading.
...
- Modify `domains/bundle/factory-plan.ts`
  - Normalize plans against `.Coda/skill-preferences.yaml`.
...
- Modify `app/commands/bundle.ts` and `app/cli/index.ts`
  - Add `Coda bundle factory-propose <name> --file <plan.json> --json` as `/Coda-any` backend.
...
describe('project skill preferences', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-skill-preferences-'));
  ...
  it('returns null when .Coda/skill-preferences.yaml is missing', async () => {
  ...
  it('normalizes mode, preferences, required Skills, policies and hash', async () => {
    await fs.writeFile(
      path.join(projectRoot, '.Coda', 'skill-preferences.yaml'),
      `version: 1
mode: strict
prefer:
  - brainstorming
...
`,
  ...
  it('applies defaults for optional fields', () => {
    const result = normalizeSkillPreferencesDocument({ version: 1 }, '.Coda/skill-preferences.yaml');
  ...
  it('rejects invalid version, mode and policy values', () => {
    expect(() =>
      normalizeSkillPreferencesDocument({ version: 2 }, '.Coda/skill-preferences.yaml'),
...
export function normalizeSkillPreferencesDocument(
  value: unknown,
  file = '.Coda/skill-preferences.yaml',
...
export function skillPreferencesPath(projectRoot: string): string {
  return path.resolve(projectRoot, '.Coda', 'skill-preferences.yaml');
}
...
- [x] **Step 1: Update Skill finder tests to remove `.Coda/skills.txt` default behavior**
...
```ts
it('scans all Skill roots when preferences are omitted instead of reading .Coda/skills.txt', async () => {
  await fs.writeFile(path.join(projectRoot, '.Coda', 'skills.txt'), 'ignored-skill\n');
...
describe('skill inventory', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-skill-inventory-'));
    projectRoot = path.join(root, 'project');
...
Expected: FAIL because finder still reads `.Coda/skills.txt` and inventory does not exist.
  ...
  return found
    .map((skill): SkillInventoryItem => {
      ...
      return {
        ...
        recommended,
        reason: recommended
          ? 'Recommended default Skill for Coda-style guided workflows.'
...
- [x] **Step 1: Rewrite Bundle candidate tests around `.Coda/skill-preferences.yaml`**

Modify `test/domains/bundle/bundle-candidates.test.ts`:

- Replace `.Coda/skills.txt` fixture writes with `.Coda/skill-preferences.yaml`.
...
it('reads project Skill preferences from .Coda/skill-preferences.yaml', async () => {
  await fs.writeFile(
    path.join(projectRoot, '.Coda', 'skill-preferences.yaml'),
    `version: 1
mode: strict
...
Expected: FAIL because `readBundleSkillPreferences` does not exist and `.Coda/skills.txt` behavior changed.
...
Leave `discoverBundleCandidates({ preferences?: string[] | null })` stable. It should receive names from `readBundleSkillPreferences` or explicit plan values. No `.Coda/skills.txt` reading should happen inside candidate discovery.
...
`,
  ...
  expect(state.factory).toMatchObject({
    ...
    preferencePath: path.join(projectRoot, '.Coda', 'skill-preferences.yaml'),
...
`,
  ...
  expect(result).toMatchObject({
    ...
    preference: {
      mode: 'advisory',
      source: path.join(projectRoot, '.Coda', 'skill-preferences.yaml'),
    },
...
bundle
  ...
  .description('Preview a /Coda-any Factory proposal without writing Bundle state')
...
git commit -m "feat: preview Coda-any factory proposals"
```
...
### Task 8: Remove `.Coda/skills.txt` User Path From Active Docs and Skill Assets
...
- Replace expectations for `.Coda/skills.txt` with `.Coda/skill-preferences.yaml`.
...
```ts
expect(combined).not.toContain('.Coda/skills.txt');
```

Modify bilingual parity to include English equivalents:

```ts
{ zh: '.Coda/skill-preferences.yaml', en: '.Coda/skill-preferences.yaml' },
...
Expected: FAIL because assets still mention `.Coda/skills.txt`.

- [x] **Step 3: Update Chinese `/Coda-any` Skill assets**
...
- Describe `/Coda-any` as “Skill 创建向导”.
- Replace `.Coda/skills.txt` with `.Coda/skill-preferences.yaml`.
- Explain that the file is project-level, user-editable, and can be generated by `/Coda-any`.
- Add `Coda bundle factory-propose <name> --file <plan.json> --json` before `factory-init`.
...
- Keep `/Coda-any -> Coda eval -> Coda publish -> distribute`.
- Keep scripts/rules/hooks as required control plane.

- [x] **Step 4: Update English `/Coda-any` Skill assets**
...
- README keeps a short `/Coda-any` note and links to operation docs.
- `SKILL-CREATION-ZH.md` covers:
  - one-sentence creation
  - first preference setup
  - hand-written `.Coda/skill-preferences.yaml`
  - proposal preview
  - Eval
  - Publish
  - Distribute
...
- Remove active-doc main-flow references to `.Coda/skills.txt`.
...
```bash
git add assets/skills-zh/Coda-any assets/skills/Coda-any docs/operations/SKILL-CREATION-ZH.md docs/operations/EVAL-USAGE-ZH.md README-zh.md README.md test/ts/Coda-any-skill.test.ts test/ts/readme.test.ts
git commit -m "docs: guide Coda-any through skill preferences"
```

---

### Task 9: Replace Remaining `.Coda/skills.txt` Test Fixtures
...
```ts
await fs.writeFile(path.join(projectRoot, '.Coda', 'skills.txt'), 'factory-alpha\n');
```
...
- **Coda Any preferences**: Replaced the unpublished `.Coda/skills.txt` user path with project-level `.Coda/skill-preferences.yaml`, inventory-backed preference setup, proposal preview, and preference-aware review readiness so users can create Coda-style Skills without learning internal Bundle files.

### Tests

- **Coda Any preference guide**: Added coverage for preference parsing, Skill inventory grouping, Factory proposal previews, strict/advisory policy readiness, generated preference evidence, and bilingual `/Coda-any` docs.
```
...
git commit -m "chore: document Coda-any preference guide changes"
```
...
- Keep `Coda bundle` as the advanced backend; do not make it the ordinary user path.
- Keep `flow.yaml` as internal or advanced authoring content only.
- Do not add `skill-preferences.yaml` under `.Coda/Coda-any/`; the accepted project-level path is `.Coda/skill-preferences.yaml`.
- `Coda bundle factory-propose` is a dry-run backend for `/Coda-any`; it must not write `.Coda/bundle-authoring` state.
...
- Spec coverage:
  ...
  - Remove `.Coda/skills.txt`: Tasks 2, 3, 8, 9.
  - Full-platform Skill inventory and user-level dedupe: Task 2.
  - `/Coda-any` proposal confirmation backend: Task 5.
