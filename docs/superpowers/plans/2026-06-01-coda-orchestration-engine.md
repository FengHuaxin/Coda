# Coda Skill Engine Implementation Roadmap

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 用分阶段、可验证的方式把 Coda 0.3.8 演进为同时支持确定性编排和 Agentic 编排的 Coda Skill Engine。

**Architecture:** 以 Coda Skill 为唯一组合单元，以 `.Coda.yaml` 为唯一状态真相源；每次执行产生持久化 Run，包含 State、Trajectory、Context、Artifacts 与 Checkpoints。Skills、Agents、Tools 保持不同职责，通过 Runtime Adapter 执行，并由 Guardrails 与 Evals 约束。
...
- `docs/superpowers/specs/2026-06-01-Coda-orchestration-engine-design.md`
...
批准的设计包含四个可独立验证的子系统。一次性锁定全部实现会让 `/Coda-any` 和 Eval
...
2. **Classic Migration**：固化 0.3.8 契约、自动迁移旧 change、shell 门面转发、内置
   `Coda-classic` Skill 和 baseline benchmark。
3. **Manual Authoring**：`Coda skill validate/inspect/run/resume/eval`、项目 Skill
   发现、快照与安装。
4. **`/Coda-any`**：`.Coda/skills.txt`、候选 Skill 实现探索、交互澄清、生成、
...
- `docs/superpowers/plans/2026-06-13-Coda-skill-engine-foundation.md`

完成门：

- Skill Package 可加载、校验和生成稳定 hash。
- `.Coda.yaml` 可保存新 Run 投影，同时完整保留 0.3.8 字段。
...
- `docs/superpowers/plans/2026-06-13-Coda-classic-migration.md`

必须覆盖：

- 0.3.8 full/hotfix/tweak 契约测试。
- 旧 `.Coda.yaml` 自动、幂等迁移。
- `Coda-state.sh`、guard、handoff、archive 改为 TS 引擎兼容门面。
...
- `docs/superpowers/plans/2026-06-15-Coda-skill-authoring.md`
...
- `Coda skill resume`
- `Coda skill eval`
- 内置 Skill 与项目 `.Coda/skills/` 的发现和覆盖规则。
- Skill 快照、显式升级与兼容检查。
...
- `docs/superpowers/plans/2026-06-15-Coda-any.md`

实现必须通过 `pnpm benchmark:bundle`，证明 Bundle compiler 在全部已注册平台上保持当前
Coda 的 Skill、rule、hook、reference 与目标路径合同。
...
- 原生 skill-creator 优先、Coda fallback 的 Eval Provider。
- benchmark、grader、人工评审、迭代优化。
- `draft -> eval -> review -> ready` 发布门。

## 跨阶段规则

- `.Coda.yaml` 始终是唯一状态真相源。
...
- 只有 Coda 需要编排、授权、快照、平台映射、审计或评估的依赖才进入 Skill Spec。
