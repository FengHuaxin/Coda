...
**Goal:** Progressively make Classic `/Coda` workflow Engine-native by sharing Resolver evidence across status, doctor, guard, and runtime eval without changing the familiar user entry commands.

**Architecture:** Keep `.Coda.yaml` as the user-visible projection and `.Coda/run-state.json` as the machine-owned Run state. Add a shared Classic diagnostics layer that reads the migrated projection, collected evidence, resolver step id, and runtime eval readiness; then route status, doctor, and guard through that same layer before expanding Engine-driven behavior.
...
- `.Coda.yaml` 仍保留用户字段，但 `run-state.json` 持有 machine-owned run 字段。
- Classic Resolver 成为 `Coda status`、guard、next-step hint、doctor 的共同事实源。
- 旧字段只作为 Classic projection，不再新增工作流事实。
- `.mjs` launcher 只负责定位 runtime 和分发命令；业务决策不得回流到 launcher。
- 新共享工具只能放在 `domains/Coda-classic/` 或 Engine 共享模块。
- Engine-native 迁移不能让现有 change 无法恢复或归档。
- Frozen 0.3.9 fixture 差分兼容测试继续通过。
- Classic runtime 共享逻辑必须来自 `domains/Coda-classic/`，修改后运行 `pnpm build:classic-runtime`。

---

## File Structure

- `domains/Coda-classic/classic-diagnostics.ts`: create shared diagnostics for Classic state, evidence, resolver step, next command, runtime mode, and fail-closed errors.
- `domains/Coda-classic/classic-runtime-evals.ts`: map Classic resolver steps to required evidence codes and runtime eval status.
- `domains/Coda-classic/index.ts`: export new diagnostics/eval helpers.
- `app/commands/status.ts`: consume Classic diagnostics instead of independently projecting state and next command.
- `app/commands/doctor.ts`: consume Classic diagnostics for `.Coda.yaml` checks and current step messages.
- `domains/Coda-classic/classic-guard.ts`: expose diagnostics in JSON mode and use shared runtime eval readiness for phase-exit messaging.
- `domains/Coda-classic/classic-state-command.ts`: keep `next` output aligned with resolver diagnostics.
- `assets/skills/Coda/scripts/Coda-runtime.mjs`: generated output after `pnpm build:classic-runtime`.
- `test/domains/Coda-classic/classic-diagnostics.test.ts`: cover shared diagnostics and malformed-state behavior.
...
- `test/domains/Coda-classic/classic-guard.test.ts`: cover guard JSON diagnostics and runtime eval readiness.
- `test/domains/Coda-classic/classic-migrate.test.ts`: add old change lazy migration idempotency checks.
- `test/domains/Coda-classic/Coda-scripts.test.ts`: ensure generated runtime still works.
...
**Files:**
- Create: `domains/Coda-classic/classic-diagnostics.ts`
- Modify: `domains/Coda-classic/index.ts`
- Test: `test/domains/Coda-classic/classic-diagnostics.test.ts`
...
Create `test/domains/Coda-classic/classic-diagnostics.test.ts`:
...
import { inspectClassicChange } from '../../../domains/Coda-classic/classic-diagnostics.js';
import { runClassicCli } from '../../../domains/Coda-classic/classic-cli.js';
...
describe('Classic diagnostics', () => {
  ...
  it('returns resolver step, evidence, and next command from one source', async () => {
    ...
    expect(diagnostic.nextCommand).toBe('/Coda-open');
  ...
  it('fails closed with an error instead of throwing to callers', async () => {
    await fs.appendFile(path.join(changeDir, '.Coda.yaml'), '\nunknown_field: true\n');
...
```bash
npx vitest run test/domains/Coda-classic/classic-diagnostics.test.ts
```
...
Export from `domains/Coda-classic/index.ts`:
...
```bash
git add domains/Coda-classic/classic-diagnostics.ts domains/Coda-classic/index.ts test/domains/Coda-classic/classic-diagnostics.test.ts
...
- Produces: doctor `.Coda.yaml` message `valid (step: <step>, mode: engine-projection)`.
...
it('uses Classic diagnostics for Coda yaml validity messages', async () => {
  ...
  const payload = JSON.parse(json);
  const CodaYaml = payload.results.find((item: { check: string }) =>
    item.check === '.Coda.yaml: demo',
  );
...
In `app/commands/doctor.ts`, replace the `.Coda.yaml` check body with:
...
**Files:**
- Create: `domains/Coda-classic/classic-runtime-evals.ts`
- Modify: `domains/Coda-classic/classic-diagnostics.ts`
- Modify: `domains/Coda-classic/index.ts`
- Test: `test/domains/Coda-classic/classic-runtime-evals.test.ts`
- Test: `test/domains/Coda-classic/classic-diagnostics.test.ts`
...
Create `test/domains/Coda-classic/classic-runtime-evals.test.ts`:

```ts
import { describe, expect, it } from 'vitest';
import { evaluateClassicRuntimeStep } from '../../../domains/Coda-classic/classic-runtime-evals.js';
...
```bash
npx vitest run test/domains/Coda-classic/classic-runtime-evals.test.ts
```
...
Export it from `domains/Coda-classic/index.ts`:
...
In `domains/Coda-classic/classic-diagnostics.ts`, import:
...
```bash
npx vitest run test/domains/Coda-classic/classic-runtime-evals.test.ts test/domains/Coda-classic/classic-diagnostics.test.ts
```
...
```bash
git add domains/Coda-classic/classic-runtime-evals.ts domains/Coda-classic/classic-diagnostics.ts domains/Coda-classic/index.ts test/domains/Coda-classic/classic-runtime-evals.test.ts test/domains/Coda-classic/classic-diagnostics.test.ts
...
**Files:**
- Modify: `domains/Coda-classic/classic-guard.ts`
- Test: `test/domains/Coda-classic/classic-guard.test.ts`

**Interfaces:**
- Consumes: `ClassicCommandOptions.json`.
- Produces: JSON payload `{ phase, change, currentStep, runtimeEval, blocked, checks }` when `Coda-runtime guard <change> <phase> --json` is used.
- Preserves: existing stderr output and exit codes for non-JSON guard calls.

- [x] **Step 1: Write failing guard JSON test**

Append to `test/domains/Coda-classic/classic-guard.test.ts`:
...
```bash
npx vitest run test/domains/Coda-classic/classic-guard.test.ts
```
...
In `domains/Coda-classic/classic-guard.ts`, import:
...
```bash
git add domains/Coda-classic/classic-guard.ts test/domains/Coda-classic/classic-guard.test.ts
...
**Files:**
- Modify: `test/domains/Coda-classic/classic-migrate.test.ts`
- Modify: `domains/Coda-classic/classic-migrate.ts` only when the idempotency test proves the current migration rewrites state on repeated reads.

**Interfaces:**
- Consumes: legacy `.Coda.yaml` without `classic_profile`, `classic_migration`, and Run fields.
...
Append to `test/domains/Coda-classic/classic-migrate.test.ts`:

```ts
it('migrates old Classic state idempotently on repeated reads', async () => {
  await fs.writeFile(
    path.join(changeDir, '.Coda.yaml'),
    `workflow: full
phase: build
...
`,
  ...
  const afterFirst = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
  const second = await ensureClassicRun(changeDir, { skillPackage: pkg });
  const afterSecond = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
...
```bash
npx vitest run test/domains/Coda-classic/classic-migrate.test.ts
```
...
If the idempotency test fails, update `domains/Coda-classic/classic-migrate.ts` so `ensureClassicRun()` returns the existing run when `classic_migration` is current:
...
Do not add new `.Coda.yaml` fields.
...
```bash
git add test/domains/Coda-classic/classic-migrate.test.ts domains/Coda-classic/classic-migrate.ts
...
**Files:**
- Modify: `assets/skills/Coda/scripts/Coda-runtime.mjs`
- Modify: `docs/architecture/ARCHITECTURE.md`
- Modify: `docs/superpowers/specs/2026-06-22-Coda-priority-improvements-design.md` if implementation reveals a spec correction.
...
Expected: `assets/skills/Coda/scripts/Coda-runtime.mjs` updates only if generated runtime content changed.
...
```bash
npx vitest run \
  test/domains/Coda-classic/classic-diagnostics.test.ts \
  test/domains/Coda-classic/classic-runtime-evals.test.ts \
  test/domains/Coda-classic/classic-resolver.test.ts \
  test/domains/Coda-classic/classic-migrate.test.ts \
  test/domains/Coda-classic/classic-guard.test.ts \
  test/domains/Coda-classic/Coda-scripts.test.ts \
...
Classic `/Coda` commands keep their existing user-facing entrypoints, but status, doctor, guard,
and runtime transitions share the Classic Resolver as the current workflow fact source. `.Coda.yaml`
remains the user-visible projection, while `.Coda/run-state.json` owns machine run state and
...
```bash
git add assets/skills/Coda/scripts/Coda-runtime.mjs docs/architecture/ARCHITECTURE.md
...
- Risk control: Existing `.Coda.yaml` projection remains; no new user state field is added; 0.3.9 compatibility and generated runtime freshness are verified.
