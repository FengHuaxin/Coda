...
依据 spec：`docs/superpowers/specs/2026-06-28-Coda-any-authoring-protocol-design.md`。

每阶段结束都要 `pnpm format:check && pnpm lint && pnpm build && pnpm test`（或对应子集）保持绿。改 `domains/factory/**` 或 `domains/bundle/**` 后无需重建 classic-runtime（那是 Coda-classic 专属），但 CLI 走 `dist/`，需 `pnpm build`。
...
0.2 **修 find-skill（H1）** `assets/skills/Coda-any/SKILL.md` + `assets/skills-zh/Coda-any/SKILL.md`
- en step 2：`use find-skill` → `use Coda bundle candidates --json to discover real Skills, then Coda skill show <name> --json to read real content and hash`。
- zh 对应行同步。

0.3 **对齐脚本清单（H2）** `assets/skills/Coda-any/reference/bundle-authoring.md`、`subagents/script-author.md`、`subagents/skill-reviewer.md`（+ zh 镜像）
- "Generated Package" / script-author "Must cover" / skill-reviewer blocking 条件：从 3 个 workflow-* 扩到 6 个（加 Coda-plan/Coda-check/Coda-hook-guard）。说明 Coda-plan/Coda-check/Coda-hook-guard 由确定性生成器产出，script-author 产它们的 **契约描述**，skill-reviewer 校验其存在。

0.4 **契约测试（M5）** `test/domains/bundle/Coda-any-skill-contract.test.ts`（新）
- 解析 `app/cli/index.ts` 注册命令名集合（静态读取或构造 program 遍历）。
- 断言 SKILL.md/reference 里出现的 `Coda bundle <x>` / `Coda publish <x>` / `Coda skill <x>` 引用 ⊆ 注册命令。
- 断言全文不含 `find-skill`。
- 断言 Coda-any 声明的产物清单 == `generateFactorySkillPackage` 实际产物路径集合（跑一次生成，对比 artifact paths）。
...
- `loadBuiltinAuthoringProtocol()`：读 Coda-any skill 自带 `reference/authoring-protocol.json`（定位方式同 candidates 发现 skill 根）。
...
1.3 **schema 文件** `assets/skills/Coda-any/reference/schemas/{script-lane,reference-lane,workflow-entry-lane,skill-core-lane,pause-points-lane,skill-review-lane}.schema.json` + zh 镜像。统一形态见 spec。

1.4 **authoring-protocol.json** `assets/skills/Coda-any/reference/authoring-protocol.json` + zh 镜像。内容见 spec。
