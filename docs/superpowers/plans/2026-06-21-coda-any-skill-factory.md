# Coda Any Skill Factory Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rework `/Coda-any` into a user-facing Coda Skill Factory that resolves real local preferred Skills, preserves preferred call order, generates Engine-aware Coda-native Skill packages, and uses Bundle/Skill CLI services as internal backends.

**Architecture:** Add a shared `find-skill` layer under `src/skill/`, bridge existing Bundle candidate discovery onto it, extend Bundle authoring state with Factory metadata, add a focused Factory generator for Engine-aware package files, then update `/Coda-any` bilingual Skill guidance. Keep existing `Coda bundle` and `Coda skill` commands working; new behavior layers on top of current Bundle and Skill Engine services.
...
- Resolve `.Coda/skills.txt` entries into real local Skill implementations with `preferenceIndex`.
- Preserve preferred order through candidate discovery and Factory metadata.
- Generate a minimal valid Coda-native Skill package from a Factory plan.
- Record ordered call-chain deviations with explicit reasons.
- Update `/Coda-any` so users see Skill-only creation while CLI remains an internal backend.

This plan does not implement a full LLM creator, full adaptive runtime, or real platform-specific Engine runner execution. Runner support is represented as generated Skill guidance and package metadata; runtime execution continues through the existing `Coda skill run/resume` contract, and dedicated runner execution belongs to a separate follow-up plan.
...
- `src/factory/package.ts`: writes a minimal Coda-native Skill package from a Factory package plan.
- `src/commands/bundle.ts`: includes preference order and source details in JSON output; no interactive model behavior.
- `assets/skills-zh/Coda-any/SKILL.md` and references: Chinese Skill Factory workflow.
- `assets/skills/Coda-any/SKILL.md` and references: English parity after Chinese behavior is settled.
...
- `test/ts/Coda-any-skill.test.ts`: bilingual Skill guidance contract.
...
describe('findPreferredSkills', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-skill-find-'));
  ...
  it('parses preference order, comments, duplicates and explicit paths', async () => {
    ...
    await fs.writeFile(
      path.join(projectRoot, '.Coda', 'skills.txt'),
      `
...
export async function readSkillPreferenceEntries(
  ...
  const preferencesPath = path.resolve(projectRoot, '.Coda', 'skills.txt');
  let source: string;
  try {
...
function searchRoots(options: Required<Pick<FindPreferredSkillsOptions, 'projectRoot'>> & {
  ...
  return [
    { root: path.resolve(options.projectRoot, '.Coda', 'skills'), origin: 'project' },
    ...platformRoots(options.projectRoot, options.homeDir),
    { root: options.builtinRoot, origin: 'builtin', platform: 'Coda' },
    ...options.extraRoots,
  ];
}
...
## Task 4: Generate Minimal Coda-native Skill Packages
...
describe('generateFactorySkillPackage', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-factory-package-'));
  });
  ...
  it('writes a valid deterministic Coda-native Skill package', async () => {
...
用户只需要调用本 Skill。CLI 是内部后端；需要持久化、恢复或运行期评估时，当前 Agent 应通过 Coda Engine action/outcome 协议推进。
`;
}
...
export async function generateFactorySkillPackage(
  ...
  const CodaRoot = path.join(packageRoot, 'Coda');
  ...
  if (plan.engineMode !== 'none') {
    await fs.writeFile(path.join(CodaRoot, 'skill.yaml'), stringify(skillDefinition(plan)));
    await fs.writeFile(
      path.join(CodaRoot, 'guardrails.yaml'),
      stringify({
  ...
  return {
    ...
    enginePath: plan.engineMode === 'none' ? null : CodaRoot,
  };
}
```
...
## Task 5: Add Factory Guidance to `/Coda-any` Chinese Skill
...
In `test/ts/Coda-any-skill.test.ts`, add these expected Chinese phrases to the first test:
...
- [x] **Step 2: Run Coda-any tests and verify RED**
...
- [x] **Step 3: Update Chinese `/Coda-any` Skill**

Modify `assets/skills-zh/Coda-any/SKILL.md` so the opening model reads:

```markdown
`/Coda-any` 是 Coda Skill Factory。用户只需要调用本 Skill，描述想创建或优化的工作流；
本 Skill 会读取用户偏好、用 `find-skill` 查找本地真实 Skill 内容，组合出 Coda-native
...
或包含脚本副作用的生成物，必须生成 `Coda/skill.yaml`、`guardrails.yaml` 和 `evals.yaml`。
...
```markdown
- `.Coda/skills.txt` 的行顺序是推荐调用顺序；生成调用链时应尽量遵守，偏离时必须在评审摘要中说明偏离项和原因。
```
...
读取 `.Coda/skills.txt` 后调用 `find-skill`。不得只按名字推测能力；必须读取最终候选的真实
`SKILL.md`、直接 reference、rules、scripts 和 hooks。

### 6. 提出默认调用链

先按 `.Coda/skills.txt` 顺序提出默认调用链。若目标、依赖、风险、上下文恢复、安全确认或平台限制要求调整顺序，列出偏离项和原因。

### 9. 生成 Coda-native Skill 源码

生成 entry Skill、internal Skill、references 和 scripts。用户不需要手动运行 `Coda bundle`
或 `Coda skill`。

### 10. 生成 Engine Package

为多步骤或高风险生成物生成 `Coda/skill.yaml`、`guardrails.yaml` 和 `evals.yaml`。
```

- [x] **Step 4: Update Chinese references**

In `assets/skills-zh/Coda-any/reference/bundle-authoring.md`, add:

```markdown
## Skill Factory 后端

`Coda bundle` 是 `/Coda-any` 的内部确定性后端。用户不需要直接执行 Bundle CLI。
本 Skill 必须把 creator 输出适配为 Coda-native Skill Package，再交给 Bundle 后端编译、
Eval、发布和分发。

`.Coda/skills.txt` 的顺序必须作为推荐调用顺序传递到 Factory metadata。若生成调用链偏离该顺序，
评审摘要必须包含偏离原因。
```

- [x] **Step 5: Run Chinese Coda-any tests**
...
```bash
git add assets/skills-zh/Coda-any test/ts/Coda-any-skill.test.ts
git commit -m "feat: update chinese Coda-any factory flow"
```

## Task 6: Sync English `/Coda-any` Behavior
...
In `test/ts/Coda-any-skill.test.ts`, extend the parity table:
...
const parity: Array<{ zh: string; en: string }> = [
  ...
  { zh: 'Coda-native Skill', en: 'Coda-native Skill' },
  ...
  { zh: '生成 `Coda/skill.yaml`', en: 'generate `Coda/skill.yaml`' },
];
```
...
- [x] **Step 3: Update English `/Coda-any` Skill**

Modify `assets/skills/Coda-any/SKILL.md` with the English equivalent:

```markdown
`/Coda-any` is the Coda Skill Factory. The user only invokes this Skill and describes the
workflow they want to create or optimize. This Skill reads user preferences, uses `find-skill`
to locate real local Skill contents, composes a Coda-native Skill, and internally calls CLI
...
`Coda/skill.yaml`, `guardrails.yaml`, and `evals.yaml`. A lightweight single-step Skill may skip
...
```markdown
- The line order in `.Coda/skills.txt` is the recommended call order. The generated call chain should follow it when possible; if it deviates from the preferred order, the review summary must explain why.
```

- [x] **Step 4: Update English references**

In `assets/skills/Coda-any/reference/bundle-authoring.md`, add:

```markdown
## Skill Factory Backend

`Coda bundle` is the internal deterministic backend for `/Coda-any`. The user does not need to run
Bundle CLI directly. This Skill must adapt creator output into a Coda-native Skill Package before
passing it to the Bundle backend for compile, Eval, publish, and distribution.

The order in `.Coda/skills.txt` must be preserved as Factory metadata. If the generated call chain
deviates from that order, the review summary must include the reason.
```

- [x] **Step 5: Run Coda-any parity tests**
...
```bash
git add assets/skills/Coda-any test/ts/Coda-any-skill.test.ts
git commit -m "feat: sync english Coda-any factory flow"
```
...
In `README-zh.md`, add a short `/Coda-any` note near existing Skill/Bundle documentation:

```markdown
`/Coda-any` 是 Coda Skill Factory：用户只调用 Skill，描述想创建或优化的工作流；
Coda 会读取 `.Coda/skills.txt` 偏好、查找本地真实 Skill 内容、尽量遵守推荐调用顺序，
...
```markdown
`/Coda-any` is the Coda Skill Factory: users invoke the Skill and describe the workflow they want
to create or optimize. Coda reads `.Coda/skills.txt`, locates real local Skill contents, preserves
...
- **`/Coda-any` Skill Factory**: Repositions `/Coda-any` as the user-facing Skill creation flow instead of a CLI-oriented Bundle guide. It now treats `.Coda/skills.txt` as ordered preference input, resolves real local Skill contents through the shared finder, generates Coda-native Skill packages with Engine-aware semantics when needed, and requires review summaries to explain any call-order deviations.

### Tests

- **Skill Factory coverage**: Adds finder, preference-order, Factory metadata, generated Skill package, and bilingual `/Coda-any` contract coverage so ordered local Skill preferences and Engine-aware generated Skills remain stable.
```
...
```bash
npx vitest run test/ts/readme.test.ts test/ts/Coda-any-skill.test.ts
git diff --check
```
...
git commit -m "docs: document Coda-any skill factory"
```
...
```bash
npx vitest run test/ts/Coda-scripts.test.ts
npx vitest run
```
...
git diff -- assets/skills/Coda/scripts/Coda-runtime.mjs
```
...
Expected: no whitespace errors. Changed files should be limited to Skill Factory source, tests, `/Coda-any` assets, README, changelog, and version files if version bump was required.

## Self-Review

- Spec coverage: Tasks 1-2 cover `find-skill`, real local Skill contents, and `preferenceIndex`; Task 3 covers Factory metadata; Task 4 covers Coda-native Skill Package generation; Tasks 5-6 cover user-facing `/Coda-any` behavior; Task 7 covers docs and changelog; Task 8 covers verification.
- Preferred order: The plan preserves `.Coda/skills.txt` order through finder output, Bundle candidates, Factory state, generated package call chains, Skill review guidance, tests, and acceptance criteria.
- Engine behavior: The plan generates Engine package files for deterministic call chains and leaves full runner execution to existing `Coda skill run/resume`, matching the first-slice scope.
