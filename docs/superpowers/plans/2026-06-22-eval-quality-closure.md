...
**Goal:** Close the P0 eval evidence loop so Coda and generated Skills produce reproducible reports with profile, Skill source, artifact references, failure attribution, and `/Coda-any` authoring quality checks.
...
- `eval/scaffold/python/validation/authoring_rubric.py`: create `/Coda-any` generated Skill package checks for the `authoring-skill` profile.
...
def test_classifies_missing_skill_as_harness_when_no_skill_invoked():
    result = classify_failures(
        ["Required skill not invoked: Coda"],
    ...
    assert result == [
        {
            "bucket": "harness",
            "check": "Required skill not invoked: Coda",
...
def test_classifies_state_failures_as_workflow_after_skill_invocation():
    result = classify_failures(
        [".Coda.yaml missing"],
        {"skills_invoked": ["Coda"], "commands_run": []},
        "Coda-workflow",
    )
...
def classify_failure(check: str, events: dict, profile: str | None) -> FailureAttribution:
    ...
    if re.search(r"\.Coda\.yaml|guard|state|transition|archive", check, re.I):
...
def test_compare_report_uses_structured_failure_attribution(tmp_path: Path):
    ...
    _write_report(reports, "Coda_FULL_039", 300, 0.03)
    workflow = {
        "name": "Coda-full-workflow-Coda_FULL",
        "passed": False,
        "checks_passed": [],
        "checks_failed": ["Required skill not invoked: Coda"],
        "events_summary": {
            "total_tokens": 200,
            "total_cost_usd": 0.02,
            "failure_attribution": [
    ...
    (reports / "Coda_full_report.json").write_text(json.dumps(workflow))
...
def test_authoring_skill_profile_checks_generated_package(tmp_path: Path):
    ...
    (package / "Coda" / "skill.yaml").write_text("apiVersion: Coda/v1alpha1\nkind: Skill\n")
    (package / "Coda" / "guardrails.yaml").write_text("allowedSkills: [demo]\n")
    (package / "Coda" / "evals.yaml").write_text("runtime: []\n")
...
```python
"""Rubric for generated `/Coda-any` authoring Skill packages."""
...
def _check_engine_contract(package: Path | None) -> tuple[float, str, list[str]]:
    ...
    present = [name for name in expected if (Coda / name).exists()]
    missing = [f"{name} missing" for name in expected if name not in present]
    if not present and not Coda.exists():
...
def test_compare_report_lists_source_evidence(tmp_path: Path):
    ...
    workflow = {
        ...
        "checks_failed": [],
        "events_summary": {
            "profile": "Coda-workflow",
            "skill_sources": [{"name": "Coda", "hash": "sha256:abc"}],
            "eval_manifest": "demo/Coda/eval.yaml",
            "artifact_references": {"report": "reports/Coda_full_report.json"},
            "total_tokens": 200,
            "total_cost_usd": 0.02,
        },
    }
    ...
    assert "Coda-workflow" in report
    assert "sha256:abc" in report
    assert "reports/Coda_full_report.json" in report
```
...
- Produces: validation checks for `SKILL.md`, `reference/resolved-skills.json`, and optional `Coda/` Engine files.
...
description = "Smoke task for generated Coda-native Skill packages."
difficulty = "easy"
category = "generic"
tags = ["authoring", "skill", "Coda-any"]
...
def main():
    ...
    engine_roots = [path.parent for path in Path(".").rglob("Coda/skill.yaml")]
...
uv run pytest local/tests/tasks/test_tasks.py \
  ...
  --eval-manifest=/path/to/generated-skill/Coda/eval.yaml \
...
- Risk control: Existing `Coda-workflow` and `generic` behavior remains routed through current profile dispatch; new attribution and evidence fields are additive.
