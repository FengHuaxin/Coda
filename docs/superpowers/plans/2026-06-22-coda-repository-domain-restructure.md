# Coda Repository Domain Restructure Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 Coda 仓库从 `src/` 技术层结构重组为顶层 `app / domains / platform / assets / eval / scripts / test / docs` 的领域结构，同时保持 CLI 对外行为和安装到用户目录后的结果兼容。

**Architecture:** 先引入统一的仓库路径注册表与过渡期构建配置，再按依赖链迁移 `platform`、`domains/Coda-classic`、`domains/skill`、`domains/integrations`、`domains/engine`、`domains/bundle`、`domains/factory` 和 `app`。Classic runtime 的源码入口迁移到新域，但输出路径继续保持为 `assets/skills/Coda/scripts/Coda-runtime.mjs`。最后按新领域镜像重组 `test/`、`scripts/` 和 `docs/`，并删除旧 `src/` 目录。

**Tech Stack:** TypeScript ESM、Node.js 20+、Commander、esbuild、Vitest、YAML、现有 `assets/manifest.json` 资源发布链。

## Global Constraints

- 保持 `Coda` CLI 对外命令名、参数名和行为不变。
...
- Preserves: classic runtime output path `assets/skills/Coda/scripts/Coda-runtime.mjs`.
...
describe('repository layout registry', () => {
  it('resolves the manifest and classic runtime output paths', () => {
    ...
    expect(layout.classicRuntime.output).toBe('assets/skills/Coda/scripts/Coda-runtime.mjs');
    expect(resolveRepositoryPath(layout.classicRuntime.output)).toBe(
      path.resolve('assets', 'skills', 'Coda', 'scripts', 'Coda-runtime.mjs'),
    );
  });
...
```json
{
  ...
  "classicRuntime": {
    "entry": "domains/Coda-classic/classic-cli.ts",
    "output": "assets/skills/Coda/scripts/Coda-runtime.mjs"
  },
...
## Task 3: 迁移 `domains/Coda-classic` 并保持 runtime 产物路径稳定

**Files:**
- Create: `domains/Coda-classic/classic-archive.ts`
- Create: `domains/Coda-classic/classic-cli.ts`
- Create: `domains/Coda-classic/classic-evidence.ts`
- Create: `domains/Coda-classic/classic-guard.ts`
- Create: `domains/Coda-classic/classic-handoff.ts`
- Create: `domains/Coda-classic/classic-hook-guard.ts`
- Create: `domains/Coda-classic/classic-migrate.ts`
- Create: `domains/Coda-classic/classic-paths.ts`
- Create: `domains/Coda-classic/classic-resolver.ts`
- Create: `domains/Coda-classic/classic-runtime-run.ts`
- Create: `domains/Coda-classic/classic-state-command.ts`
- Create: `domains/Coda-classic/classic-state.ts`
- Create: `domains/Coda-classic/classic-store.ts`
- Create: `domains/Coda-classic/classic-validate-command.ts`
- Create: `domains/Coda-classic/index.ts`
...
- Modify: `test/ts/Coda-scripts-guard.test.ts`
- Modify: `test/ts/Coda-scripts-hook-guard.test.ts`
- Modify: `test/ts/Coda-scripts-recovery.test.ts`
- Modify: `test/ts/Coda-scripts.test.ts`
- Modify: `test/ts/helpers/Coda-test-utils.ts`

**Contracts:**
- Produces: `domains/Coda-classic/` as the new source root for classic runtime logic.
- Preserves: output file `assets/skills/Coda/scripts/Coda-runtime.mjs`.
...
```bash
git mv src/compat/classic-archive.ts domains/Coda-classic/classic-archive.ts
git mv src/compat/classic-cli.ts domains/Coda-classic/classic-cli.ts
git mv src/compat/classic-evidence.ts domains/Coda-classic/classic-evidence.ts
git mv src/compat/classic-guard.ts domains/Coda-classic/classic-guard.ts
git mv src/compat/classic-handoff.ts domains/Coda-classic/classic-handoff.ts
git mv src/compat/classic-hook-guard.ts domains/Coda-classic/classic-hook-guard.ts
git mv src/compat/classic-migrate.ts domains/Coda-classic/classic-migrate.ts
git mv src/compat/classic-paths.ts domains/Coda-classic/classic-paths.ts
git mv src/compat/classic-resolver.ts domains/Coda-classic/classic-resolver.ts
git mv src/compat/classic-runtime-run.ts domains/Coda-classic/classic-runtime-run.ts
git mv src/compat/classic-state-command.ts domains/Coda-classic/classic-state-command.ts
git mv src/compat/classic-state.ts domains/Coda-classic/classic-state.ts
git mv src/compat/classic-store.ts domains/Coda-classic/classic-store.ts
git mv src/compat/classic-validate-command.ts domains/Coda-classic/classic-validate-command.ts
git mv src/compat/index.ts domains/Coda-classic/index.ts
```
...
Inside `domains/Coda-classic/*`, rewrite every import that still points at `../engine/*` or `../skill/*` from the old `src/compat` depth so the moved files compile from their new location. At minimum, this includes the imports currently present in:
...
Use `../../domains/engine/*` and `../../domains/skill/*` only where the moved file now truly crosses domains; keep `./classic-*` imports local within `domains/Coda-classic/`.
...
```typescript
// test/ts/classic-evidence.test.ts
import { collectClassicEvidence, evidenceSatisfied } from '../../domains/Coda-classic/classic-evidence.js';
import type { ClassicStateProjection } from '../../domains/Coda-classic/classic-state.js';

// test/ts/classic-migrate.test.ts
import { ensureClassicRun } from '../../domains/Coda-classic/classic-migrate.js';
import { readClassicState, writeClassicState } from '../../domains/Coda-classic/classic-store.js';
import type { ClassicState } from '../../domains/Coda-classic/classic-state.js';

// test/ts/classic-resolver.test.ts
import { resolveClassicNextStep } from '../../domains/Coda-classic/classic-resolver.js';
import type { ClassicEvidence } from '../../domains/Coda-classic/classic-evidence.js';
import type { ClassicState } from '../../domains/Coda-classic/classic-state.js';
...
// test/ts/classic-runtime.test.ts
const { runClassicCli } = await import('../../domains/Coda-classic/classic-cli.js');
```
...
```bash
npx vitest run test/ts/classic-runtime.test.ts test/ts/classic-contract.test.ts test/ts/classic-guard.test.ts test/ts/classic-handoff.test.ts test/ts/classic-archive.test.ts test/ts/Coda-scripts.test.ts test/ts/Coda-scripts-recovery.test.ts
```
...
```bash
git add domains/Coda-classic scripts/build-classic-runtime.mjs build.js test/ts test/ts/helpers/Coda-test-utils.ts
git commit -m "refactor: move classic runtime into Coda-classic domain"
```
...
- Preserves: `bin/Coda.js` command surface.
...
```bash
git add app bin/Coda.js tsconfig.json vitest.config.ts package.json test/ts/helpers/ensure-cli-built.ts
...
git mv scripts/Coda-bundle-compatibility-benchmark.mjs scripts/benchmark/Coda-bundle-compatibility-benchmark.mjs
...
```json
{
  "scripts": {
    ...
    "benchmark:bundle": "pnpm build && node scripts/benchmark/Coda-bundle-compatibility-benchmark.mjs",
...
git mv test/ts/classic-archive.test.ts test/domains/Coda-classic/classic-archive.test.ts
git mv test/ts/classic-contract.test.ts test/domains/Coda-classic/classic-contract.test.ts
git mv test/ts/classic-evidence.test.ts test/domains/Coda-classic/classic-evidence.test.ts
git mv test/ts/classic-guard.test.ts test/domains/Coda-classic/classic-guard.test.ts
git mv test/ts/classic-handoff.test.ts test/domains/Coda-classic/classic-handoff.test.ts
git mv test/ts/classic-hook-guard.test.ts test/domains/Coda-classic/classic-hook-guard.test.ts
git mv test/ts/classic-migrate.test.ts test/domains/Coda-classic/classic-migrate.test.ts
git mv test/ts/classic-resolver.test.ts test/domains/Coda-classic/classic-resolver.test.ts
git mv test/ts/classic-runtime.test.ts test/domains/Coda-classic/classic-runtime.test.ts
git mv test/ts/classic-state.test.ts test/domains/Coda-classic/classic-state.test.ts
...
- **Repository layout**: Reorganized the codebase into top-level app, domain, and platform areas while preserving Coda CLI behavior and installed skill output paths.
```
...
```text
src/compat -> domains/Coda-classic
src/cli -> app/cli
...
pnpm build
npx vitest run test/domains/Coda-classic/Coda-scripts.test.ts test/ts/skills.test.ts test/app/update.test.ts
```
...
- `npx vitest run test/domains/Coda-classic/Coda-scripts.test.ts test/ts/skills.test.ts test/app/update.test.ts`
- Confirm `assets/manifest.json` still ships `Coda/scripts/Coda-runtime.mjs`
- Confirm `bin/Coda.js` resolves `dist/app/cli/index.js`
