# Coda Manual Skill Authoring Implementation Plan
...
- [x] 显式目录优先。
- [x] 项目 `.Coda/skills/<name>` 覆盖内置 Skill。
- [x] 无效项目覆盖失败关闭。
- [x] 找不到 Skill 时给出搜索位置。
...
- [x] 从 `<change>/.Coda/skill-snapshots/<hash>` 恢复 SkillPackage。
...
Plan 4 `/Coda-any` 的详细计划。不要在 Plan 3 中提前实现 `.Coda/skills.txt`、Skill
