# Coda Product UX Closure Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 `/Coda-any -> Coda eval -> Coda publish` 打磨成普通用户可顺着完成创建、确认、恢复、验证、发布和分发的闭环路径。

**Architecture:** 在现有 Bundle Factory、Skill preference、readiness、publish façade 之上增加用户可读摘要和确认契约，不新增第二套状态机。核心实现集中在 `domains/bundle/*` 的只读 summary、proposal、readiness 和 distribution preview，再由 `app/commands/*` 暴露给 `/Coda-any` 使用。中文 Skill/docs 先落地并测试通过，再同步英文和 README。
...
- 普通用户主路径固定为 `/Coda-any -> Coda eval -> Coda publish`。
- 不开放普通用户手写 `flow.yaml` 的路径。
- 不开放普通用户手写 `Coda/skill.yaml`、`guardrails.yaml`、`checks.yaml` 的路径。
- 不新增图形化编排器、复杂 DSL、条件表达式语言或单独的工作流引擎。
- 不删除 `Coda bundle` 后端命令；它仍用于高级调试、自动化和兼容。
- 不让 `/Coda-any` 自动跳过用户确认、Eval evidence、review readiness 或 executable disclosure。
- 不把 portable `hooks/*.yaml` 直接当作目标平台原生 hook 配置安装。
- 不修改 Superpowers 或 OpenSpec 原始 Skill。
- 项目偏好事实源保持 `.Coda/skill-preferences.yaml`。
- 创建和发布状态事实源保持 Bundle Factory / Bundle authoring state。
- 生成物必须继续包含 `SKILL.md`、`scripts/`、`rules/`、`hooks/`、`reference/`、`Coda/skill.yaml`、`Coda/guardrails.yaml`、`Coda/checks.yaml`、`Coda/eval.yaml`、`bundle.yaml`。
...
这份 spec 覆盖 5 个用户体验主题，但它们不是 5 个独立产品：都围绕同一条 `/Coda-any` 创建和发布路径，且复用同一组 Bundle state、factory metadata、readiness 和 distribution 后端。因此用一份 implementation plan 是合理的。
...
| `Coda publish` façade | `app/commands/publish.ts`, `app/cli/index.ts` |
...
- Create `domains/bundle/next-action.ts`
  ...
  - Produces both backend command and user-facing `Coda publish` command where possible.
  - Builds resume summaries from Bundle authoring state.

- Create `domains/bundle/factory-guide.ts`
  - Builds `/Coda-any` first-use and resume guide from project preferences, Skill inventory, and active Bundle state.
...
- Modify tests:
  ...
  - `test/ts/Coda-any-skill.test.ts`
  - `test/ts/readme.test.ts`

- Modify docs and Skill assets:
  - `assets/skills-zh/Coda-any/SKILL.md`
  - `assets/skills-zh/Coda-any/reference/bundle-authoring.md`
  - `assets/skills-zh/Coda-any/reference/eval-provider.md`
  - `assets/skills/Coda-any/SKILL.md`
  - `assets/skills/Coda-any/reference/bundle-authoring.md`
  - `assets/skills/Coda-any/reference/eval-provider.md`
...
function state(overrides: Partial<BundleAuthoringState> = {}): BundleAuthoringState {
  return {
    ...
    mode: 'create',
    status: 'draft',
    draftPath: '/project/.Coda/bundle-drafts/demo-skill',
...
describe('Bundle next action', () => {
  it('prefers user-facing publish commands after Factory generation', () => {
      ...
      state({
        factory: {
          ...
          generatedSkillPackage: {
            ...
            internalSkills: [],
            packageRoot: '/project/.Coda/bundle-drafts/demo-skill/skills/demo-skill',
            enginePath: null,
            evalManifestPath:
              '/project/.Coda/bundle-drafts/demo-skill/skills/demo-skill/Coda/eval.yaml',
          },
        },
      }),
    );
    ...
    expect(action).toMatchObject({
      ...
      category: 'eval',
      userCommand: 'Coda eval run --manifest <generated-skill>/Coda/eval.yaml --quick --html',
    });
    expect(action.backendCommand).toBe('Coda bundle eval-plan demo-skill --level quick');
  });
      ...
      state({
        factory: {
          ...
          generatedSkillPackage: {
            ...
            enginePath: null,
            evalManifestPath: '/draft/skills/demo-skill/Coda/eval.yaml',
          },
        },
      }),
...
export function determineBundleNextAction(state: BundleAuthoringState): BundleNextAction {
  ...
  if (!state.eval || state.eval.hash !== state.currentHash || !state.eval.passed) {
    ...
    return {
      ...
      backendCommand: `Coda bundle eval-plan ${state.name} --level quick`,
      userCommand: manifest
        ? `Coda eval run --manifest ${manifest} --quick --html`
        : 'Coda eval run --skill-path <generated-skill> --quick --html',
      requiresUserConfirmation: true,
    };
  }
...
- Produces:
  ...
  - CLI backend `Coda bundle factory-guide --json`
...
describe('Bundle Factory first-use guide', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'Coda-factory-guide-'));
    projectRoot = path.join(root, 'project');
    homeDir = path.join(root, 'home');
    builtinRoot = path.join(root, 'builtin');
    await fs.mkdir(path.join(projectRoot, '.Coda'), { recursive: true });
  });
  ...
  it('guides first-use projects without saved preferences', async () => {
    ...
    expect(guide).toMatchObject({
      ...
      preference: {
        state: 'missing',
        path: path.join(projectRoot, '.Coda', 'skill-preferences.yaml'),
      },
      firstRun: true,
      userMessage: {
        title: 'Start with /Coda-any',
      },
    });
    ...
    expect(guide.nextQuestions).toContain(
      'Should Coda save these preferences to .Coda/skill-preferences.yaml?',
    );
  });
...
export async function buildBundleFactoryGuide(options: {
  ...
  return {
    ...
    nextQuestions: [
      ...
      'Which discovered Skills should Coda prefer?',
      'Should Coda save these preferences to .Coda/skill-preferences.yaml?',
      'May Coda generate scripts, rules, and hooks as the control plane?',
    ],
    userMessage: hasResumable
      ? {
          title: 'Resume /Coda-any',
...
bundle
  ...
  .description('Summarize /Coda-any first-use, preferences, and resumable Factory flows')
...
```ts
it('prints the Factory first-use guide for /Coda-any', async () => {
  ...
  expect(result).toMatchObject({
    ...
    userMessage: { title: 'Start with /Coda-any' },
  });
...
git commit -m "feat: add Coda-any factory guide"
```
...
- Produces:
  ...
  - CLI flag `Coda bundle factory-init <name> --confirmed-proposal`
...
it('prints a user-decision Factory proposal before initialization', async () => {
  ...
  expect(proposal).toMatchObject({
    ...
    canGenerate: true,
    userSummary: {
      title: 'Create guided-planning as a Coda-native Skill',
...
const userSummary: BundleFactoryProposalSummary = {
  title: `Create ${options.name} as a Coda-native Skill`,
  goal: plan.goal,
  ...
  requiredConfirmations: [
    {
      id: 'generate-scripts',
      label: 'Allow Coda to generate scripts as part of the control plane',
      required: true,
      reason: 'Scripts let the generated Skill perform deterministic checks and recovery steps.',
    },
    {
      id: 'generate-rules',
      label: 'Allow Coda to generate orchestration rules',
      required: true,
      reason: 'Rules keep the Agent aligned with the generated workflow.',
    },
    {
      id: 'generate-hooks',
      label: 'Allow Coda to generate portable hook descriptors',
      required: true,
...
const actions: BundleFactoryProposalAction[] = [
  {
    ...
    command: `Coda bundle factory-init ${options.name} --file ${path.resolve(options.filePath)} --confirmed-proposal`,
    writesState: true,
  },
  {
    id: 'revise-proposal',
    label: 'Revise the plan before generating',
    command: 'Ask /Coda-any to revise the goal, preferences, or candidate choices',
    writesState: false,
  },
  {
    id: 'cancel',
...
- Produces:
  - JSON `resumeSummary` on `Coda bundle status`, `Coda bundle list`, `Coda publish status`, `Coda publish list`
  ...
  bundles: [
    expect.objectContaining({
      resumeSummary: expect.objectContaining({
        ...
        currentStep: 'needs-eval',
        recommendedNextStep: expect.objectContaining({
          userCommand: expect.stringContaining('Coda eval run'),
        }),
      }),
    }),
    expect.objectContaining({
...
expect(blocked.userSummary.items).toEqual(
  expect.arrayContaining([
    ...
    expect.objectContaining({
      ...
      severity: 'blocker',
      nextAction: expect.objectContaining({
        command: expect.stringContaining('Coda eval run'),
      }),
    }),
  ]),
);
  ...
  switch (code) {
    ...
    case 'capability':
      return {
        ...
        command: `Coda publish distribute ${bundleName} --platform <platform> --scope project --preview`,
      };
    case 'executable':
      return {
    ...
    default:
      return {
        impact: 'Coda needs more information before it can publish safely.',
...
- Produces:
  ...
  - CLI `Coda publish distribute <name> --preview`
  - CLI `Coda bundle distribute <name> --preview`
...
### Task 7: Update `/Coda-any` Chinese Skill And Docs

**Files:**
- Modify: `assets/skills-zh/Coda-any/SKILL.md`
- Modify: `assets/skills-zh/Coda-any/reference/bundle-authoring.md`
- Modify: `assets/skills-zh/Coda-any/reference/eval-provider.md`
- Modify: `docs/operations/SKILL-CREATION-ZH.md`
- Modify: `docs/operations/EVAL-USAGE-ZH.md`
- Test: `test/ts/Coda-any-skill.test.ts`

**Interfaces:**
- Consumes:
  - `Coda bundle factory-guide --json`
  - `Coda bundle factory-propose --json`
  - `Coda bundle factory-init --confirmed-proposal`
  - `Coda publish status/review/approve/run/distribute --preview`
- Produces:
  - Chinese `/Coda-any` workflow that prioritizes resume, first-use guide, proposal confirmation, Eval, readiness, and publish preview.

- [x] **Step 1: Update failing Chinese Skill assertions**

Modify `test/ts/Coda-any-skill.test.ts` expected phrases. Add these Chinese phrases to the required list:

```ts
'Coda bundle factory-guide',
'首次使用向导',
'恢复摘要',
'Current step',
...
```bash
npx vitest run test/ts/Coda-any-skill.test.ts
```

Expected: fail because Chinese Skill docs do not yet contain the new guide/preview phrases in the required order.

- [x] **Step 3: Update Chinese `/Coda-any` workflow**

Modify `assets/skills-zh/Coda-any/SKILL.md`:

- Add `Coda bundle factory-guide --project . --json` as the first deterministic backend call.
- Say `/Coda-any` must resume existing flow before creating a new one unless user explicitly says to start over.
- Say first-use guide must explain `.Coda/skill-preferences.yaml` and ask before saving.
...
1. `confirm-generate` - 确认生成，随后调用 `Coda bundle factory-init <name> --file <plan> --confirmed-proposal`
...
```md
发布前必须展示 `Coda publish review <name> --platform <id>` 的 `Publish readiness:` 与 `User next steps:`。
分发前必须先运行 `Coda publish distribute <name> --platform <id> --scope project --preview`。
...
- Keep `Coda eval` as the only ordinary Eval path.
...
```md
/Coda-any 创建 -> Coda eval 验证 -> Coda publish review/approve/run -> Coda publish distribute --preview -> Coda publish distribute
```
...
- Show `Coda publish review` after eval.
...
```bash
git add assets/skills-zh/Coda-any/SKILL.md assets/skills-zh/Coda-any/reference/bundle-authoring.md assets/skills-zh/Coda-any/reference/eval-provider.md docs/operations/SKILL-CREATION-ZH.md docs/operations/EVAL-USAGE-ZH.md test/ts/Coda-any-skill.test.ts
git commit -m "docs: update chinese Coda-any user workflow"
```
...
**Files:**
- Modify: `assets/skills/Coda-any/SKILL.md`
- Modify: `assets/skills/Coda-any/reference/bundle-authoring.md`
- Modify: `assets/skills/Coda-any/reference/eval-provider.md`
- Modify: `docs/operations/SKILL-CREATION.md`
- Modify: `docs/operations/EVAL-USAGE.md`
- Modify: `README-zh.md`
- Modify: `README.md`
- Modify: `CHANGELOG.md`
- Modify: `docs/superpowers/specs/2026-06-24-Coda-product-ux-closure-priority-design.md`
- Modify: `docs/superpowers/plans/2026-06-24-Coda-product-ux-closure-priority.md`
- Test: `test/ts/Coda-any-skill.test.ts`
...
In `test/ts/Coda-any-skill.test.ts`, add parity pairs:
...
7. Generate the Coda-native Skill
...
- Replace any direct ordinary-user `Coda bundle distribute` suggestion with `Coda publish distribute --preview` followed by confirmed `Coda publish distribute`.
- Keep the full `Coda bundle` command list inside advanced backend details.
...
- **Coda Any user guidance**: Added a first-use Factory guide, confirmable composition proposals, resume summaries, and distribution preview so `/Coda-any` can guide users through creation, Eval, publish, and platform distribution without requiring Bundle CLI knowledge.
...
- **Coda Any lifecycle**: Added coverage for Factory guide output, proposal confirmation metadata, resume summaries, readiness user summaries, and publish distribution preview.
```
...
After Tasks 1-8 pass, update `docs/superpowers/specs/2026-06-24-Coda-product-ux-closure-priority-design.md`:
...
```bash
npx vitest run test/ts/Coda-any-skill.test.ts test/ts/readme.test.ts
```
...
```bash
git add assets/skills/Coda-any/SKILL.md assets/skills/Coda-any/reference/bundle-authoring.md assets/skills/Coda-any/reference/eval-provider.md docs/operations/SKILL-CREATION.md docs/operations/EVAL-USAGE.md README-zh.md README.md CHANGELOG.md docs/superpowers/specs/2026-06-24-Coda-product-ux-closure-priority-design.md docs/superpowers/plans/2026-06-24-Coda-product-ux-closure-priority.md test/ts/Coda-any-skill.test.ts test/ts/readme.test.ts
git commit -m "docs: document Coda-any product ux closure"
```
