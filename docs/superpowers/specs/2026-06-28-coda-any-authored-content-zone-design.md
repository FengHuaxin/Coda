# Coda Any 内容创作区设计（Authored Content Zone）

日期：2026-06-28

状态：设计增补，待实现。本文修正 `2026-06-28-Coda-any-authoring-protocol-design.md` 的一个偏差：生成模板不应试图"写满"skill，领域决策内容必须由 agent 动态创作。

## 背景

authoring-protocol 设计落地后，实测产物（`team-Coda-execute/SKILL.md` 约 45 行）与 Coda 原始 skill（`Coda-build/SKILL.md` 326 行、`Coda/SKILL.md` 237 行）差距很大。根因不是"模板不够丰富"，而是**方向错了**：再大的静态模板也写不出领域决策（不知道 elementui 的 API、项目组件约定、具体 workflow 取舍）。Coda 的质量来自**人类作者写的领域决策**，脚本是控制面兜底。Coda-any 要达到同等质量，应**用 LLM agent 替代人类作者**，而不是把模板做大。
...
- 创作 brief 以 Coda 真实 skill 为质量标尺。
...
- `workflowProtocol.kind === 'Coda-five-phase-overlay'` → 所有节点 `delegates`。
...
- 创作 brief（`subagents/skill-core-author.md`、`workflow-entry-author.md`，en+zh）：只创作 Authored 区，以 Coda 真实 skill 为质量标尺 + 章节大纲。
...
- 富草稿 e2e：给 kernel 主体节点喂 80-120 行决策内容，产物在该节点具备 Coda 级决策核心。
