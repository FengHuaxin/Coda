# Coda Any Skill Bundle Creator 设计
...
`/Coda-any` 读取候选实现、交互澄清、调用原生 `skill-creator` 或经用户同意使用 Coda
...
- 平台能力声明和必要的平台覆盖
- 可选的 Coda Engine 编排元数据

分发后的 Skills 由目标平台原生运行，不要求用户通过 Coda Engine 启动。Coda Engine
在 Plan 4 中主要提供编译 IR、静态验证、Eval，以及需要时的可选高级编排。

本阶段交付：

- `/Coda-any` 中文和英文 Skill
- 平台无关 Skill Bundle manifest 和目录模型
- `.Coda/skills.txt` 候选偏好池
- 新建和优化已有 Bundle
...
- `/Coda-any` 生成 Skill Bundle，不生成强绑定 Engine 的单一 Skill Package。
- Bundle 是平台无关源码，分发时编译为目标平台目录和配置。
- Bundle 可声明多个用户入口 Skill 和内部辅助 Skill。
- 目标平台原生运行编译后的 Skills，不依赖 Coda Engine。
...
- 当前 Coda 必须能由 Bundle 模型表达并重新生成，作为首个兼容基准。
- 创建由宿主 Agent 驱动，CLI 不直接配置或调用模型 API。
- 优先使用当前平台原生 `skill-creator`。
- 原生 creator 不可用时，必须询问用户是否启用 Coda fallback。
...
- 不要求平台通过 Coda Engine 运行生成 Skill。
- 不把 `Coda/skill.yaml` 作为所有生成产物的必需文件。
- 不让 CLI 猜测用户意图或自主调用 LLM。
- 不把 Bundle 创建生命周期写入 `.Coda.yaml` 或建模为 change Run。
...
用户目标 + 候选 Skill
     ...
     /Coda-any
          ...
          |
          v
  Coda Bundle Authoring Adapter
          |
          v
 .Coda/bundle-drafts/<name>/
          |
          v
...
- `/Coda-any` 默认只生成用户选择的语言，不因 Bundle 模型支持多语言而强制增加 token
  成本。

当前 Coda 的中英文资源集必须能够映射为同一 Bundle 的 `zh` 和 `en` locale。
...
`engine/` 使用现有 Coda Skill Engine 模型，可由 Bundle compiler 生成 Plan 3 可加载的
Skill Package 或等价快照。没有 `engine/` 时，Bundle 仍可完整编译、验证、Eval 和分发。

Engine 是可选运行增强层，不是平台原生 Skill 的启动前提。

## 6. Coda Bundle Authoring Adapter

原生 `skill-creator` 和 Coda fallback 都不是 Bundle 发布器。它们生成普通 Skill 内容、
...
新建 Bundle 写入 `.Coda/bundle-drafts/<name>/`。

优化已有 Bundle 时：

1. 从项目 `.Coda/bundles/<name>/` 或显式 Bundle 来源复制到 draft。
...
`.Coda/bundle-authoring/<name>.json` 至少记录：
...
- creator provider：`native | Coda-fallback`
- Bundle 语言
...
`.Coda/skills.txt` 一行一个偏好 Skill 名：
...
- 表示 `/Coda-any` 优先探索的候选，不是固定顺序或严格白名单。
- 空行和以 `#` 开头的注释忽略。
- 重复项去重并保留首次出现顺序。
...
- authoring 启动后，`.Coda/skills.txt` 的变化不会自动改变当前 draft。
...
然后读取 `.Coda/skills.txt`；不存在时扫描当前平台可用 Skill 并让用户选择候选。
...
2. 可用时优先调用原生 creator。
3. 不可用时说明能力差异并询问是否使用 Coda fallback。
...
当前 Coda 作为首个 Bundle compiler 基准：
...
基准要求 Bundle 模型可以表达当前 Coda 的分发意图，并由 compiler 重新生成等价的受管
...
Coda fallback Provider 只在用户明确同意后使用。
...
`.Coda/bundle-evals/<name>/<bundle-hash>/`。只有结果属于当前 Bundle hash，所有必需
...
用户必须明确批准整个 Bundle。`Coda bundle review <name> --approve` 记录批准人、时间和
Bundle hash。拒绝或要求修改时保持未发布状态；任何文件修改后旧批准自动失效。

### 12.2 Publish

`Coda bundle publish <name>`：

1. 重新计算 Bundle hash。
2. 校验当前 hash 已通过 Eval。
3. 校验人工批准绑定同一 hash。
4. 重新运行静态、安全和参考平台 dry-run 编译。
5. 原子复制到 `.Coda/bundles/<name>/`。
...
- `Coda bundle draft create <name>`
- `Coda bundle draft optimize <bundle>`
- `Coda bundle status <name>`
- `Coda bundle compile <name> --platform <id>`
- `Coda bundle eval-record <name> --result <file>`
- `Coda bundle review <name> --approve|--reject`
- `Coda bundle publish <name> [--overwrite]`
- `Coda bundle distribute <name>`

`/Coda-any` 使用这些命令持久化确定性状态。高级用户也可以手工调用它们恢复或审计流程。
所有命令支持 `--json`。

`draft create/optimize` 只初始化目录和 authoring state，不生成自然语言内容。Bundle 内容由
宿主 Agent、creator 和 Bundle Authoring Adapter 写入 draft。

现有 `Coda skill` 命令继续管理单个 Coda Engine Skill Package，不与 Bundle CLI 混用。

## 14. 跨平台分发

`Coda bundle distribute <name>` 复用 `Coda init` 的平台检测、交互选择、scope、路径映射
和安全复制机制。
...
16. 当前 Coda 可由 Bundle 模型表达并重新生成等价受管产物。
17. 中文和英文 `/Coda-any` 行为一致。
