...
**日期：** 2026-06-23
**状态：** 已实现并按当前代码打勾
**范围：** 梳理当前 Coda 在 Skill、`/Coda-any`、Eval 与打包后 CLI 用户路径上的剩余优化项

## 1. 背景

Coda 0.4.0-beta.1 已经完成了几条关键用户能力链：

- Classic runtime 已迁移到纯 Node runtime。
- `status`、`doctor`、guard 与 runtime eval 正在共用同一条诊断证据路径。
- `/Coda-any` 已经从 Bundle CLI 使用说明转为 Coda Skill Factory 用户入口。
...
- `/Coda-any` 的内部门禁已经存在，但用户和 Agent 仍需要理解较多 Bundle 子命令才能判断下一步。
...
- `Coda status` 能给出活跃 change 和下一步工作流状态；没有活跃 change 时能清晰返回。
- `Coda bundle review-summary` 文本模式已经展示 readiness、blockers、warnings 和 evidence。
- `/Coda-any` 中英文 Skill 已明确要求发布前读取 review summary readiness，并在 Eval 缺失或 readiness 非 publishable 时停止发布。
- Eval README 已提供任意本地 Skill 和 generated Skill manifest 的运行方式。
- Eval profile 已包括 `generic`、`Coda-workflow`、`authoring-skill`，可以区分普通 Skill、Coda workflow 和 `/Coda-any` 产物。

### 2.2 仍然不方便的部分

- 用户首次接触 `/Coda-any` 时，README 会展示完整 Bundle CLI 生命周期，容易误以为自己需要手动串起所有命令。
...
- 打包后 CLI 没有足够的烟测覆盖，导致 `dist` 资源路径问题能通过源码层测试但在 `bin/Coda.js` 用户路径上暴露。
...
在编译后运行 `bin/Coda.js` 时，这会解析到 `dist/assets`，而不是仓库根目录的 `assets`。当前本地验证中：

- `assets/manifest.json` 存在。
- `dist/assets/manifest.json` 不存在。
- `node bin/Coda.js doctor . --scope project` 会因找不到 `dist/assets/manifest.json` 失败。

这不是文案问题，而是会直接影响用户命令的发布级风险。

## 3. 目标

- 修复构建后 CLI 对资源目录的解析，使 npm 包和本地 `bin/Coda.js` 用户路径稳定可用。
- 让 `/Coda-any` 的下一步恢复、Eval、review 和 publish 判断更直接，减少 Agent 推理成本。
...
- 不让 `/Coda-any` 自动跳过人工 approval、Eval evidence 或 executable disclosure。
...
| 优先级 | 状态 | 问题 | 用户影响 | 目标状态 |
|--------|------|------|----------|----------|
| P0 | [x] | 构建后 CLI 找不到 `assets` | `doctor`、`init`、`update` 等命令可能直接失败 | `bin/Coda.js` 在构建后和 npm 包内都能稳定定位 manifest 与 Skill assets |
| P0 | [x] | 缺少打包级 CLI 烟测 | 源码测试无法覆盖真实用户入口 | CI 或本地测试能验证构建后的 CLI 关键命令 |
| P1 | [x] | `/Coda-any` 下一步不够显性 | 用户或 Agent 需要理解多个 Bundle 子命令 | Bundle 状态提供稳定 next action 和恢复说明 |
| P1 | [x] | Eval 入口过于维护者化 | 用户需要记 pytest 参数组合 | `Coda eval` 封装常用 Skill eval、manifest eval、quick smoke 和 HTML 报告 |
| P2 | [x] | 文档与 CLI help 偏命令全集 | 新用户不知道从哪条路径开始 | README/help 提供任务式 quickstart |
| P2 | [x] | `Coda skill run/resume/eval` 文本输出偏底层 | deterministic run 不容易被普通用户理解 | 文本输出包含下一步、pending action、artifact/eval 线索 |

## 6. P0：构建后 CLI 资源路径闭环

### 6.1 问题

当前 `getAssetsDir()` 以编译后模块位置推导 `assets`。源码测试从 TypeScript 源目录导入模块时可以工作，但用户通过 `bin/Coda.js` 运行编译产物时，路径会变成 `dist/assets`。
...
- [x] `pnpm build` 后，`node bin/Coda.js doctor . --scope project` 不因 `manifest.json` 缺失失败。
- [x] `node bin/Coda.js init --help`、`doctor --help`、`update --help` 在构建后可运行。
- [x] 至少一个测试通过真实 `bin/Coda.js` 或编译产物入口执行，而不是只导入 TypeScript 源码。
...
当前测试覆盖了大量源码模块、Classic runtime 和 CLI command 函数，但缺少构建后入口烟测。结果是资源路径、`bin/Coda.js`、package files、post-build 布局这类问题容易漏掉。

### 7.2 设计方向

新增一个轻量打包烟测，不做真实外部安装，不访问网络，不运行高成本流程。它只验证构建后用户入口：

- `node bin/Coda.js --help`
- `node bin/Coda.js status <tmp-project>`
- `node bin/Coda.js doctor <tmp-project> --scope project`
...
`/Coda-any` 已经通过 Bundle 后端维护确定性状态，也有 readiness blockers。但用户仍可能在这些信息之间来回切换：
...
Suggested command: Coda bundle factory-resolve <name> ...
...
`/Coda-any` Skill 应读取这个 next action，而不是重新猜测 Bundle 生命周期。JSON 输出应提供机器可读字段，文本输出应提供人类可读说明。

### 8.3 验收标准

- [x] `Coda bundle status <name> --json` 或新命令包含 next action。
- [x] 非 JSON 输出能直接说明下一步、原因和是否需要用户决策。
- [x] `/Coda-any` 中英文文案都要求读取 next action。
...
这对维护者可接受，但不适合作为 Coda 用户主路径。尤其 `/Coda-any` 生成 `Coda/eval.yaml` 后，用户期望是“跑这个 Skill 的 quick eval”，而不是理解 pytest suite、task、profile、report-config。

### 9.2 设计方向

新增 `Coda eval` 顶层命令，封装现有 harness：

```bash
Coda eval run --manifest ./my-skill/Coda/eval.yaml --quick
Coda eval run --skill-path ./my-skill --skill-name my-skill --profile generic --quick
Coda eval collect --manifest ./my-skill/Coda/eval.yaml
Coda eval compare --baseline Coda_FULL_039 --current Coda_FULL --html
```
...
- `--collect-only` 暴露为 `Coda eval collect`，用于低成本发现验证。
- `--html` 自动生成临时 report config 并输出报告路径。

### 9.3 验收标准

- [x] 用户可从仓库根目录运行 `Coda eval run --manifest <path>`。
- [x] 命令输出包含 experiment id、profile、task、report path 和失败归因摘要。
- [x] `Coda eval collect --manifest <path>` 不运行 Docker/Claude，只验证发现路径。
- [x] README 用 `Coda eval` 作为主路径，pytest 命令保留为维护者路径。
...
- **开始 Coda 工作流**：`Coda init` → `/Coda`
- **创建/优化 Skill**：`/Coda-any`
- **评估 Skill**：`Coda eval run --manifest ...`
- **诊断卡住状态**：`Coda status` → `Coda doctor`
- **发布前检查 Bundle**：`Coda bundle review-summary`

CLI help 可逐步增加 examples 或 command descriptions，但不需要一次性重写 Commander help。优先在 README、`Coda bundle status` 文本输出、`Coda eval` 输出里降低入口成本。

### 10.3 验收标准

- [x] README-zh 先更新，用户确认后同步 README。
- [x] 中英文结构保持一致。
- [x] README 不把完整 Bundle CLI 生命周期当作 `/Coda-any` 普通用户主流程。
...
`Coda skill run/resume/eval` 是 Engine 能力入口，但普通用户很难从底层字段理解：
...
Next: ask the Agent to complete the pending action, then run Coda skill resume ...
Artifacts: report=...
...
- 作为新用户，我运行 `Coda doctor` 时不应该因为发布包内部路径缺失而看到 Node stack trace。
- 作为 Skill 作者，我调用 `/Coda-any` 后，希望它告诉我下一步要解决候选、跑 eval、等待 review 还是 publish，而不是让我理解所有 Bundle 子命令。
- 作为维护者，我希望任何“已通过 eval”的结论都能追溯到具体 profile、Skill hash、task、report 和 artifact。
- 作为生成 Skill 的用户，我希望用一个 `Coda eval run --manifest ...` 命令完成 quick eval，并拿到可打开的报告路径。
...
3. [x] 让 `/Coda-any` 中英文 Skill 读取 next action，而不是重新推断生命周期。
4. [x] 增加 `Coda eval collect/run` 的薄封装。
5. [x] 更新 README-zh，再同步 README。
6. [x] 增强 `Coda skill run/resume/eval` 文本输出。

## 14. 风险与约束

- 资源路径修复不能依赖当前工作目录；用户可能从任意目录运行全局 `Coda`。
- Eval CLI 封装不能隐藏高成本行为；quick/full、Docker、Claude、LangSmith 都必须显式。
- `/Coda-any` next action 不能成为第二套状态机；它必须从现有 Bundle authoring state 和 review readiness 派生。
...
- [x] 安装或构建后运行 `Coda doctor` 不遇到资源路径错误。
- [x] 调用 `/Coda-any` 后能从输出中看到明确下一步。
- [x] 对生成 Skill 运行 quick eval 时不需要直接记 pytest 命令。
- [x] 发布 Bundle 前能看到机器可读和人类可读的 readiness 证据。
- [x] README 能用任务式路径解释 Skill、`/Coda-any` 和 eval 的关系。
