...
**日期：** 2026-06-22
**状态：** 已部分完成（P0/P1 主体已由后续 plan 实现；剩余用户体验闭环由 `2026-06-23-Coda-user-facing-ux-closure-design.md` 继续追踪）
**范围：** 将当前 Coda 0.4.0-beta 后续改进收敛为可执行的优先级 spec

## 1. 背景

Coda 0.4.0-beta 已经完成了几件关键架构跃迁：
...
- `/Coda-any` 已从 Bundle CLI 使用说明重定位为 Skill Factory，并开始使用确定性 CLI 后端。
...
- eval 能跑，但还需要更强的证据质量、可复现性和 `/Coda-any` 产物门禁。
- `/Coda-any` 能生成，但生成质量、候选恢复、dogfood 和发布前审查还要更确定。
- Coda Classic 已经 TypeScript runtime-native，但还不是完全 Engine-native。
...
- 确保 eval、`/Coda-any`、新 workflow 机制和 Coda Skill 本身朝同一个 Engine-native 方向收敛。
...
- 不替代现有详细设计：
  ...
  - `2026-06-21-Coda-any-skill-factory-design.md`
  - `2026-06-14-Coda-classic-migration-design.md`
  - `2026-06-01-Coda-orchestration-engine-design.md`
- 不一次性切换用户可见 `/Coda` 行为。
...
Engine-native 是 Coda 的收敛方向，但不应被理解为“所有 Skill 必须立刻接入完整 Engine”。迁移应按风险和价值分层：先让需要恢复、证据、guardrails 和 eval 的流程 Engine-native，再保留轻量 Skill 的快速路径。
...
- 准备作为团队共享、发布、复用或 `/Coda-any` ready 产物的 Skill。
...
非 Engine 路径会失去结构化 Run 恢复、pending action 约束、runtime eval 证据、可复用审批记录和统一轨迹。`/Coda-any` 可以推荐这类轻量输出，但 review summary 必须明确披露。
...
- `.Coda.yaml`、`run-state.json`、OpenSpec artifacts 和 Factory metadata 的 owner 必须清楚，不能新增漂移事实源。
...
| 优先级 | 方向 | 目标状态 |
|--------|------|----------|
| P0 | Eval 质量闭环 | 任何 Coda 或生成 Skill 的结论都能追溯到可复现实验、明确 profile、Skill hash、artifact evidence 和成本数据。 |
| P0 | `/Coda-any` dogfood 与生成质量 | `/Coda-any` 能稳定使用本地真实 Skill 生成可审查、可 eval、可分发的 Coda-native Skill。 |
| P1 | Classic Engine-native 迁移 | `/Coda` 系列入口逐步由 Engine Run/Resolver 驱动，launcher 和 Skill 文案只负责入口和人机协议。 |
...
当前 eval 已经能比较 Coda workflow、0.3.9 baseline 和 generic Skill，但仍容易出现两类误判：
...
此外，`/Coda-any` 产物要进入实际使用，就必须有一条低成本但可信的 eval path。否则 Skill Factory 会产生“看起来能用，但没人敢发布”的半成品。
...
3. **Skill-agnostic profile 完整落地**
   - Coda 评分只属于 `Coda-workflow` profile。
   - 任意本地 Skill 默认走 `generic` profile。
   - `/Coda-any` 产物走 `authoring-skill` profile，第一版可以复用 generic，但必须增加生成物结构和 review evidence 检查。
...
- `/Coda-any` 生成物可通过 eval manifest 直接进入 quick eval。
...
## 7. P0：`/Coda-any` Dogfood 与生成质量

### 7.1 问题

`/Coda-any` 的产品方向已经清晰：用户只调用 Skill，CLI 作为内部确定性后端。但当前仍有几个薄弱点：

- `.Coda/skills.txt` 偏好解析、歧义恢复和缺失处理需要更稳定的 CLI-backed recovery。
...
`/Coda-any` 后续工作以 `docs/superpowers/specs/2026-06-21-Coda-any-skill-factory-design.md` 为主设计，本 spec 只定义优先级和收敛边界。

必须优先完成四个闭环：

1. **Dogfood E2E**
   - 使用真实 `.Coda/skills.txt` 输入。
...
3. **生成质量**
   ...
   - 偏离 `.Coda/skills.txt` 顺序时，review summary 必须说明原因。
...
`2026-06-21-Coda-any-skill-factory-design.md` 仍是 `/Coda-any` 的主设计来源，但它是进行中的设计，不应被当作“当前代码已完全实现”的事实清单。后续实现要按当前代码位置和已落地边界推进：
...
- `app/cli/index.ts` 已暴露 `factory-init`、`factory-generate`、`factory-resolve`、`eval-plan`、`eval-record`、`review-summary`、`publish` 和 `distribute` 等后端入口；`/Coda-any` Skill 文案应把它们当作内部工具，而不是用户主流程。
- 当前 Factory 产物会在 entry Skill 内嵌入 `SKILL.md`、`reference/resolved-skills.json`、source summaries，以及可选 `Coda/skill.yaml`、`guardrails.yaml`、`evals.yaml`。这与“生成 Coda-native Skill Package”的方向吻合。
...
- CLI E2E 测试能从 `.Coda/skills.txt` 到 review summary 完整跑通。
- `/Coda-any` Skill 文案中不要求用户手动运行 Bundle CLI 主流程。
- 生成物可以通过 `Coda skill validate` 和 Bundle compile。
...
- `Coda-classic` 内部 Skill Package 尚未成为实际驱动用户 workflow 的唯一语义源。
...
- **迁移风险**：Classic 已有 change 可能包含旧 `.Coda.yaml` 状态、旧 artifact 布局或半完成归档，读写逻辑必须懒迁移且可重试。
...
2. **Engine package as contract**
   - 内部 `Coda-classic` Skill 声明 full/hotfix/tweak 稳定步骤。
...
4. **User-visible compatibility**
   - `/Coda`、`/Coda-open`、`/Coda-build` 等入口保留。
   - 用户不需要理解 `Coda-classic` 内部 Skill。
   - Engine-native 迁移不能让现有 change 无法恢复或归档。

### 8.4 验收标准

- `Coda status`、guard、doctor 和 next-step hint 对同一 malformed state 给出一致结论。
...
- 旧 `.Coda.yaml` change 首次读取可迁移，重复读取幂等。
...
Coda 的新 workflow 机制现在有多个事实源：

- `.Coda.yaml` 用户字段。
- `.Coda/run-state.json` Engine 字段。
...
| 事实 | Owner |
|------|-------|
| 用户可配置 workflow 字段 | `.Coda.yaml` |
| machine-owned run 字段 | `.Coda/run-state.json` |
...
- `.Coda.yaml` schema、state command、validate command 和测试 fixture 对字段一致。
...
- `/Coda-any`、eval、Engine-native Classic 的详细行为都有 docs 链接。
...
1. **Eval P0 收尾**
   - 完成 source metadata、failure attribution、manifest-driven `/Coda-any` quick eval。
   - 让后续所有改进都能被同一套 eval 证明。

2. **`/Coda-any` P0 Dogfood**
...
- **CLI/E2E**
  - `Coda bundle factory-*`
  - `Coda skill run/resume/eval`
  - `Coda status/doctor/guard`
  - eval pytest runner。

- **Compatibility**
  - Frozen 0.3.9 Classic fixtures。
  - Existing Coda workflow eval corpus。
  - Generated `/Coda-any` dogfood fixture。
...
- `/Coda-any` 不得执行候选 Skill 的脚本，只能读取和总结。
...
- `/Coda-any` Factory 当前以 entry Skill 内嵌 Engine 文件为主，不应假设 Bundle-level `engine.enabled` 已是主实现路径。

## 14. 成功状态

完成本 spec 对应优先改进后，Coda 应达到以下状态：

- 修改 Coda 或生成 Skill 后，有可信 eval 说明质量、成本和风险。
- `/Coda-any` 可以实际 dogfood：读取本地真实 Skill，生成 Coda-native Skill，并通过 eval/review 后发布。
- `/Coda` Classic workflow 对用户保持熟悉，但内部由 Engine Run/Resolver/evidence 驱动。
...
- `docs/superpowers/plans/2026-06-22-Coda-any-dogfood-quality.md`
