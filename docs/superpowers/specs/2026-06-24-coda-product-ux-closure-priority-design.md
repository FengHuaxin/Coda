# Coda 普通用户可用性闭环二期 Spec

日期：2026-06-24

状态：草案，待用户确认后进入 implementation plan

范围：把 `/Coda-any`、Skill 组合、Eval、Publish/Distribute 的剩余用户体验问题按优先级整理为一套可实现规格

## 背景

Coda 现在已经具备一组关键底座：

- `/Coda-any` 已经是普通用户创建 Coda-native Skill 的主入口。
- `.Coda/skill-preferences.yaml` 已经成为项目级 Skill 偏好的唯一普通用户文件。
- Bundle Factory 已能扫描候选 Skill、生成组合方案、写入计划和生成物证据。
- 生成物方向已经明确为完整控制面：`SKILL.md`、`scripts/`、`rules/`、`hooks/`、`reference/`、`Coda/*.yaml`、Eval manifest 和 Bundle 状态。
- `Coda eval` 已经承担用户评估 Skill 的入口。
- `Coda publish` / `Coda bundle` 的后端能力已经能支持 review、publish、distribute。

但从普通用户视角看，Coda 还没有达到“足够好用”。主要问题不是后端缺能力，而是能力暴露方式仍偏工程化：

- 首次使用 `/Coda-any` 时，用户不知道自己应该先描述目标、选择偏好、确认方案，还是先理解 Bundle 命令。
...
本 spec 承接已有完成项，不回滚现有设计，也不重新开放手写编排路径。目标是把 Coda 打磨成普通用户能顺着 `/Coda-any` 一路完成创建、验证、发布和分发的闭环产品。
...
```text
用户只描述想要什么 Skill
  -> /Coda-any 发现项目偏好和本机已有 Skill
  -> Coda 给出可确认的组合方案
  -> 用户确认或要求修改
  -> Coda 生成完整控制面 Skill
  -> 用户用 Coda eval 验证
  -> 用户用 Coda publish 审核、发布、分发
  -> 中途退出后可以从同一条路径恢复
```
...
- `flow.yaml`
- `bundle.yaml`
- `.Coda/bundle-authoring/*.json`
- `.Coda/bundle-factory-plans/*/plan.json`
- `Coda/skill.yaml`
...
1. 让首次使用 `/Coda-any` 的用户不需要记命令，也不需要理解 Bundle，即可开始创建 Skill。
2. 让 `/Coda-any` 在写入生成物前展示一个可决策的组合确认页。
3. 让 `/Coda-any` 对中断流程提供清晰恢复，不重复生成、不丢上下文。
4. 让 readiness 从内部检查语变成用户能行动的发布建议。
5. 让 publish/distribute 成为用户可理解的发布助手，而不是 Bundle 后端命令集合。
6. 保持 `.Coda/skill-preferences.yaml` 和 Bundle authoring state 作为已有事实源，不新增第二套状态机。
...
| 优先级 | 状态 | 主题 | 用户问题 | 目标状态 |
| --- | --- | --- | --- | --- |
| P0 | [x] | 首次使用向导化 | 用户不知道从哪里开始，也不想记命令 | `/Coda-any` 自动发现上下文、引导偏好、保存项目偏好 |
| P1 | [x] | 组合方案确认页 | 用户看不懂将要组合什么、生成什么、风险是什么 | 写入前展示可确认、可修改、可拒绝的方案 |
| P2 | [x] | 断点恢复体验 | 中途退出后不知道上次做到哪里 | `/Coda-any` 优先恢复进行中流程，并说明下一步 |
| P3 | [x] | readiness 用户语 | blocker/warning 太像内部诊断 | 输出“能不能发布、为什么、怎么修、下一步” |
| P4 | [x] | 发布和分发助手 | `bundle`、`publish`、`distribute` 心智混乱 | `Coda publish` 收口为用户发布助手，`bundle` 退为高级后端 |
...
```text
/Coda-any -> Coda eval -> Coda publish
```

`Coda bundle`、`factory-*`、内部 plan、Bundle authoring state 仍保留，但不作为 README 和 `/Coda-any` 主线要求用户记忆。
...
这些摘要可以由 CLI 输出，也可以由 `/Coda-any` Skill 消化后转述给用户；但字段和语义必须稳定，避免每次靠 Agent 自己猜。

### 3. 不新增第二套事实源

事实源保持不变：

- 项目偏好：`.Coda/skill-preferences.yaml`
...
`/Coda-any` 的目标不是只产出一个 `SKILL.md`。默认生成物必须包含：
...
用户第一次使用 `/Coda-any` 时，真实问题通常是：

```text
我想创建一个像 Coda 一样能稳定推进流程的 Skill。
我有一些偏好的 Skill，但不想每次都手写一串命令。
我不知道当前项目里有没有保存过偏好。
```

当前能力已经支持项目级偏好和候选扫描，但 `/Coda-any` 还需要把这些能力包装成自然的首次使用流程。

### 用户体验

当用户调用 `/Coda-any` 时，它应先判断当前项目状态：

1. 如果存在未完成的创建流程，优先进入 P2 的恢复流程。
2. 如果没有 `.Coda/skill-preferences.yaml`，进入首次使用向导。
...
你想创建什么 Skill？
你希望优先复用哪些已有 Skill？
这些偏好是否保存到 .Coda/skill-preferences.yaml？
是否允许生成 scripts/rules/hooks 作为控制面？
```

Skill 发现应扫描 Coda 支持的平台 Skill，并按去重后的用户可读名称展示。遇到同名 Skill 时，不直接让用户看内部路径列表，而是用来源分组说明：
...
- `/Coda-any` 必须先读取 `.Coda/skill-preferences.yaml`，没有时再生成推荐。
- 推荐偏好必须来自真实扫描结果，不能写死为固定列表。
- Skill 去重必须同时保留来源证据，避免隐藏同名冲突。
- 首次向导保存偏好前必须让用户确认。
- 保存后的偏好文件必须与 `.Coda/config.yaml` 同级。
...
- [x] 无偏好文件时，`/Coda-any` 能解释它将扫描 Skill 并创建项目偏好。
- [x] 用户可以不记任何 `Coda bundle` 命令完成首次偏好确认。
- [x] `.Coda/skill-preferences.yaml` 保存前有明确确认。
- [x] 同名 Skill 展示来源差异，并按策略要求用户选择或失败。
- [x] 文档明确说明用户可以手写 `.Coda/skill-preferences.yaml`，但不是必须。

## P1：组合方案确认页

### 用户问题

现在 `/Coda-any` 的后端可以生成 plan 和 metadata，但用户真正需要的是一个确认页：
...
在写入 draft 或进入不可逆步骤前，`/Coda-any` 必须展示组合方案确认页。确认页应该像这样组织：

```text
目标
  创建一个用于 <任务> 的 Coda-native Skill
...
`/Coda-any` 启动时应优先检查是否存在未完成流程，并给出恢复摘要：
...
恢复流程要避免重复生成。如果用户输入的目标或偏好发生变化，Coda 应解释这是继续旧流程还是创建新流程。

### 功能要求

- `/Coda-any` 必须在创建新流程前检查 active Bundle Factory / authoring state。
- 恢复摘要必须包含目标、当前阶段、已完成、未完成、建议下一步。
- 恢复摘要必须包含关键证据路径，但不把路径作为主要用户语言。
- 如果偏好 hash 或目标摘要变化，必须提示用户选择继续旧流程或开始新流程。
- 放弃流程必须是显式动作，不能因重新调用 `/Coda-any` 自动覆盖。
- JSON 输出必须保留机器可判定的 `nextAction`，文本输出必须是用户可读行动建议。

### 后端边界

- 复用 Bundle authoring state，不新增 `.Coda/Coda-any-state.yaml`。
- 可以增强 state summary / next action 聚合。
- 对旧状态做兼容读取，缺字段时降级为保守恢复提示。

### 验收标准

- [x] 中断后再次调用 `/Coda-any`，默认展示恢复摘要而不是直接新建。
...
原因：
  1. 还没有 Eval 证据。
     下一步：运行 Coda eval run --skill-path <draft-skill>

  2. 生成物声明了 hooks，但尚未确认目标平台支持。
     下一步：运行 Coda publish distribute --preview

修完后：
  重新运行 Coda publish review
```
...
- `/Coda-any` 必须把 readiness 作为发布前停顿点，而不是自动越过。
...
- [x] `/Coda-any` 在 readiness 非可发布时停止，并转述用户结论。
...
普通用户主路径应收敛到 `Coda publish`：

```bash
Coda publish status
Coda publish review
Coda publish approve
Coda publish run
Coda publish distribute --preview
...
- README 和 `/Coda-any` 文档主线使用 `Coda publish`，不要求普通用户记 `Coda bundle`。
- `Coda publish` 不新增状态机，只包装 Bundle authoring state。
- 分发前必须 preview scripts/rules/hooks 的目标平台写入计划。
- 如果目标平台不支持 required capability，必须阻止或要求用户明确处理。
- portable hook descriptor 必须由 Coda 编译/映射到平台原生配置，不能直接假装所有平台都能识别。
- 分发动作必须保留 rollback 或失败摘要，避免半安装后用户不知道状态。

### 后端边界

- `Coda bundle` 命令保持兼容。
- 可以新增或增强 `Coda publish` façade。
- 不自动向外部平台写入，除非用户已经确认目标和执行性能力。
- 不在 README 主线展开 Bundle 内部命令。

### 验收标准

- [x] 用户能通过 `Coda publish status/review/approve/run/distribute` 理解发布流程。
- [x] `/Coda-any` 的发布指引只暴露 `Coda publish` 主路径。
- [x] `distribute --preview` 明确列出目标平台、写入文件、scripts/rules/hooks、unsupported capability。
- [x] 分发失败时输出已写入、已回滚、需要人工处理的内容。
- [x] `Coda bundle` help 明确为高级后端，不再作为普通用户 quickstart 主线。

## 跨优先级数据契约

### 项目偏好

`.Coda/skill-preferences.yaml` 是项目级偏好的事实源。P0-P2 都必须读取它，并在需要时记录偏好 hash，用于判断恢复流程是否仍匹配当前输入。
...
- `docs/zh/` 下 `/Coda-any`、Eval、Publish/Distribute 相关文档
- `docs/` 对应英文文档
- `README-zh.md`
- `README.md`
- CLI help 文案
...
- 打包烟测：构建后 `Coda publish --help`、`Coda eval --help`、`Coda status` 不因资源路径或 help 注册失败。

## 分阶段实现建议

### 第一阶段：P0 + P1

先让 `/Coda-any` 的开始和确认闭环可用：
...
- [x] 新用户可以只调用 `/Coda-any` 开始创建 Skill，不需要先学习 Bundle CLI。
- [x] 用户可以看到并确认组合方案，知道会复用哪些 Skill、生成哪些控制面。
- [x] 中断后再次调用 `/Coda-any`，用户能清楚恢复而不是重新开始。
- [x] Eval 和 readiness 的输出能告诉用户是否能发布以及下一步怎么做。
- [x] 用户可以通过 `Coda publish` 完成审核、发布、分发，不需要理解 `bundle` 内部命令。
...
- 本 spec 没有要求用户学习 `Coda bundle` 才能完成普通路径。
