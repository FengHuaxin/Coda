# Coda 稳定组合 Skill 控制面设计

日期：2026-06-23

## 背景

当前 `/Coda-any` 已经开始承担“从用户目标生成可评估、可发布 Skill”的职责，但现有 factory 仍偏向生成一个线性的 `callChain`：

- 生成入口 `SKILL.md`。
- 生成 `Coda/skill.yaml`、`Coda/guardrails.yaml`、`Coda/evals.yaml`、`Coda/eval.yaml`。
...
本设计将 `/Coda-any` 生成物收束为一类明确产物：**稳定组合 Skill Bundle**。它不是单独的 `SKILL.md`，而是一套包含入口文档、Engine 计划、脚本、规则、hook、运行检查、评估 manifest 和分发声明的完整控制面。

## 目标

1. `/Coda-any` 生成的 Skill 默认具备稳定自动推进、断点恢复和越界阻断能力。
2. 组合 Skill 的运行时只执行最终编译出的无环执行计划，不在运行时递归展开来源 Skill。
3. 生成 Bundle 必须包含 `scripts`、`rules`、`hooks`，并把它们声明为 required capability。
4. `hooks/*.yaml` 明确作为 Coda portable hook descriptor，由 `Coda publish distribute` 编译为各平台原生配置。
5. 将运行时检查文件从 `evals.yaml` 收束为更清晰的 `checks.yaml`，保留 `eval.yaml` 作为 Eval 平台 manifest。
6. 用户路径收束为 `/Coda-any -> Coda eval -> Coda publish -> distribute`，底层 `skill` / `bundle` CLI 保持高级调试定位。

## 非目标

- 不开放手动编排 Skill 的新用户路径。
- 不要求所有已有手写 Skill 立即迁移到稳定组合 Bundle。
- 不把 Coda portable hook descriptor 当作平台原生 hook 配置直接安装。
- 不让目标平台在缺少 required hook/rule/script 能力时静默降级。
- 不改变内置 Classic Coda 的 `.Coda.yaml` 状态机事实源。

## 核心概念

### 稳定组合 Skill Bundle

`/Coda-any` 生成的默认产物。它包含：
...
/Coda-any 创建 Skill
  -> Coda eval 验证 Skill
  -> Coda publish review/approve/run
  -> Coda publish distribute 分发到目标 Agent 平台
```

底层命令定位：

- `Coda skill`: 检查、安装、运行单个 Engine-native Skill 的低层工具。
- `Coda bundle`: 检查、编译、调试 Bundle 的低层工具。
- `Coda eval`: 用户可见的评估入口。
- `Coda publish`: 用户可见的发布与分发入口。

README 和用户文档应以 `/Coda-any -> eval -> publish` 为主线。手动 YAML、手动 Bundle、手动 hook descriptor 不进入新用户路径。
...
resources:
  ...
  scripts:
    - id: Coda-plan
      path: skills/my-workflow/scripts/Coda-plan.mjs
      sideEffect: write
      runtime: node
    - id: Coda-check
      path: skills/my-workflow/scripts/Coda-check.mjs
      sideEffect: read
      runtime: node
    - id: Coda-hook-guard
      path: skills/my-workflow/scripts/Coda-hook-guard.mjs
      sideEffect: read
      runtime: node
  assets: []
platforms:
...
### `skills/<entry-skill>/Coda/skill.yaml`

最终执行计划。它由 `/Coda-any` 编译生成，运行时读取它推进 Step。
...
### `skills/<entry-skill>/Coda/guardrails.yaml`
...
### `skills/<entry-skill>/Coda/checks.yaml`
...
### `skills/<entry-skill>/Coda/eval.yaml`

Eval 平台 manifest。它不是运行时检查文件，而是告诉 `Coda eval` 如何评估这个 Skill。
...
Coda portable hook descriptor。它不是任何平台的原生配置。
...
```text
读取用户目标
  -> 读取 .Coda/skills.txt 和用户偏好
  -> 发现候选 Skill
  -> 读取候选 SKILL.md / Coda/flow.yaml / references / scripts
  -> 展开来源 Skill 小流程
  -> 解决 Choice
  -> 检查环路
  -> 编译最终 Plan
...
- 必须读取并遵守 `Coda/skill.yaml`。
- 每个 Step 完成后必须通过 `Coda-plan.mjs` 写入 outcome。
...
hook 的具体平台落地由 `Coda publish distribute` 完成。源 YAML 不直接安装。

## Eval 门禁

对稳定组合 Skill Bundle，`Coda eval` 至少应验证：

1. `bundle.yaml` 声明 required `skills/scripts/rules/hooks/references`。
2. `SKILL.md` 存在并引用最终工作方式。
3. `Coda/skill.yaml` 是无环 Plan。
4. `Coda/guardrails.yaml` 只允许 Plan 中声明的 Skill/Tool/Agent。
5. `Coda/checks.yaml` 存在并可加载。
6. `Coda/eval.yaml` 存在并可被 eval harness 发现。
...
`Coda publish distribute` 写入前必须展示：
...
- 不把平台 hook 生成逻辑放入 `/Coda-any`。
- `/Coda-any` 只生成 portable descriptors 和 required capability。
...
Coda 自身稳定的经验是 `Skill 文档 + scripts + rules + hooks` 形成闭环。`/Coda-any` 生成的稳定组合 Skill 也应继承这套机制。

### 为什么 hook YAML 不是平台配置

各 Agent 平台 hook 格式不同。Coda 必须保留一个 portable descriptor，然后由分发层编译成平台原生配置。这样可以：
...
- 文档主路径改为 `/Coda-any -> Coda eval -> Coda publish`。
...
- [x] `/Coda-any` 生成的新 Bundle 包含完整 required control plane。
...
- [x] `Coda eval` 能验证控制面完整性。
- [x] `Coda publish distribute` 能把 hook descriptor 编译到支持的平台，并在不支持的平台 fail closed。
- [x] 文档只把 `/Coda-any -> eval -> publish -> distribute` 作为普通用户主路径。
- [x] `Coda skill` 和 `Coda bundle` 的帮助文案明确为低层调试/高级入口。
...
Coda 下一步优化的核心不是新增更多命令，而是把 `/Coda-any` 生成物升级为稳定组合 Skill Bundle：
...
这让 Coda 的用户体验从“生成一个 Skill 文档”升级为“生成、验证、发布、分发一套可恢复、可阻断、可评估的自动化 Skill 系统”。
