# Coda Any 创作协议化设计（Authoring Protocol）

日期：2026-06-28

状态：设计稿，待实现。本文取代 `/Coda-any` 创作侧"散文 + 手搓 JSON"的现状，作为 Phase 0–3 的统一规格。

范围：把 `/Coda-any` 自己的创作过程从"模型自由发挥"升级为"协议驱动"，并修复审查证据伪造、命令引用失真、产物清单不一致等问题，目标是让生成物达到 Coda 手写 skill 的可用度。不修改 Superpowers / OpenSpec / 用户原始 Skill；不改变 runtime 侧已有的 `workflow-protocol.json` 契约。

## 背景

审查 `assets/skills/Coda-any/**` 与后端（`domains/bundle/factory.ts`、`domains/factory/package.ts`、`domains/workflow-contract/builtins.ts`、`app/cli/index.ts`）后发现一个核心矛盾与若干脱节：
...
2. **find-skill 命令不存在（HIGH）**：SKILL 指示 agent `use find-skill`，但 CLI 无此命令，实际是 `Coda bundle candidates` / `Coda skill show`。
3. **脚本清单三方不一致（HIGH）**：生成器产出 6 个脚本（Coda-plan/Coda-check/Coda-hook-guard/workflow-state/workflow-guard/workflow-handoff），但 SKILL、bundle-authoring、script-author、skill-reviewer 只认 3 个 workflow-*。
4. **生成内容薄（MEDIUM）**：entry/node skill 是公式化模板，缺乏 Coda 的领域智能。
5. **缺契约测试（MEDIUM）**：现有测试只校验"文档包含词汇"，不校验命令/产物清单一致性。

关键判断：Coda-any 已在 **runtime 侧** 用对了"protocol 驱动"模式（`workflow-protocol.json` 是确定性事实源，脚本只解释它）；但 **创作侧** 没用同一套思想。本设计把 runtime 侧已验证的模式反向应用到创作侧。
...
- 让 `/Coda-any` 的创作过程由 `authoring-protocol.json`（确定性事实源）驱动，主会话"解释执行"它，对称于 runtime 侧的 `workflow-protocol.json`。
- 让生成的 Bundle 区分 **确定性脊梁**（protocol/scripts/manifest/Coda yaml，永远模板化、可复现）与 **LLM 内容叶子**（entry/node SKILL.md、decision-points、recovery，优先采用子代理草稿），通过 content-merge 合并。
...
- 不把 `/Coda-any` 绑定到 Claude Code 的 `Workflow` 工具。
...
`authoring-protocol.json` 是 Coda-any skill 自带的 **固定创作契约**（随 skill 发布），描述"一次创作如何编排"。每个生成 Bundle 的具体执行记录仍写入 Bundle 内的 `reference/authoring-lanes.json`（已有，扩展）。
...
```jsonc
{
  ...
  "lanes": {
    "script": {
      ...
      "claims": [
        ...
        "script:Coda-plan", "script:Coda-check", "script:Coda-hook-guard"
      ],
  ...
  "merge": {
    "deterministicBackbone": [
      ...
      "scripts/Coda-check.mjs", "scripts/Coda-hook-guard.mjs", "scripts/Coda-plan.mjs",
      "bundle.yaml", "Coda/skill.yaml", "Coda/guardrails.yaml", "Coda/checks.yaml", "Coda/eval.yaml",
...
- `protocolHash`：规范 JSON 的 sha256，`Coda bundle authoring-plan` 校验其与 skill 自带版本一致。
...
```jsonc
{
  ...
  "artifacts": [
    { "path": "../team-Coda-execute/SKILL.md", "kind": "skill", "content": "..." }
  ],
  "claims": [
    { "id": "node-skill:team-Coda-execute", "kind": "skill", "paths": ["../team-Coda-execute/SKILL.md"], "summary": "..." }
  ],
  "findings": [
...
校验失败（schema 不合规、status 为 BLOCKED/NEEDS_CONTEXT、缺关键 claim）时，`Coda bundle authoring-record` 拒绝落盘并要求补上下文。
...
- `usability`：是否如 Coda 般清晰、可恢复、可审计、不过曝内部元数据。
...
```bash
Coda bundle authoring-plan <name> --depth quick|full --json
#   读 Bundle factory 元数据 + skill 自带 authoring-protocol.json，
#   产出本次创作的 dag/lane 清单/depth/expected-claims，校验 protocolHash。

Coda bundle authoring-record <name> --lane <id> --file <out.json> [--json]
...
`Coda bundle factory-generate`：把 `state.factory.authoringContent` 作为 `contentDrafts` 传入生成器。
...
现有 human `Coda bundle review --approve` 保留，与 LLM 审查 **叠加** 门禁（两者都过才 ready）。

## 仓库契约测试（Phase 0 / M5）

在 `test/domains/bundle/` 与 `test/repository/` 新增：

1. `Coda-any-skill-contract.test.ts`：解析 `assets/skills/Coda-any/SKILL.md` 与 reference，断言其中引用的每个 `Coda <...>` 命令 ⊆ `app/cli/index.ts` 实际注册命令；断言 `find-skill` 不再出现。
2. 断言 Coda-any 声明的 "Generated Package" 清单 == `generateFactorySkillPackage` 实际产物（含 6 脚本）。
...
- 更新 `assets/skills/Coda-any/SKILL.md` 与 `authoring-subagents.md`（中英）：指导主会话——支持 subagent 的平台并发派发 wave1，wave2 依赖 script，barrier 汇聚后审查；无 subagent 平台按 DAG 依赖内联顺序执行，语义不变，`authoring-lanes.json` 记 `dispatchMode`。
...
- `assets/skills/Coda-any/**` 与 `assets/skills-zh/Coda-any/**`：新增 `reference/authoring-protocol.json`、`reference/schemas/*.schema.json`；修订 SKILL.md（find-skill、脚本清单、DAG 指导、触发规范）。
- `test/`：契约测试 + authoring 域测试。

## 验收标准

- Coda-any 的 SKILL/reference 引用的命令全部在 CLI 注册；`find-skill` 不再出现。
- Coda-any 声明的产物清单与生成器实际产物一致（含 6 脚本）。
...
- 在临时目录用 Coda-any 真实跑通 customize-Coda、new workflow-kernel、upgrade-existing 三类场景，生成物可加载、状态机可推进、审查证据真实。
