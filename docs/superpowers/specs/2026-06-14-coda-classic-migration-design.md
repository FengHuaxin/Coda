...
Coda 0.3.8 的经典流程仍由多个 shell 脚本直接解析和修改
`.Coda.yaml`。完整流程、hotfix、tweak、handoff、guard、archive 和 hook
分别持有一部分状态机逻辑，存在以下问题：
...
1. 保留现有 `/Coda`、`/Coda-open`、`/Coda-design`、`/Coda-build`、
   `/Coda-verify`、`/Coda-archive`、`/Coda-hotfix` 和 `/Coda-tweak`。
2. 用一个内部 `Coda-classic` Skill 描述经典流程的稳定步骤图。
...
- 不改变用户的经典工作流交互方式。
- 不新增用户可见的 `Coda-classic` 命令。
- 不删除 0.3.8 字段；字段移除属于后续独立迁移。
- 不实现 Plan 3 的手动编排作者体验。
- 不实现 Plan 4 的 `/Coda-any` agentic Skill 组合。
...
`Coda-classic` 是内部 Skill。manifest 新增 `internalSkills`：
...
## 8. 内部 `Coda-classic` Skill

双语言包：

```text
assets/skills-zh/Coda-classic/
  SKILL.md
  Coda/
    skill.yaml
    guardrails.yaml
...
1. 定位 `Coda-env.sh`；
2. 组装 runtime 路径；
3. `exec node "$Coda_RUNTIME" <command> "$@"`。
...
```text
assets/skills/Coda/scripts/Coda-runtime.mjs
```

只生成这一份。Coda 安装器对所有语言均从 `assets/skills` 获取 `/scripts/`
资源，因此无需在中文资源树重复生成。
