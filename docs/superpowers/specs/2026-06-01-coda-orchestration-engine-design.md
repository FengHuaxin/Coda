# Coda Skill Engine - 设计文档

- 初稿日期：2026-06-01
- 重构日期：2026-06-13
- 状态：已实现主体能力并完成术语修订（Foundation、Manual Authoring、Classic 运行时与 `/Coda-any` 主线已落地；后续 Engine-native 收敛由 2026-06-22/23 specs 追踪）
- 目标版本基线：Coda 0.3.8

## 1. 背景

原设计希望把 Coda 从固定的 OpenSpec + Superpowers 五阶段流程，改造成由 YAML
状态图驱动的通用 Skill 编排引擎。

该方向抓住了“编排不应硬编码在 Skill 散文中”的问题，但在分支开发期间，master
已经补齐了大量稳定性能力：

- `Coda-state.sh transition/next/check --recover`
- `auto_transition`
- plan-ready 暂停与恢复
...
因此，原计划中“另建 `.Coda.flow.yaml`，再让新引擎逐步替代现有流程”的方案已经
不再成立。它会造成两套状态源、两套恢复逻辑和两套稳定性契约。

本设计重新定义目标：

> Coda 不只是一个手工 workflow 引擎，而是一个创建、执行、恢复和评估复杂 Skill
> 的通用运行时。用户既可手工编排，也可通过 `/Coda-any` 让 Agent 根据目标和现有
> Skill 自主组合，但两条路径最终交付同一种可验证的 Coda Skill。

## 2. 产品概念

对外只引入一个核心概念：**Coda Skill**。

- **Manual Skill**：高级用户手工定义内部 Skill Spec。
- **Agentic Skill**：由 `/Coda-any` 通过交互澄清、能力探索和 Agent 组合生成。
- **Coda Skill Engine**：执行、约束、持久化、恢复和评估 Coda Skill 的底层引擎。
...
Coda Skill 必须能在上下文压缩、进程中断、模型切换和能力来源变化后恢复。稳定性不能
依赖 Agent 记住之前的对话。
...
Coda Skill 对外仍是普通 Skill 目录，对内增加由引擎消费的机器描述。
...
并非每个 Skill 都必须拥有所有可选目录。`/Coda-any` 根据实际需求生成最小结构。
...
两者使用同一个执行循环。手工创建和 `/Coda-any` 创建是 Skill 的来源，不是两套运行
模式；任一来源都可以选择 deterministic 或 adaptive Orchestration。

### 5.3 Skills

Skills 是 Coda 的唯一可组合行为单元。一个 Skill 可以依赖其他 Skill，但只有当 Coda
需要发现、选择、排序、版本锁定或单独评估该依赖时，才把它声明到 Skill Spec。

Skill 内部自行使用的 Tool 或 MCP 不会自动提升为编排节点。Coda 不把所有底层能力
...
MCP 是 Tool 的连接协议和来源，不与 Tool 平级成为编排节点。只有当 Coda 需要对 Tool
...
每次执行 Coda Skill 都产生一个 Run。Run 包含：
...
Checkpoint 不复制另一份可独立修改的 State，`.Coda.yaml` 始终是唯一状态真相源。
...
Coda 不追求把 Skill 内所有概念面面俱到地建模。只有引擎需要拥有以下职责时，概念
才进入 Skill Spec：
...
Coda Skill Engine 是 `.Coda.yaml` 的唯一状态写入者。

旧 shell 命令暂时保留，但只作为兼容门面调用新引擎，不再实现独立状态转换。

## 7. 状态模型

`.Coda.yaml` 继续是唯一运行状态文件，保留 0.3.8 的现有字段，并渐进增加：
...
Skill 启动时将以下信息快照到 change 的 `.Coda/`：
...
恢复时读取快照，不自动跟随后来修改的 Skill 或 `.Coda/skills.txt`。升级运行中 Skill
必须走显式升级和兼容校验。
...
`/Coda-any` 是 Agentic Skill 创建器，不是临时 workflow runner。

### 9.1 创建流程

1. 读取用户描述和 `.Coda/skills.txt`。
...
`/Coda-any` 根据任务风险和长度选择性注入 Coda 已验证的能力：
...
- 平台缺失完整评估能力时，使用 Coda 兼容 Provider。
...
现有 OpenSpec + Superpowers 流程改造成首个内置 Coda Skill。
...
```text
                    +----------------------+
手工 Skill Spec --->|                      |
                    |  Coda Skill Package |---> Eval Provider ---> Ready
/Coda-any -------->|                      |
                    +----------+-----------+
                               |
                    +----------v-----------+
...
- 建立 `.Coda.yaml` 增量 schema、Trajectory、Context、Artifacts 和 Checkpoints。
...
| 原计划 | 新设计 |
|---|---|
| `*.flow.yaml` 是产品核心 | Coda Skill 是产品核心，Skill Spec 是内部 IR |
| 用户主要手写 workflow | 同时支持手工定义和 `/Coda-any` Agentic 创建 |
| node = one skill | Skill 是组合单位；Agents 与 Tools 是声明依赖，不强制成为节点 |
| 固定状态图决定下一步 | deterministic 或受 Guardrails 约束的 adaptive Orchestration |
| `.Coda.flow.yaml` 独立状态 | 扩展现有 `.Coda.yaml`，保持单一真相源 |
...
- 用户可以手工创建并运行 Coda Skill。
- 用户可以通过 `/Coda-any` 仅描述目标和候选 Skill，获得可安装的 Coda Skill。
- `/Coda-any` 会读取候选 Skill 实现，而不是仅按名称排列。
