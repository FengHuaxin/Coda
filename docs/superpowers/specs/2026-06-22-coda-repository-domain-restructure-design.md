...
**范围：** 将 Coda 仓库从以技术层为主的布局调整为以领域为主的布局，同时保持 CLI 对外行为和安装结果兼容
...
- `src/compat/` 实际承担了 Coda Classic 产品面的主要逻辑；
...
2. 让新人能够从顶层结构快速理解 Coda 的主要子系统。
3. 明确区分入口装配、业务领域和平台基础设施。
4. 将 Eval 提升为一等主题，而不是脚本附属物。
5. 保持 `Coda` CLI 对外命令、参数和行为兼容。
...
- 不在本次重组中重新设计 Coda Classic、Skill Engine、Bundle 或 Eval 的功能行为。
...
### 6.1 `domains/Coda-classic`

承接当前 `src/compat/`。其真实语义不是“兼容层”，而是 Coda Classic 产品面。
...
1. `src/compat/` -> `domains/Coda-classic/`
...
4. 将 `src/compat/` 重新命名并迁移为 `domains/Coda-classic/`。
