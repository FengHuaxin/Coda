...
**范围：** `/Coda-any` 体验重定位与 Engine 闭环设计

## 1. 背景

现有 `/Coda-any` 设计把它定位为 Skill Bundle Creator：用户通过 Skill 引导创建 Bundle，
CLI 负责候选发现、草稿、Eval、发布和分发。这个设计已经能表达平台无关的多 Skill
Bundle，但用户体验仍偏向“高级用户知道 Bundle 生命周期”，且 Engine 只作为可选元数据。

新的产品目标是：`/Coda-any` 本身就是用户的创作入口。用户不需要学习 `Coda bundle`
命令，也不需要知道 Engine CLI 的细节；用户只调用 `/Coda-any`，描述目标或偏好，
Agent 读取本地真实 Skill 内容，组合并生成一个类似 `/Coda` 的 Coda-native Skill，
再由 `/Coda-any` 内部调用 CLI 完成校验、发布和跨平台分发。

因此，本设计将 `/Coda-any` 从“Bundle CLI 使用指南”重定位为
**Coda Skill Factory**：一个面向用户的 Skill 创作体验，内部使用 Bundle 后端和
...
- `/Coda-any` 作为唯一用户入口完成 Skill 创作、验证、发布和可选分发。
- 用户只需要调用 Skill，不需要手动执行 `Coda bundle` 或 `Coda skill` 命令。
- `.Coda/skills.txt` 表示用户偏好和推荐调用顺序，`/Coda-any` 必须本地查找真实 Skill
  内容，而不是只按名字推测。
- 产物默认是 Coda-native Skill：包含 `SKILL.md`、references、scripts，并可包含
  `Coda/skill.yaml`、guardrails 和 runtime evals。
- 生成的 Skill 可以像 `/Coda` 一样被分发到 Coda 支持的平台。
- CLI 是内部后端：负责状态、编译、校验、Eval、发布、分发和安全检查。
- Engine 是运行语义底座：负责生成 Skill 的结构化编排、guardrails、恢复和 eval 合同。
- 平台原生 Skill 仍是入口，但入口应指示 Agent 使用 Coda 生成的 Skill 语义；需要持久化时通过
  Engine runner 间接驱动。
...
- 不把 Superpowers 或 OpenSpec 原始 Skill 改写成 Coda 私有版本。
- 不在没有用户确认时执行创建期 Eval 或跨平台分发。
- 不把 `.Coda/skills.txt` 解释为严格白名单；它是偏好输入和顺序信号。
- 不在第一版实现完全 adaptive 的自动规划 Agent runtime。
- 不让平台 Hook 或脚本绕过现有能力缺口检查和可执行披露确认。

## 4. 核心决策

- `/Coda-any` 是产品界面，CLI 是内部实现细节。
- 生成物优先是 Coda-native Skill Package，再通过 Bundle 包装为跨平台分发单元。
...
- 平台分发后仍安装为平台原生 Skill，但 Skill 文案和脚本应知道如何定位 Coda runtime 或说明
  如何由当前 Agent 走 Engine action/outcome 协议。
- 用户偏好 Skill 必须通过本地 `find-skill` 解析为真实路径、内容、hash 和来源。
- `.Coda/skills.txt` 的行顺序是用户偏好的推荐调用顺序。Factory 应尽量按该顺序设计
  entry/internal Skill 的调用链；如果因为目标、依赖、风险或平台限制偏离顺序，必须在评审摘要中说明原因。
- `/Coda-any` 允许推荐不启用 Engine 的轻量 Skill，但必须显式说明能力损失。

### 4.1 当前实现对齐状态（2026-06-23）

本设计仍是 `/Coda-any` Skill Factory 的产品和验收参考。当前代码已完成主体后端闭环；剩余差距集中在用户主路径、输出解释、README 任务路径和更细粒度 readiness 证据上，继续由 2026-06-23 UX spec 追踪。

已与当前代码吻合的部分：

- `assets/skills/Coda-any/SKILL.md` 与 `assets/skills-zh/Coda-any/SKILL.md` 已把 `/Coda-any` 定位为用户入口，CLI 是内部确定性后端。
...
- `domains/factory/package.ts` 已生成 entry Skill、`reference/resolved-skills.json`、source summaries，并按 `engineMode` 输出 `Coda/skill.yaml`、`guardrails.yaml`、`evals.yaml`。
- `domains/engine/standalone-run.ts` 与 `--run-id` 支持独立 `.Coda/runs/<run-id>` 运行目录。

仍需由后续 UX 闭环继续推进的部分：

- 旧设计中的 `src/*` 路径已不再代表当前代码位置；实现应使用 `domains/*` 与 `app/*` 下的现有模块。
- Bundle-level `engine.enabled` 当前不是 Factory 主路径。Factory 主要通过生成 entry Skill 内的 `Coda/` Engine 文件表达 Engine 语义。
...
因此，本设计仍建议参考其中的产品模型、安全规则、用户流程和验收标准；具体文件路径、实现切分和完成度判断应以当前代码与 `2026-06-22-Coda-priority-improvements-design.md` 为准。
...
生成 Coda-native Skill 源码
        ...
        +--> Coda/skill.yaml / guardrails.yaml / evals.yaml
        |
        v
内部 Bundle 后端
        |
...
用户看到的是 Skill 创作流程；CLI 只在 `/Coda-any` 内部作为确定性工具运行。高级用户仍可通过
CLI 审计或恢复，但这不是主路径。
...
`/Coda-any` 优先读取项目 `.Coda/skills.txt`：
...
- 项目级 Coda Skill：`<project>/.Coda/skills/`。
- Coda 内置 Skill：`assets/skills/`、`assets/skills-zh/`。
...
当同一个 Skill 来自扫描而非 `.Coda/skills.txt` 时，`preferenceIndex` 为 `null`。当来自
`.Coda/skills.txt` 时，`preferenceIndex` 必须保留去重后的首次出现顺序，用于后续调用链建模。

## 7. Skill Factory 输出

`/Coda-any` 生成的源码目录应以 Coda-native Skill 为中心：
...
轻量单步 Skill 可以不启用 Engine，但 `/Coda-any` 必须说明这会失去 Run 恢复和 runtime eval。
...
当前 Agent 执行动作
      |
      v
Coda skill resume --status ... --summary ...
```
...
- Change 绑定：用户在 Coda change 内运行时，使用 `openspec/changes/<name>/`。
- 独立运行：无 change 时，使用 `.Coda/runs/<run-id>/`。

第一版可以优先实现 change 绑定，并在无 change 时让 `/Coda-any` 生成的 Skill 创建独立 run
...
```bash
Coda skill start-or-resume <skill> --run-dir <dir> --json
```

或在现有命令上封装：

```bash
Coda skill run <skill> --change <dir> --json
Coda skill resume --change <dir> --json
```
...
`Coda bundle` 继续存在，但角色调整为内部后端：

- `/Coda-any` 调用 `draft create/optimize` 初始化状态。
- `/Coda-any` 写入生成的 Skill Factory 源码。
- CLI 做结构校验、hash、compile、eval-record、review、publish、distribute。
- 用户只看到摘要、风险、能力缺口和确认问题。

CLI 文档应面向高级用户和自动化，但 `/Coda-any` Skill 文案不应把 CLI 操作作为用户主流程。

## 10. `/Coda-any` 用户流程

1. 恢复已有 factory 状态。
2. 询问用户目标：新建 Skill、优化已有 Skill，或组合偏好 Skill。
3. 读取 `.Coda/skills.txt`，通过 `find-skill` 查找真实 Skill。
4. 展示缺失/歧义候选并等待用户处理。
5. 读取最终候选的 `SKILL.md`、直接 reference、rules、scripts 和 hooks；不执行候选脚本。
6. 总结候选能力、冲突、适用边界、组合机会和偏好顺序。
7. 先按 `.Coda/skills.txt` 顺序提出默认调用链；如需要调整，列出每个调整的原因。
8. 与用户确认新 Skill 的入口、目标、非目标、成功标准、恢复需求、Eval 档位和分发平台。
9. 生成 Coda-native Skill 源码和可选 internal Skill。
...
需要修改上一版 `/Coda-any` 设计中的以下假设：

- 将“Engine 只作为可选元数据或未来运行时信息”改为“Engine 是 Coda-native Skill 的运行语义底座，按流程复杂度默认生成”。
- 将“用户可手工调用 Bundle CLI 恢复流程”降级为高级审计能力；主路径必须由 `/Coda-any` 自动调用。
- 将 `.Coda/skills.txt` 从“名字列表”升级为“偏好查询输入”，由 `find-skill` 解析真实内容。
- 将 `.Coda/skills.txt` 的行顺序升级为推荐调用顺序，生成调用链时尽量遵守；偏离必须解释。
- 将 Bundle 产物从“平台原生 Skill 集合”升级为“Coda-native Skill Package + 跨平台 Bundle 包装”。
...
- 通过 `domains/bundle/candidates.ts` 桥接 Bundle candidates 与 Skill finder，保持 `.Coda/skills.txt` 偏好顺序。
...
- 继续扩展 `domains/factory/package.ts` 中从候选 Skill 和用户目标生成 entry Skill 与 `Coda/skill.yaml` 草稿的适配层。
- 校验生成的 Skill Package 能被 `Coda skill validate` 加载。
- 生成 guardrails 和 evals 的最小安全默认值。

### 13.4 Runner

- 基于 `domains/engine/standalone-run.ts` 继续完善 `.Coda/runs/<run-id>` 或通用 `--run-dir`。
- 提供平台入口可调用的薄 runner 命令，并与 `/Coda-any` 生成的 entry Skill 文案对齐。
- 确保 runner 不直接执行 LLM，只产生/接收 action outcome。

### 13.5 `/Coda-any` Skill 文案
...
- README 保持克制，只增加 `/Coda-any` Skill Factory 的简短说明并链接 docs。
- 详细行为写入 docs。
...
- `find-skill`：
  - `.Coda/skills.txt` 注释、去重、顺序和 `preferenceIndex`。
  - 显式路径解析。
  - 项目/全局/内置/插件多来源歧义。
  - 缺失候选。
...
- Factory generator：
  ...
  - 生成 `Coda/skill.yaml`、guardrails、evals。
  - 默认调用链尽量遵守 `preferenceIndex`。
  - 偏离偏好顺序时输出结构化原因。
  - 生成物可通过 `Coda skill validate`。
  - script Tool 路径安全和副作用声明。

- Runner：
  - change 绑定 run。
  - 独立 `.Coda/runs/<run-id>` run。
  - start/resume 幂等。
...
- Bundle 分发：
  - 生成的 Coda-native Skill 能编译到至少一个参考平台。
...
1. 用户可以只调用 `/Coda-any` 创建一个新的 Coda-style Skill。
2. 用户不需要手动运行 `Coda bundle` 或 `Coda skill` 才能完成主流程。
3. `.Coda/skills.txt` 中的偏好会被解析为本地真实 Skill 内容。
4. `.Coda/skills.txt` 的顺序会作为推荐调用顺序进入生成逻辑。
5. 生成物尽量遵守偏好顺序；无法遵守时必须输出偏离原因。
6. 缺失和歧义偏好会展示给用户处理。
7. 生成物包含可分发的 entry Skill。
8. 多步骤或高风险生成物包含 Engine Package。
9. 生成物可通过 `Coda skill validate`。
10. 生成物可通过 Bundle 后端发布为 ready。
11. 用户确认后可分发到 Coda 支持的平台。
...
15. 中文和英文 `/Coda-any` 行为一致。
16. Classic workflow 和现有 `Coda bundle` CLI 行为不回归。
