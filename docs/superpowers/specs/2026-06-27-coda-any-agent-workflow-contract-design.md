# Coda Any Agent Workflow Contract Design

日期：2026-06-27

状态：已按实现落地，作为 `/Coda-any` 后续演进的当前规格。

范围：让基于 Coda 现有 Skill 的五阶段定制、任意 Skill workflow、手动 Skill 编排、runtime scripts、eval、review、publish readiness 共享一套 Agent workflow contract。系统尚未上线，本规格直接定义新的事实源，不承担旧草案兼容负担。

## 背景

`/Coda-any` 需要支持三类用户目标：

1. 基于 Coda 现有 Skill 的五阶段定制：保留 Coda 主流程控制能力，同时在执行、子代理执行、审查等节点要求项目 Skill 参与。
...
- 让 Coda 五阶段定制与任意 workflow kernel 共用同一个 Workflow Protocol。
...
- `id`：稳定 schema id，例如 `Coda.plan.v1`。
...
- Control Node 的状态推进由 workflow state 或 Coda state 校验。
- 自定义校验器必须进入 executable disclosure。

## Coda 五阶段定制

基于 Coda 现有 Skill 的五阶段定制覆盖的是现有阶段 Skill，而不是修改 `/Coda` 命令本身：
...
- `open`：control，产出 `Coda.intake.v1`。
- `design`：producer，产出 `Coda.design.v1`。
- `plan`：producer，产出 `Coda.plan.v1`。
- `execute`：control，产出 `Coda.execution-evidence.v1`。
- `subagent-execute`：handoff，产出 `Coda.handoff.v1`。
- `review`：guardrail，产出 `Coda.review.v1`，可选。
- `verify`：control，产出 `Coda.verify.v1`。
- `archive`：control，产出 `Coda.archive.v1`。
...
这个例子不会替换 Coda 主控制节点；它只要求指定节点必须调用额外 Skill 并记录 evidence。

## 任意 Skill Workflow

任意 workflow 不继承 Coda 的 OpenSpec-specific 语义，而是生成 `workflow-kernel`：

- Nodes 来自用户目标、偏好 Skill、手动定义或自动规划。
- Edges 定义成功、失败、暂停和恢复路径。
- State 写入 `.Coda/runs/<workflow>/state.json`。
- Runtime scripts 读取 `reference/workflow-protocol.json`。

这种模式适合研究-写作、设计-实现-审查、数据处理、团队规范检查等非 Coda 流程。
...
- `Coda-check.mjs`：给 eval/readiness 复用，检查 protocol、schemas、artifacts 和 state。
- `Coda-hook-guard.mjs`：在需要 hook guard 的平台上按 protocol 和 state 阻断无效操作。
...
`Coda eval`、review summary、publish readiness 均读取 Workflow Protocol。
...
这不会替换 Coda 主流程，也不会改变计划、验证或归档产物。
```
...
请确认 team-planning 满足 Coda.plan.v1，或提供它产出的文件路径和校验方式。
```

## 模块划分

- `domains/workflow-contract`：WorkflowDefinition、WorkflowProtocol、Node、Binding、OutputSchema、Guardrail、normalization、validation、hashing、内置 Coda template。
...
- `/Coda-any` 用同一套 Workflow Contract 表达 Coda 五阶段定制、任意 workflow 和手动编排。
- 用户可以要求执行和子代理执行节点必调 `elementui`，生成物不替换 Coda 主流程。
...
- 生成 Skill 保持 Coda-like 的单入口、自动推进、停顿点、恢复、证据和阻断体验。
