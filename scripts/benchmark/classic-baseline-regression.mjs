...
const RUNTIME = path.join(REPO_ROOT, 'assets', 'skills', 'coda', 'scripts', 'Coda-runtime.mjs');
const CLASSIC_SKILL_ROOT = path.join(REPO_ROOT, 'assets', 'skills', 'coda-classic');
...
function run(cwd, args, options = {}) {
  return spawnSync(process.execPath, [RUNTIME, ...args], {
    ...
    env: {
      ...process.env,
      CODA_CLASSIC_SKILL_ROOT: CLASSIC_SKILL_ROOT,
      ...options.env,
    },
  });
}
...
function state(cwd, ...args) {
  ...
  if (args[0] === 'set' && args[2] === 'phase') {
    ...
    // benchmark scenarios.
    options.env = { ...process.env, CODA_FORCE_PHASE: '1' };
  }
...
async function readState(changeDir) {
  const yamlState = parse(await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8'));
  try {
    const runJson = JSON.parse(await fs.readFile(path.join(changeDir, '.Coda', 'run-state.json'), 'utf8'));
...
async function profileScenario(workspace, profile) {
  ...
  const firstBytes = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
  const second = run(directory, ['hook-guard'], { input: hookInput('src/index.ts') });
  const secondBytes = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
...
async function retryFixScenario(workspace) {
  ...
  return result(
    ...
    startedAt,
    {
      ...
      migrationSuccess: migrated.classic_migration === 1 && migrated.skill === 'Coda-classic',
...
async function handoffResumeScenario(workspace) {
  ...
  const recoveredBytes = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
  const repeated = run(directory, ['handoff', name, 'design', '--write']);
  const repeatedBytes = await fs.readFile(path.join(changeDir, '.Coda.yaml'), 'utf8');
...
async function archiveRecoveryScenario(workspace) {
  ...
  const trajectory = (await fs.readFile(path.join(archiveDir, recovered.trajectory_ref), 'utf8'))
    ...
    .filter(Boolean)
    .map((line) => JSON.parse(line));
  const recoveredBytes = await fs.readFile(path.join(archiveDir, '.Coda.yaml'), 'utf8');
  const repeated = run(directory, ['archive', name], {
    env: { CODA_OPENSPEC: openspec },
  });
  const repeatedBytes = await fs.readFile(path.join(archiveDir, '.Coda.yaml'), 'utf8');
  const recoveries = trajectory.filter(
...
async function malformedScenario(workspace) {
  ...
  state(directory, 'init', name, 'full');
  const changeDir = path.join(directory, 'openspec', 'changes', name);
  const stateFile = path.join(changeDir, '.Coda.yaml');
...
export async function runClassicBaselineBenchmark(options = {}) {
  ...
  const workspace = options.workspace
    ...
    : await fs.mkdtemp(path.join(os.tmpdir(), 'coda-classic-benchmark-'));
