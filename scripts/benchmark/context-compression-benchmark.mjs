...
export async function runBenchmark(options = {}) {
  ...
  const root = path.join(config.workspace, '.Coda', 'benchmark', 'context-compression');
...
async function generateHandoff(cwd) {
  ...
  if (!bash) {
    throw new Error('Bash or Git Bash is required to generate Coda handoff context');
  }
...
async function runCodexMode({ codexCommand, model, cwd, mode }) {
  const contextFile =
    mode === 'beta'
      ? `openspec/changes/${CHANGE_NAME}/.Coda/handoff/spec-context.md`
      : `openspec/changes/${CHANGE_NAME}/.Coda/handoff/design-context.md`;
...
function buildPrompt(mode, contextFile, contextText) {
  return [
    'You are running a local Coda context-compression benchmark.',
    `Mode: ${mode}`,
    `Context source: ${contextFile}`,
    'Use only the inline Coda handoff context below. Do not call tools and do not inspect files.',
