...
`/Coda-any` 的创作成果应由平台原生 subagent 分工产出，再交给主会话组装和后端 CLI 记录状态。
...
主会话负责保留全局上下文、汇总成果、调用 `Coda bundle` 和 `Coda publish`，subagent 只产出可审查草稿。
...
1. 脚本作者 subagent：`Coda-any/reference/subagents/script-author.md`
2. reference 作者 subagent：`Coda-any/reference/subagents/reference-author.md`
3. workflow entry 作者 subagent：`Coda-any/reference/subagents/workflow-entry-author.md`
4. Skill 核心作者 subagent：`Coda-any/reference/subagents/skill-core-author.md`
5. 停顿点作者 subagent：`Coda-any/reference/subagents/pause-points-author.md`
6. Skill 审查 subagent：`Coda-any/reference/subagents/skill-reviewer.md`
...
上面的角色 brief 顺序只是创作 DAG 的线性展开，并非要求严格串行。权威 DAG 在 `reference/authoring-protocol.json` 与 `Coda bundle authoring-plan <name> --depth quick|full --json`：
...
- 每个 lane 产出在下一个依赖 wave 开始前，必须经 `Coda bundle authoring-record <name> --lane <id> --file <out.json> --json` 校验并记录。
...
- `/Coda` 定制场景下的受保护边界：`open / design / build / verify / archive`、`.Coda.yaml`、decision point、verify-result-transition、archive-delta-sync。
...
- subagent 不能调用 `Coda bundle`、`Coda publish`、`Coda skill`，也不能执行候选 Skill 的脚本。
