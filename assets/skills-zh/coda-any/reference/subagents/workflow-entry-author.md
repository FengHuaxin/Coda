...
生成器把 entry SKILL.md 组装成确定性 **Auto 区**（frontmatter、Workflow Nodes 路由表、Skill Bindings、Guardrails And Evidence、Runtime And Recovery）+ 由你编写的 **Authored 区**（`## Decision Core`）。**你只写 Decision Core 正文，不写整个文件**。主会话通过 `Coda bundle authoring-record <name> --lane workflow-entry --file <out.json>` 记录；artifact `SKILL.md` 的 `content` 即 Decision Core 正文。

质量标尺：`Coda/SKILL.md` 的 Decision Core（完整 entry Decision Core 范例见 `reference/authored-zone-example.md`）。写 agent 可读的决策规则——机械路由已由 Auto 区的 `workflow-state.mjs next` 处理，所以聚焦判断：

- **语义化当前节点检测** — 如何判断用户在哪个 Node，而非只跑脚本。建模 Coda 的 Step 0（从用户消息检测意图，检查 Node 顺序，处理"属于前序/后序 Node"的冲突）+ Step 1（读状态，文件优先于过期状态）。
...
- `/Coda` 定制场景下必须保留的 open / design / build / verify / archive 边界。
...
- 对 `/Coda` 定制，entry 必须列出必调槽位 Skill，但只能作为阶段内义务说明，不能变成 entry 立即执行清单。
- 用户停顿点、恢复路径和参考文件清楚可见。
- 对 `/Coda` 定制，说明保留 open / design / build / verify / archive 主路径和阶段守卫。
