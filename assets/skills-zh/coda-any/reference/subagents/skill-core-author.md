...
编写 internal Node Skill 的用户可见核心内容。entry Skill 由 workflow entry 作者单独负责。目标是产出 Coda-like 的多 Node workflow，
不得把来源 Skill 简单串联，也不复制粘贴原 Skill 全文。

必须覆盖：

- 每个 internal Node Skill
- `Coda/skill.yaml` 中的阶段调用语义
- 每个 `node-skill:<skill-name>` claim

## 创作区（你写什么）

生成器把每个 Node SKILL.md 组装成确定性 **Auto 区**（frontmatter、Node Goal、Entry Check、Skill Implementation、Required Skill Calls、Output Schemas、Evidence Record、Guardrails、Exit Check、Recovery）+ 由你编写的 **Authored 区**（`## Guidance`）。**你只写 Guidance 正文，不写整个文件**。主会话通过 `Coda bundle authoring-record <name> --lane skill-core --file <out.json>` 记录你的产出；artifact `../<node-skill>/SKILL.md` 的 `content` 即 Guidance 正文。

质量标尺：真实的 Coda 阶段 skill（如 `Coda-build/SKILL.md`）；完整 substance 节点 Guidance 范例见 `reference/authored-zone-example.md`。写决策内容，不写套话。在 Guidance 内用 `###` 子标题（嵌套在 `## Guidance` 之下）：
...
- **delegates** 节点（Coda-five-phase-overlay，委托给已安装的富 Skill）：短 Guidance 注即可——富内容由被委托 Skill 承载，不要复制。
...
- 对 `/Coda` 定制，保留 `open / design / build / verify / archive` 与 `.Coda.yaml` 语义。
- 对任意 Skill 组合，整理为 Coda-like 多 Node workflow。
