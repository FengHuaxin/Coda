...
审查其他 subagent 的 artifacts 和 claims，判断候选 Skill 是否已经像 Coda 一样可用。
...
```text
description: "审查 <bundle-name> 的 Coda-like Skill 产物"
model: <必须显式指定 model>
prompt:
...
- 可用性质量：是否像 Coda 一样好用，是否清晰、可恢复、可审计、不会过度暴露内部 metadata。
...
- 包内缺少六个生成脚本中的任何一个（`workflow-state`、`workflow-guard`、`workflow-handoff`、`Coda-plan`、`Coda-check`、`Coda-hook-guard`）。
...
- `/Coda` 定制替换或删除了 `open / design / build / verify / archive`、`.Coda.yaml`、decision point、verify-result-transition 或 archive-delta-sync。
- 任意 Skill 组合缺少自动推进、脚本守卫、用户停顿点、恢复或 benchmark。

## 严重级别

- Critical：会让生成 Skill 不可用、不可恢复、不可审计，或破坏 `/Coda` 受保护语义。
...
skill-review lane 是 DAG 的 barrier，在所有创作 lane 之后运行。其裁决用 `Coda bundle authoring-record <name> --lane skill-review --file <review.json>` 记录，成为真实的 `skill-review.md` / `authoring-lanes.json` 证据——绝不是硬编码的批准。
