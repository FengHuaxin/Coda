...
设计候选 Skill 的脚本契约，而不是复制 Coda Classic 的脚本。脚本必须根据当前 workflow protocol、
...
factory 还会从同一份 `workflow-protocol.json` 确定性生成 `scripts/Coda-plan.mjs`、`scripts/Coda-check.mjs`、`scripts/Coda-hook-guard.mjs`。不要重复设计这三个脚本；你的契约只针对 skill-core 作者与 entry 作者引用的 `workflow-*.mjs`。
...
prompt:
  ...
  不要猜测或自行补全缺失协议。
  不要调用 Coda bundle、Coda publish、Coda skill，也不要执行候选 Skill 的脚本。
...
退出检查必须来自当前 workflow protocol 和组合 Skill 的目标，不得照搬 Coda Classic 脚本。
