---
name: Coda-classic
description: "Use when Coda 内部需要恢复、迁移或推进 full、hotfix、tweak 经典工作流 Run；不得用于用户直接调用。"
---

# Coda Classic

`Coda-classic` 是 Coda Engine 使用的内部兼容 Skill。它把既有
full、hotfix、tweak 流程映射为稳定步骤，但不替代用户面对的 `/Coda*`
命令。

## 调用边界

- 不得作为用户命令直接调用。
- 用户仍调用 `/Coda`、`/Coda-open`、`/Coda-design`、`/Coda-build`、
  `/Coda-verify`、`/Coda-archive`、`/Coda-hotfix` 或 `/Coda-tweak`。
- 进入本 Skill 前必须先由 `ensureClassicRun()` 完成旧状态迁移和双投影校验。
- 当前步骤必须由 Classic Resolver 根据 `.Coda.yaml` 与结构化 evidence
  计算，不得根据对话历史猜测。
...
3. action 指向公开 Coda Skill 时，完整遵守该 Skill 的用户确认点和退出条件。
