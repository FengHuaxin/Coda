...
规范路径：`Coda/reference/context-recovery.md`

本协议由所有可能触发上下文压缩的 Coda 子 skill 共享。当 agent 怀疑发生上下文压缩（之前对话被摘要、找不到之前讨论的内容）时，按本协议恢复。
...
2. 重新阅读 `Coda/reference/subagent-dispatch.md` 获取 Coda 专属扩展
3. 读取 `openspec/changes/<name>/.Coda/subagent-progress.md`，恢复当前 task 或 final review、实现提交、RED/GREEN 证据、已通过审查、未解决反馈和审查-修复轮次
4. 禁止在主会话中直接执行 task
