---
name: Coda-open
description: "Coda Phase 1: Open. Invoke with /Coda-open. Explore ideas through OpenSpec, confirm requirements clarification, then create change structure (proposal + design + tasks)."
---
...
When splitting is recommended, must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user's choice.
...
Every accepted split item must be created as an independent change through `/Coda-open`, not by calling `/opsx:new` directly. `/Coda-open` creates both OpenSpec artifacts and `.Coda.yaml`, ensuring each change enters the Coda state machine.

Must not create proposal.md, design.md, or tasks.md before the user completes the PRD split choice. If the user chooses to create multiple changes, the current `/Coda-open` invocation only completes split confirmation and coordination, then enters `/Coda-open` for each split item in the user-confirmed order.
...
In batch split mode, a single split item must not auto-advance to `/Coda-design` after completing the open phase. After splitting is complete, must pause and ask the user which change to start; after the user chooses, advance only that change into `/Coda-design`, while other changes remain active and can be resumed later through `/Coda`.

Minimal resume rule: do not add a dedicated batch state file. On resume, first check already-created active changes; split items that already exist and contain `.Coda.yaml` must not be created again, while uncreated split items continue through `/Coda-open` according to the user-confirmed split list. If the confirmed split list cannot be recovered from the conversation, must ask the user to confirm the split list again before continuing.

### 1b. Requirements Clarification Completion Confirmation (Blocking Point)

Before creating OpenSpec artifacts, must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to confirm requirements clarification is complete.
...
Before creating the change directory (`openspec new change`), must follow the `Coda/reference/decision-point.md` protocol to pause and let the user decide the change name. Must not auto-generate or silently infer the change name.
...
Must not run `openspec new change` or create `.Coda.yaml` before the user confirms the final change name. If the chosen/converted name collides with an existing change, report the collision and ask the user to choose another name.

### 2. Create Change Structure + Initialize State

**Immediately execute:** Use the Skill tool to load the `openspec-new-change` skill. Skipping this step is prohibited.

Full `/Coda` workflow must not use the Skill tool to load the `openspec-propose` skill by default; only load it when the user explicitly requests generating the proposal and artifacts in one pass.
...
node "$Coda_STATE" init <name> full
```
...
**Idempotency**: All open phase operations can be safely re-executed. If `.Coda.yaml` is already at `phase: open` and all three artifact files exist, skip completed steps and continue from the first missing step.
...
After the three documents are created and content completeness check passes, **must follow the `Coda/reference/decision-point.md` protocol to pause and wait for user confirmation**. Must not execute phase guard or auto-transition before user confirmation.
...
- **Phase guard**: Run `node "$Coda_GUARD" <change-name> open --apply`; after all PASS, auto-transitions to next phase

Must use `--apply` before exit, otherwise `.Coda.yaml` remains at `phase: open` and the next phase entry check will fail.
