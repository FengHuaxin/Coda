---
name: Coda-build
description: "Coda Phase 3: Plan and Build. Invoke with /Coda-build. Create plans and select execution method (subagent or direct) for implementation."
---

# Coda Phase 3: Plan and Build (Build)
...
node "$Coda_STATE" check <name> build
```

Proceed to Step 1 after verification passes. The script outputs specific failure reasons when verification fails.

**Idempotency**: All build phase operations can be safely re-executed. Read `.Coda.yaml` `phase` field to confirm still in build, read plan header `base-ref`, then use `grep -n '\- \[ \]' tasks.md | head -1` to find the first unchecked task. Already-committed tasks must not be re-committed.
...
```bash
node "$Coda_STATE" set <name> plan docs/superpowers/plans/YYYY-MM-DD-feature.md
```
...
| B | Pause to switch model | Record `build_pause: plan-ready`, stop this `/Coda-build` invocation, and allow the user to resume later from `/Coda` or `/Coda-build` |

This is a user decision point. **Must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to explicitly choose**. Must not auto-continue and must not write the pause into `build_mode`.

When the user chooses to continue:

```bash
node "$Coda_STATE" set <name> build_pause null
```

When the user chooses to pause:

```bash
node "$Coda_STATE" set <name> build_pause plan-ready
```
...
This is a user decision point. **Must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to explicitly choose isolation method, execution method, and TDD mode**. Must not choose `branch` or `worktree` based on recommendation rules, and must not choose the execution method or TDD mode based on recommendation rules. Recommendation rules are for suggestion only, not a substitute for user confirmation.

After user selection, update `isolation`, execution method, and TDD mode fields:

```bash
node "$Coda_STATE" set <name> isolation <branch|worktree>
```

- If the user chooses `executing-plans`: run `node "$Coda_STATE" set <name> subagent_dispatch null`, then run `node "$Coda_STATE" set <name> build_mode executing-plans`
- If the user chooses `subagent-driven-development`: first confirm the current platform has real background subagent / Task / multi-agent dispatch capability; after confirming, run `node "$Coda_STATE" set <name> subagent_dispatch confirmed`, then run `node "$Coda_STATE" set <name> build_mode subagent-driven-development`
...
Run `node "$Coda_STATE" set <name> tdd_mode <tdd|direct>`
...
Run `node "$Coda_STATE" set <name> review_mode <off|standard|thorough>`

`isolation` is a script-enforced hard constraint. Full workflow init may temporarily leave it as `null`, but only before this step. If it remains `null`, both the `build → verify` guard and `Coda-state transition build-complete` will fail.

`subagent_dispatch` is a script-enforced hard constraint. `build_mode: subagent-driven-development` requires `subagent_dispatch: confirmed` before leaving the build phase, otherwise both `Coda-guard.mjs build --apply` and `Coda-state transition build-complete` will fail.

`tdd_mode` is a script-enforced hard constraint. Full workflow must have `tdd_mode` selected as `tdd` or `direct` before leaving the build phase, otherwise both `Coda-guard.mjs build --apply` and `Coda-state transition build-complete` will fail.

`review_mode` is a script-enforced hard constraint. Full workflow must have `review_mode` selected as `off`, `standard`, or `thorough` before leaving the build phase, otherwise both `Coda-guard.mjs build --apply` and `Coda-state transition build-complete` will fail. Legacy state files without this field follow a compat path, but should be backfilled on recovery.
...
```bash
node "$Coda_STATE" set <name> direct_override true
node "$Coda_STATE" set <name> build_mode direct
```
...
- **branch**: Recommend a branch name based on the workflow type and current date, then let the user confirm or input a custom name. This is a user decision point — **must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm or override the branch name**. Must not skip this step and create the branch directly.
  ...
  - Read the `workflow` field from `.Coda.yaml` to determine the prefix
...
- `build_mode: subagent-driven-development`: The main session only coordinates and must not write implementation code directly. **Immediately execute:** Use the Skill tool to load the Superpowers `subagent-driven-development` skill. After the skill loads, read `Coda/reference/subagent-dispatch.md` for Coda-specific extensions (real background dispatch, task isolation, checkoff verification, TDD constraints, continuous execution, context recovery) and apply them alongside the skill's workflow. If they conflict, the more specific Coda extensions take precedence.
- If the current platform has no real background agent dispatch capability, must pause and wait for the user to choose main window execution instead. After the user chooses, must run `node "$Coda_STATE" set <name> build_mode executing-plans`, then follow the `build_mode: executing-plans` branch to load the Superpowers `executing-plans` skill. Must not continue executing tasks before the user explicitly chooses.
...
- `build_mode: subagent-driven-development`: The main session does not load the TDD skill. TDD constraints and evidence thresholds are defined in `Coda/reference/subagent-dispatch.md`; every background implementer and fix agent must use the Skill tool to load the Superpowers `test-driven-development` skill and follow the Coda-injected TDD hard constraint.
...
- the `requesting-code-review` skill must be loaded before `node "$Coda_GUARD" <change-name> build --apply`
...
| Large | Brand-new capability requirements | **Must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm the split**; after user confirms, create independent change through `/Coda-open` |

**50% Threshold Determination**: Using initial task count in tasks.md as baseline, if new tasks exceed half of that total, it's considered outside original plan scope, **must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to decide whether to split into a new change**.

When creating an independent change, must invoke `/Coda-open`, not `/opsx:new` directly. `/Coda-open` creates both OpenSpec artifacts and `.Coda.yaml`, preventing the new change from leaving the Coda state machine.

**User choices must include**:
- "Split into new change" — create independent change via `/Coda-open`
...
- **Context compression recovery**: Follow `Coda/reference/context-recovery.md` with phase set to `build`.
- **User manual-change resume**: handle uncommitted changes through `Coda/reference/dirty-worktree.md`. That protocol defines checks, attribution, and prohibitions. Build-specific handling:
...
- **Phase guard**: Run `node "$Coda_GUARD" <change-name> build --apply`; after all PASS, state advances to `phase: verify`
...
Configuration can live in the change `.Coda.yaml`, or in repo-root `.Coda.yaml` / `Coda.yaml` / `.Coda.yml` / `Coda.yml`.
