---
name: coda-verify
description: "Coda Phase 4: Verify and Close. Invoke with /coda-verify. Verify implementation matches design, handle development branch."
---

# Coda Phase 4: Verify and Close (Verify)
...
node "$Coda_STATE" check <change-name> verify
```
...
```bash
node "$Coda_STATE" scale <change-name>
```
...
Before verification begins, handle uncommitted changes through `coda/reference/dirty-worktree.md` protocol. Verify phase special handling:
...
```bash
PLAN=$(node "$Coda_STATE" get <change-name> plan)
...
**Override mechanism**: If the agent or user believes the automated assessment is inappropriate, override at any time with `node "$Coda_STATE" set <change-name> verify_mode <light|full>`.

### 1b. Verification Failure Decision (Blocking Point)

When verification does not pass, **must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to decide whether to fix or accept the deviation**. Must not automatically run `node "$Coda_STATE" transition <change-name> verify-fail`, nor automatically invoke `/coda-build`.
...
- **Fix all**: Run `node "$Coda_STATE" transition <change-name> verify-fail`, then invoke `/Coda-build` to fix
...
```bash
RECORDED_HASH=$(node "$Coda_STATE" get <change-name> handoff_hash)
CURRENT_HASH=$(node "$Coda_HANDOFF" <change-name> --hash-only 2>/dev/null || echo "")
```
...
**When not passing**: Report failures, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/coda-build` to fix:
...
When verification does not pass: report missing items, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/coda-build` to supplement:
...
- If check item 6 finds contradictions (delta spec has content but design doc does not reflect it), **must use the current platform's available user input/confirmation mechanism as a single-select question to pause and wait for the user to choose the handling method**; must not select automatically. Options:
  ...
  - Option B: After user selects B, run `node "$Coda_STATE" transition <change-name> verify-fail`, then invoke `/coda-build`; `/coda-build`'s Spec Incremental Update rules will load the Superpowers `brainstorming` skill to update Design Doc + delta spec
...
Verification report must be saved to disk and recorded in `.coda.yaml`; after branch handling completes, state fields must also be written. Do not manually set `verify_result: pass`; use guard for auto-transition.
...
node "$Coda_STATE" set <change-name> verification_report docs/superpowers/reports/YYYY-MM-DD-<change-name>-verify.md
node "$Coda_STATE" set <change-name> branch_status handled
```
...
- Branch handled
- `verification_report` in `.coda.yaml` points to an existing verification report file
- `branch_status: handled` in `.coda.yaml`
- **Phase guard**: Run `node "$Coda_GUARD" <change-name> verify --apply`; after all PASS, auto-transitions to `phase: archive` through `coda-state transition verify-pass`

After both verification and branch handling are complete, run guard for auto-transition:

```bash
node "$Coda_GUARD" <change-name> verify --apply
```
...
Follow `coda/reference/context-recovery.md` with phase set to `verify`.
...
Note: after `coda-archive` starts, it must first execute the final archive confirmation blocking point and wait for the user to explicitly choose "Confirm archive" before running the archive script. Must not automatically archive just because verification passed.
