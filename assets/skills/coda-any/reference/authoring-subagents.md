...
`/coda-any` authoring outputs should be drafted by platform-native subagents, then assembled and recorded by the main session through the backend CLI. Claude Code, Codex, Gemini, Copilot, and other platforms expose subagents differently; this reference defines responsibilities, inputs, and outputs, not a provider-specific dispatch API.

Read this overview first, then give only the matching role brief to each subagent. Do not merge the six role briefs into one large prompt. The main session keeps global context, aggregates outputs, and calls `coda bundle` and `coda publish`; subagents only produce reviewable drafts.
...
1. Script author subagent: `coda-any/reference/subagents/script-author.md`
2. Reference author subagent: `coda-any/reference/subagents/reference-author.md`
3. Workflow entry author subagent: `coda-any/reference/subagents/workflow-entry-author.md`
4. Skill core author subagent: `coda-any/reference/subagents/skill-core-author.md`
5. Pause point author subagent: `coda-any/reference/subagents/pause-points-author.md`
6. Skill review subagent: `coda-any/reference/subagents/skill-reviewer.md`
...
The Role Briefs order above is a linearization of the authoring DAG, not a mandate to run strictly sequential. The authoritative DAG lives in `reference/authoring-protocol.json` and `coda bundle authoring-plan <name> --depth quick|full --json`:
...
- Every lane output is validated and recorded via `coda bundle authoring-record <name> --lane <id> --file <out.json> --json` before the next dependent wave begins.
...
- For customize existing Coda Skills, the protected boundary: `open / design / build / verify / archive`, `.coda.yaml`, decision point, verify-result-transition, and archive-delta-sync.
...
- Subagents cannot call `coda bundle`, `coda publish`, or `coda skill`, and cannot execute candidate Skill scripts.
