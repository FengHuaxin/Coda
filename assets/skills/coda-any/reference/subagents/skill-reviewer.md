...
Review the other subagents' artifacts and claims, then decide whether the candidate Skill is usable like Coda. The review must give a clear conclusion: `Review passed` or blocking findings.
...
```text
description: "Review the Coda-like Skill outputs for <bundle-name>"
...
- Skill contract fit: whether the output satisfies the user-confirmed goal, workflow protocol,
  ...
  requirements.
- Usability quality: whether it is as usable as Coda: clear, recoverable, auditable, and not overexposing internal metadata.
...
- Missing any of the six generated scripts (`workflow-state`, `workflow-guard`, `workflow-handoff`, `coda-plan`, `coda-check`, `coda-hook-guard`) in the package.
...
- Customizing existing Coda Skills replaces or removes `open / design / build / verify / archive`, `.coda.yaml`, decision point, verify-result-transition, or archive-delta-sync.
...
- Critical: makes the generated Skill unusable, unrecoverable, unauditable, or breaks protected `/coda` semantics.
...
The skill-review lane is the DAG barrier and runs after every author lane. Its verdict is recorded with `coda bundle authoring-record <name> --lane skill-review --file <review.json>` and becomes the real `skill-review.md` / `authoring-lanes.json` evidence —it is NOT a hardcoded approval.
