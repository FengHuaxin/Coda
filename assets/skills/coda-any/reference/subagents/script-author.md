...
Design the generated Skill's script contract instead of copying Coda Classic scripts. Scripts must use the current workflow protocol, user-selected Node labels, and real composed Skill outputs to define automatic advancement, exit checks, recovery, and evidence recording.
...
The factory also deterministically generates `scripts/coda-plan.mjs`, `scripts/coda-check.mjs`, and `scripts/coda-hook-guard.mjs` from the same `workflow-protocol.json`. Do not duplicate or redesign those three; your contract is for the `workflow-*.mjs` scripts that the Skill core author and entry author reference.
...
- Protected `.coda.yaml` semantics when users customize existing Coda Skills
...
prompt:
  ...
  Do not call coda bundle, coda publish, or coda skill, and do not execute candidate Skill scripts.
...
Exit checks must come from the current workflow protocol and composed Skill goal, not copied Coda Classic scripts.
