---
name: coda-design
description: "Coda Phase 2: Deep Design. Invoke with /coda-design. Produce Design Doc and delta spec through brainstorming."
---

# Coda Phase 2: Deep Design (Design)
...
The script reads the change `.coda.yaml` `context_compression` snapshot, then generates and records the matching handoff package.
...
Beta mode (`context_compression: beta` in project `.coda/config.yaml`, snapshotted into `.coda.yaml` when the change is created) generates:
...
After brainstorming produces a design proposal, **must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to explicitly confirm the design proposal**. Must not create the final Design Doc, write `design_doc`, run design guard, or enter `/coda-build` before user confirmation.
...
`openspec/changes/<name>/.coda/handoff/brainstorm-summary.md` structure:
...
- `openspec/changes/<name>/.coda/handoff/design-context.md` (or `spec-context.md` in beta mode)
- `openspec/changes/<name>/.coda/handoff/design-context.json` (or `spec-context.json` in beta mode)
...
- Design Doc frontmatter contains `coda_change`, `role: technical-design`, `canonical_spec: openspec`
- `handoff_context` and `handoff_hash` written to `.coda.yaml` (enforced by guard)
...
- `design_doc` written to `.coda.yaml`
- **Phase guard**: Run `node "$CODA_GUARD" <change-name> design --apply`; after all PASS, auto-transitions to `phase: build`
...
Follow `coda/reference/context-recovery.md` with phase set to `design`.
