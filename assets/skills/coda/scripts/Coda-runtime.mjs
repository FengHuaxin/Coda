#!/usr/bin/env node
import { createRequire as __codaCreateRequire } from 'module';
const require = __codaCreateRequire(import.meta.url);
...
async function writeRunState(changeDir, state) {
  await fs3.mkdir(path3.join(changeDir, ".Coda"), { recursive: true });
  const file = path3.join(changeDir, RUN_STATE_FILE);
  const temporary = path3.join(changeDir, ".Coda", `run-state.${randomUUID()}.tmp`);
...
var init_state = __esm({
  "domains/engine/state.ts"() {
    ...
    RUN_STATE_FILE = ".Coda/run-state.json";
  }
});

// domains/coda-classic/classic-cli.ts
import { pathToFileURL } from "url";

// domains/coda-classic/classic-archive.ts
import { createHash as createHash3 } from "crypto";
import { spawnSync } from "child_process";
import { promises as fs10 } from "fs";
import path10 from "path";

// domains/coda-classic/classic-paths.ts
...
async function findClassicYamlFile(changeDir) {
  for (const candidate of [".Coda.yaml", ".coda.yaml", "Coda.yaml", "coda.yaml"]) {
    const filePath = path.join(changeDir, candidate);
...
async function resolveClassicChangeDirectory(name) {
  ...
  if (await exists(filesystemPath(archiveRoot))) {
    ...
    for (const entry2 of await fs.readdir(filesystemPath(archiveRoot), { withFileTypes: true })) {
      ...
      if (await exists(path.join(filesystemPath(candidate), ".Coda.yaml")) || await exists(path.join(filesystemPath(candidate), ".coda.yaml"))) {
...
// domains/coda-classic/classic-runtime-run.ts
import { promises as fs9 } from "fs";
import path9 from "path";
import { fileURLToPath } from "url";

// domains/coda-classic/classic-evidence.ts
...
async function collectClassicEvidence(changeDir, projection) {
  ...
  const checkpoint = projection.run ? path2.resolve(changeDir, projection.run.checkpointRef) : path2.join(changeDir, ".coda", "checkpoint.json");
...
// domains/coda-classic/classic-store.ts
var import_yaml = __toESM(require_dist(), 1);
import { randomUUID as randomUUID2 } from "crypto";
import { promises as fs4 } from "fs";
import path4 from "path";

// domains/coda-classic/classic-state.ts
init_state();
...
async function readClassicState(changeDir) {
  const document = await readDocument(path4.join(changeDir, ".coda.yaml"));
  ...
  if (!run && doc.run_id && doc.skill) {
    ...
    if (run) {
      ...
      const file = path4.join(changeDir, ".coda.yaml");
      const temporary = path4.join(changeDir, `.coda.yaml.${randomUUID2()}.tmp`);
...
function startRun(pkg, runId, skillHash) {
  return {
    ...
    iteration: 0,
    pending: null,
    pendingRef: ".coda/pending-action.json",
    trajectoryRef: ".coda/trajectory.jsonl",
    contextRef: ".coda/context.md",
    artifactsRef: ".coda/artifacts.json",
    checkpointRef: ".coda/checkpoint.json",
    status: "running",
    retries: {}
  };
}
...
function narrowSkillDefinition(document, filePath) {
  ...
  assertEnum(document.apiVersion, ["coda/v1alpha1"], filePath, "apiVersion");
...
async function readRuntimeChecks(codaRoot) {
  const checksPath = path6.join(codaRoot, "checks.yaml");
  const evalsPath = path6.join(codaRoot, "evals.yaml");
...
async function loadSkillPackage(root) {
  ...
  const codaRoot = path6.join(packageRoot, "coda");
  await fs6.access(path6.join(packageRoot, "SKILL.md"));
  const skillPath = path6.join(codaRoot, "skill.yaml");
  const guardrailsPath = path6.join(codaRoot, "guardrails.yaml");
  ...
  const runtimeChecks = await readRuntimeChecks(codaRoot);
...
async function createSkillSnapshot(pkg, changeDir) {
  ...
  const snapshotsRoot = path7.resolve(changeDir, ".coda", "skill-snapshots");
...
async function ensureClassicRun(changeDir, options) {
  ...
  const expectedSnapshotDir = path8.join(changeDir, ".coda", "skill-snapshots", expectedHash);
...
async function classicSkillRoot() {
  ...
  const candidates = [
    process.env.CODA_CLASSIC_SKILL_ROOT,
    path9.resolve(runtimeDirectory, "..", "..", "coda-classic"),
    path9.resolve(runtimeDirectory, "..", "..", "assets", "skills", "coda-classic"),
    path9.resolve("assets", "skills", "coda-classic")
  ].filter((candidate) => Boolean(candidate));
  for (const candidate of candidates) {
    if (await directoryExists(candidate)) return candidate;
  }
  throw new Error("Coda Classic internal Skill package is not installed");
}
...
var classicArchiveCommand = async (args) => {
  ...
  try {
    ...
    const openspec = process.env.CODA_OPENSPEC || "openspec";
    output.stderr.push(`=== Coda Archive: ${change} ===`);
    const activeExists = await findClassicYamlFile(activeDir) !== null;
    ...
    if (!changeDir || !await findClassicYamlFile(changeDir)) {
      throw new ArchiveFailure(red(`FATAL: .Coda.yaml not found in ${activeDir}/`));
    }
    if (recoveredArchive) {
    ...
    if (classic.verifyResult !== "pass") {
      throw new ArchiveFailure(
        red(
          `FATAL: verify_result is '${classic.verifyResult}', expected 'pass'. Run coda-verify first.`
        )
      );
    }
        ...
        if (archiveRun.error && archiveRun.error.code === "ENOENT") {
          throw new ArchiveFailure(
            [
              ...
              red("Install OpenSpec or set CODA_OPENSPEC to the openspec executable.")
            ].join("\n")
          );
        }
    ...
    if (dryRun) {
      ...
      output.stderr.push(
        yellow(`  [DRY-RUN] Would set archived: true in ${archiveDir}/.coda.yaml`)
      );
      output.stepsOk += 1;
      output.stepsTotal += 1;
    }
...
// domains/coda-classic/classic-guard.ts
...
// domains/coda-classic/classic-runtime-evals.ts
var STEP_EVIDENCE = {
...
// domains/coda-classic/classic-diagnostics.ts
...
// domains/coda-classic/classic-validate-command.ts
...
var classicValidateCommand = async (args) => {
  ...
  const yamlFile = await findClassicYamlFile(directory) ?? path11.join(directory, ".Coda.yaml");
  const lines = [`[VALIDATE] ${label}/${path11.basename(yamlFile)}`];
  let errors = 0;
  let warnings = 0;
  const fail3 = (message) => {
    errors += 1;
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      fail3(".coda.yaml does not exist");
...
var APPLY_MESSAGE = {
  open: "  [APPLY] .coda.yaml updated: phase=PLACEHOLDER",
  design: "  [APPLY] .coda.yaml updated: phase=build",
  build: "  [APPLY] .coda.yaml updated: phase=verify, verify_result=pending",
  verify: "  [APPLY] .coda.yaml updated: phase=archive, verify_result=pass"
};
...
async function readField(changeDir, field2) {
  const candidates = [".coda.yaml", "Coda.yaml", ".Coda.yaml", "coda.yaml"];
  let file = null;
  for (const candidate of candidates) {
    const candidatePath = path12.join(changeDir, candidate);
    if (await exists4(candidatePath)) {
      file = candidatePath;
      break;
    }
  }
  if (!file) {
    throw new GuardFailure(`ERROR: .Coda.yaml not found in ${changeDir}`);
  }
  const document = (0, import_yaml4.parseDocument)(await fs12.readFile(file, "utf8"), { uniqueKeys: false });
  if (document.errors.length > 0) {
    throw new GuardFailure(`ERROR: Invalid .Coda.yaml: ${document.errors[0].message}`);
  }
  const record = document.toJS();
...
async function projectConfigValue(field2, changeDir) {
  ...
  for (const config of [".coda.yaml", "coda.yaml", ".coda.yml", "coda.yml"]) {
...
async function preflight(changeDir, name) {
  ...
  if (!await findClassicYamlFile(changeDir)) {
    throw new GuardFailure(red2(`FATAL: .Coda.yaml not found in ${changeDir}`));
  }
...
`);
    throw new GuardFailure(red2("FATAL: .Coda.yaml schema validation failed"));
  }
}
...
async function buildPasses(changeDir) {
  if (process.env.CODA_SKIP_BUILD === "1") return { status: 0, output: "" };
...
Next: restore the Superpowers plan file or update .coda.yaml plan before leaving build.`
    );
  }
...
Next: ask the user to choose branch or worktree, create the chosen isolation, then run:
  node "$CODA_STATE" set ${change} isolation <branch|worktree>`
  );
}
...
Next: ask the user to choose an execution mode, then run:
  node "$CODA_STATE" set ${change} build_mode <subagent-driven-development|executing-plans>`
  );
}
...
Next: confirm the current platform has a real background subagent/Task/multi-agent dispatcher, then run:
  node "$CODA_STATE" set ${change} subagent_dispatch confirmed
Or ask the user to switch to executing-plans and run:
  node "$CODA_STATE" set ${change} build_mode executing-plans`
  );
}
...
Next: ask the user to choose TDD enforcement level, then run:
  node "$CODA_STATE" set ${change} tdd_mode <tdd|direct>`
  );
}
...
Next: ask the user to choose review strength, then run:
  node "$CODA_STATE" set ${change} review_mode <off|standard|thorough>`
  );
}
...
Next: create the Design Doc and run: node "$CODA_STATE" set ${change} design_doc <path>`
  );
}
async function designHandoffContextValid(changeDir, change) {
  ...
  if (!context || context === "null") {
    return fail(
      `handoff_context is missing from .coda.yaml
Next: run node "$CODA_HANDOFF" ${change} design --write before invoking Superpowers.`
    );
  }
  if (!await nonempty(context)) {
    return fail(
      `handoff_context does not point to a non-empty file: ${context}
Next: regenerate the design handoff with coda-handoff.mjs.`
    );
  }
...
Next: rerun coda-handoff.mjs so Superpowers receives the current OpenSpec context.`
    );
  }
...
async function designHandoffMarkdownTraceable(changeDir) {
  const context = await readField(changeDir, "handoff_context");
  if (!context || context === "null") return fail("handoff_context is missing from .coda.yaml");
  const markdown = `${context.replace(/\.json$/u, "")}.md`;
  if (!await nonempty(markdown))
    return fail(`design handoff markdown is missing or empty: ${markdown}`);
  const source = await fs12.readFile(markdown, "utf8");
  const problems = [];
  if (!/^Generated-by: Coda-handoff\.mjs$/mu.test(source)) {
  ...
  if (designDoc && designDoc !== "null") {
    blocked2 = await runChecks(output, [
      ...
      check("Design Doc frontmatter links current change", async () => {
        ...
        return await designDocFrontmatterHas(designDoc, "Coda_change", change) ? pass() : fail("");
      }),
    ...
    output.stderr.push(
      yellow2("  [WARN] No design_doc recorded in .coda.yaml (optional for hotfix/tweak)")
    );
  }
  return blocked2;
}
...
// domains/coda-classic/classic-handoff.ts
...
async function readField2(changeDir, field2) {
  const file = await findClassicYamlFile(changeDir);
  if (!file) {
    throw new HandoffFailure(`ERROR: .Coda.yaml not found in ${changeDir}`);
  }
  const document = (0, import_yaml5.parseDocument)(await fs13.readFile(file, "utf8"), { uniqueKeys: false });
  if (document.errors.length > 0) {
    throw new HandoffFailure(`ERROR: Invalid .Coda.yaml: ${document.errors[0].message}`);
  }
  const record = document.toJS();
  const value = record[field2];
  if (value === null || value === void 0) return "";
...
var classicHandoffCommand = async (args) => {
  ...
  try {
    ...
    if (phase !== "design" || mode !== "--write") {
      throw new HandoffFailure(
        red3("Usage: Coda-handoff.mjs <change-name> design --write [--full]")
      );
    }
    let handoffMode;
    ...
    const yaml = await findClassicYamlFile(changeDir);
    if (!yaml) {
      throw new HandoffFailure(red3(`ERROR: .Coda.yaml not found in ${changeDir}`));
    }
    ...
    const handoffDir = `${changeDir}/.Coda/handoff`;
...
// domains/coda-classic/classic-hook-guard.ts
...
function allowed(message) {
  return result(0, `[Coda-HOOK] allowed: ${message}`);
}
...
function parseProjectRoot(args) {
  ...
  return path13.resolve(value && !value.startsWith("--") ? value : process.cwd());
}
function relativeToProjectRoot(target, projectRoot) {
  const relative = normalized(path13.relative(projectRoot, target));
  if (relative === "") return "";
  if (relative.startsWith("../") || relative === ".." || path13.isAbsolute(relative)) return null;
  return relative;
}
async function projectRelative(target, projectRoot) {
  const rawCandidate = path13.isAbsolute(target) ? target : path13.resolve(process.cwd(), target);
  ...
  try {
    const parent = await fs14.realpath(path13.dirname(rawCandidate));
    const physicalCandidate = path13.join(parent, path13.basename(rawCandidate));
    ...
    if (!path13.isAbsolute(target)) return normalized(target).replace(/^\.\//u, "");
  }
...
async function activeChanges(projectRoot) {
  const changesDir = path13.join(projectRoot, "openspec", "changes");
  ...
  for (const entry2 of (await fs14.readdir(changesDir, { withFileTypes: true })).sort(
    ...
    if (!existsSync2(path13.join(changeDir, ".Coda.yaml"))) continue;
...
async function governingChange(relativePath2, projectRoot) {
  ...
  if (relativePath2.startsWith(prefix)) {
    ...
    if (name && name !== "archive") {
      const changeDir = path13.join(projectRoot, "openspec", "changes", name);
      const stateFile2 = path13.join(changeDir, ".Coda.yaml");
...
function isCodaConfig(relativePath2) {
  return relativePath2 === ".Coda.yaml" || relativePath2 === "Coda.yaml" || relativePath2 === ".Coda.yml" || relativePath2 === "Coda.yml" || relativePath2.startsWith(".Coda/") || relativePath2.includes("/.Coda/");
}
function openSpecAllowed(relativePath2, phase) {
  if (!relativePath2.startsWith("openspec/")) return null;
  const stateFile2 = relativePath2.endsWith("/.Coda.yaml") || relativePath2.endsWith("/.openspec.yaml");
  const proposal = relativePath2.endsWith("/proposal.md") || relativePath2.endsWith("/design.md") || relativePath2.endsWith("/tasks.md");
  const handoff = relativePath2.includes("/.Coda/");
...
function blocked(relativePath2, phase) {
  const guidance = phase === "open" ? [
    ...
    "  NEXT: finish the Design Doc, then run Coda-guard design --apply to enter build"
  ] : [
  ...
  return result(
    ...
    [
      "",
      "╔══════════════════════════════════════════╗",
      "║     Coda PHASE GUARD — WRITE BLOCKED    ║",
      "╚══════════════════════════════════════════╝",
      "",
      `  Current phase: ${phase}`,
...
var classicHookGuardCommand = async (args) => {
  ...
  try {
    ...
    return result(
      2,
      `[Coda-HOOK] blocked: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  if (!governing) return allowed("no active Coda change");
  if (governing.archived) return allowed(`${relativePath2} (own change archived)`);
  if (isCodaConfig(relativePath2)) {
    return allowed(`${relativePath2} (whitelist: Coda config)`);
  }
...
// domains/coda-classic/classic-state-command.ts
...
async function readDocument2(file) {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      fail2(
        `ERROR: .Coda.yaml not found at ${path14.relative(process.cwd(), file).replaceAll("\\", "/")}`
      );
    }
    throw error;
  }
  const document = (0, import_yaml6.parseDocument)(source, { uniqueKeys: false });
  if (document.errors.length > 0) fail2(`ERROR: Invalid .Coda.yaml: ${document.errors[0].message}`);
  return document;
}
...
async function projectConfigValue2(field2) {
  const file = path14.resolve(".coda", "config.yaml");
...
async function contextCompression() {
  const value = process.env.CODA_CONTEXT_COMPRESSION ?? await projectConfigValue2("context_compression") ?? "off";
...
async function autoTransition() {
  const value = process.env.CODA_AUTO_TRANSITION ?? await projectConfigValue2("auto_transition") ?? "true";
...
async function reviewModeDefault() {
  const value = process.env.CODA_REVIEW_MODE ?? await projectConfigValue2("review_mode") ?? "standard";
...
async function stateFile(name) {
  ...
  const file = await findClassicYamlFile(change.directory) ?? path14.join(change.directory, ".Coda.yaml");
  return {
    ...change,
    file
  };
...
async function setField(output, name, field2, value, options = {}) {
  ...
  if (field2 === "phase" && !options.internal && process.env.CODA_FORCE_PHASE !== "1") {
    fail2(
      "ERROR: Setting 'phase' directly is not allowed; it bypasses state machine evidence checks.\n  Use: coda-state.mjs transition <change-name> <event>\n  Repair-only escape hatch: CODA_FORCE_PHASE=1 coda-state.mjs set <change-name> phase <value>"
    );
  }
  ...
  if (field2 === "phase" && !options.internal) {
    output.stderr.push(
      ...
      yellow4("  Consider using: coda-state.mjs transition <change-name> <event>")
    );
  }
...
async function init(output, name, workflow) {
  ...
  if (await exists6(file)) fail2(`ERROR: .Coda.yaml already exists at ${label}/.Coda.yaml`);
  ...
  await atomicWrite2(file, document.toString());
  output.stderr.push(green4(`Initialized: ${label}/.Coda.yaml (workflow=${workflow})`));
}
...
async function transition(output, name, event) {
  ...
  if (event === "open-complete") {
    ...
    if (!report || !await exists6(path14.resolve(report))) {
      fail2(
...
async function next(output, name) {
  ...
  if (!await exists6(file)) fail2(`ERROR: .Coda.yaml not found at ${label}/.Coda.yaml`);
  ...
  const skill = phase === "open" ? "Coda-open" : phase === "design" ? "Coda-design" : phase === "verify" ? "Coda-verify" : phase === "archive" ? "Coda-archive" : phase === "build" ? workflow === "hotfix" ? "Coda-hotfix" : workflow === "tweak" ? "Coda-tweak" : "Coda-build" : null;
  if (!skill) {
...
async function check2(output, name, phase) {
  ...
  output.stdout.push(`=== Entry Check: coda-${phase} ===`);
  if (!await exists6(file)) fail2(`ERROR: .Coda.yaml not found at ${label}/.Coda.yaml`);
  let blocked2 = false;
  ...
  if (phase === "design") {
    ...
    const designDoc = await readField3(name, "design_doc");
    if (workflow === "full") {
      (designDoc && designDoc !== "null" && await exists6(path14.resolve(designDoc)) ? pass2 : reject)(`design_doc=${designDoc} (expected: non-null and file exists)`);
    } else {
...
async function recoverOpen(output, directory) {
  ...
  output.stdout.push(
    "",
    complete === 3 ? "Recovery action: All artifacts complete. Run /Coda-open user confirmation, then guard to transition." : complete === 0 ? "Recovery action: No artifacts created yet. Start from /Coda-open Step 1 (explore and clarify)." : "Recovery action: Some artifacts incomplete. Resume /Coda-open from the first missing artifact."
  );
}
async function recoverDesign(output, name, directory) {
  ...
  if (design && design !== "null" && await exists6(path14.resolve(design))) {
    output.stdout.push(
      "Recovery action: Design Doc already created and linked. Run guard to transition to build."
    );
  } else if (handoff && handoff !== "null" && await exists6(path14.resolve(handoff))) {
    output.stdout.push(
...
async function recoverBuild(output, name, directory) {
  ...
  let planTotal = 0;
  let planDone = 0;
  if (plan && plan !== "null" && await exists6(path14.resolve(plan))) {
    const planLines = (await fs15.readFile(path14.resolve(plan), "utf8")).split(/\r?\n/u);
...
async function recoverVerify(output, name) {
  ...
  output.stdout.push(
    ...
    "",
    result2 === "pass" && branch === "handled" ? "Recovery action: Verification complete. Run guard to transition to archive." : result2 === "fail" ? "Recovery action: Verification failed and rolled back to build. Resume from /Coda-build." : "Recovery action: Verification not yet started or in progress. Run scale assessment then verify."
  );
}
async function recoverArchive(output, name) {
  output.stdout.push(
    ...
    "",
    "Recovery action: Run /Coda-archive to complete archiving."
  );
}
...
async function scale(output, name) {
  ...
  if (await exists6(specs)) {
    for (const entry2 of await fs15.readdir(specs)) {
      if (await exists6(path14.join(specs, entry2, "spec.md"))) deltaSpecs += 1;
    }
  }
  const plan = await readField3(name, "plan");
  let baseRef = "";
  if (plan && plan !== "null" && await exists6(path14.resolve(plan))) {
    const match = (await fs15.readFile(path14.resolve(plan), "utf8")).match(/^base-ref:\s*(.+)$/mu);
...
var classicStateCommand = async (args) => {
  ...
  try {
    ...
    if (subcommand === "init") {
      required(rest, 2, "Usage: coda-state.mjs init <change-name> <workflow>");
      await init(output, rest[0], rest[1]);
    } else if (subcommand === "get") {
      required(rest, 2, "Usage: coda-state.mjs get <change-name> <field>");
      validateChangeName4(rest[0]);
      output.stdout.push(await readField3(rest[0], rest[1]));
    } else if (subcommand === "set") {
      required(rest, 3, "Usage: coda-state.mjs set <change-name> <field> <value>");
      validateChangeName4(rest[0]);
      await setField(output, rest[0], rest[1], rest[2]);
    } else if (subcommand === "transition") {
      required(rest, 2, "Usage: coda-state.mjs transition <change-name> <event>");
      await transition(output, rest[0], rest[1]);
    } else if (subcommand === "check") {
      required(rest, 2, "Usage: coda-state.mjs check <change-name> <phase> [--recover]");
      if (rest[2] === "--recover") await recover(output, rest[0]);
      else await check2(output, rest[0], rest[1]);
    } else if (subcommand === "scale") {
      required(rest, 1, "Usage: coda-state.mjs scale <change-name>");
      await scale(output, rest[0]);
    } else if (subcommand === "task-checkoff") {
      required(rest, 2, "Usage: coda-state.mjs task-checkoff <file> <task-text>");
      await taskCheckoff(output, rest[0], rest[1]);
    } else if (subcommand === "next") {
      required(rest, 1, "Usage: coda-state.mjs next <change-name>");
...
function commandError(command) {
  if (!command) {
    return {
      exitCode: 64,
      stderr: `Usage: coda-runtime <${CLASSIC_COMMANDS.join("|")}> [args]`
    };
  }
  return {
    exitCode: 64,
