...
> 此规则每轮注入，防止长上下文时遗忘 Coda 流程状态。
> Hook 平台额外执行 `Coda-hook-guard.mjs` 进行硬性拦截；
> 此 Rule 是所有平台通用的软性防线。
...
有活跃 Coda change 时（`openspec/changes/<name>/.Coda.yaml` 存在），**每次开始执行操作前**必须读取 `phase` 字段确认当前阶段。
...
仅看 `phase` 字段不够——还必须确认"是如何到达这个阶段的"。每次准备写源代码前，先用下表自检 `.Coda.yaml` 是否处于**非法空跳**状态（绕过了前置阶段）。命中任一行，立即停止写源代码，按动作回到对应阶段补齐产物，不得信任 `phase` 字段直接续跑。

| 检测到 | 判定 | 动作 |
|--------|------|------|
| `phase: build` + `workflow: full` + `design_doc` 为空/null | 绕过 design 空跳 | 停止写源代码，运行 `/Coda-design` 补 Design Doc 并过 guard |
| `phase: build` + proposal/design/tasks 任一缺失或为空 | 绕过 open 空跳 | 回 `/Coda-open` 补齐三件套 |
| `phase: archive` + `verify_result` ≠ `pass` | 绕过 verify 空跳 | 回 `/Coda-verify` 完成验证 |
...
升级态说明：预设（hotfix/tweak）命中升级信号并经用户确认升级后，通过 `Coda-state transition <name> preset-escalate` 合法地变为 `workflow: full` + `phase: design` + `design_doc: null`。此时 `phase: design` + `design_doc` 为空**属正常升级前置态**，不是非法空跳——agent 应进入 `/Coda-design` 补 Design Doc。该终态不命中上表「绕过 design 空跳」行（该行仅检测 `phase: build`）。
...
- **阶段退出**: `Coda-guard <name> <phase> --apply`（必须看到 ALL CHECKS PASSED）
- **压缩恢复**: `Coda-state check <name> <phase> --recover`
- **状态更新**: 关键操作后通过 `Coda-state set` 更新字段，禁止手工编辑 .Coda.yaml
- **阶段推进只能经 guard/transition**: 禁止用 `Coda-state set <name> phase <值>` 手动跳阶段（会绕过证据校验，脚本已硬拦截）；确需修复畸形状态时才用 `Coda_FORCE_PHASE=1` 逃生阀。预设（hotfix/tweak）升级到 full 必须用 `Coda-state transition <name> preset-escalate`——这是唯一能合法回退 phase 到 design 并同步 workflow/classic_profile 的通道，直接 `set phase design` 和 `set classic_profile` 都会被硬拦截
- **handoff 生成**: `Coda-handoff <name> design --write`（禁止手写摘要）
...
1. 第一个脚本操作 = `Coda-handoff <name> design --write`（未生成 handoff 禁止加载 brainstorming）
...
1. 第一步运行 `Coda-state scale <name>` 确定验证级别
2. 验证失败后列出失败项等用户选择，CRITICAL 必须修
3. 连续 3 次失败后必须让用户选择接受偏差或继续修
4. 用户选择修复时运行 `Coda-state transition <name> verify-fail`：该转移会把 `phase` 回退到 `build`（不是停在 verify），随后进入 `/Coda-build` 修复，修完重新过 build→verify guard
...
2. 读取 `Coda/reference/subagent-dispatch.md` 获取 Coda 专属扩展 (re-read `Coda/reference/subagent-dispatch.md` for Coda-specific extensions)
3. 读取 `openspec/changes/<name>/.Coda/subagent-progress.md` 恢复精确阶段、证据和审查-修复轮次 (Read `openspec/changes/<name>/.Coda/subagent-progress.md` to recover the exact stage, evidence, and review-fix round)
