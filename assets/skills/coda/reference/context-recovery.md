...
Canonical path: `Coda/reference/context-recovery.md`

This protocol is shared by all coda sub-skills that may trigger context compression. When the agent suspects context compression has occurred (previous conversation summarized, cannot find previously discussed content), follow this protocol to recover.

## Recovery Steps

```bash
node "$CODA_STATE" check <change-name> <phase> --recover
```
...
3. Read `openspec/changes/<name>/.Coda/subagent-progress.md` to recover the current task or final review, implementation commit, RED/GREEN evidence, passed reviews, unresolved feedback, and review-fix round
