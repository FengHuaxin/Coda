...
Canonical path: `coda/reference/coda-yaml-fields.md`

This file is the field reference for the `.coda.yaml` state file. Consult on demand; not loaded inline with skills.
...
| `isolation` | `branch` or `worktree`, workspace isolation mode. Full init may be `null` but only until `/coda-build` Step 3; hotfix/tweak default to `branch` |
| `verify_mode` | `light` or `full`; may be empty |
| `auto_transition` | `true` or `false`. Only controls whether to automatically invoke the next skill after phase guard advances phase; `false` outputs `manual` from `coda-state next`, pausing next-skill invocation but not blocking phase field updates |
