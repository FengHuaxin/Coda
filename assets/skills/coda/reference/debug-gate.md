...
Canonical path: `Coda/reference/debug-gate.md`

This protocol is shared by Coda sub-skills that directly modify code, including build, hotfix, and tweak. Enter the Debug Gate when a crash, unexpected behavior, test failure, or build failure appears while running the program, tests, build, or manual verification.
