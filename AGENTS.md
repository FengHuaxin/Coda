...
```bash
npx vitest run test/domains/Coda-classic/Coda-scripts.test.ts   # shell 脚本测试
npx vitest run                                   # 全量测试
```
...
脚本位于 `assets/skills/Coda/scripts/`，当前发布形态是薄 `.mjs` launcher + 生成的 `Coda-runtime.mjs`：

- 运行时源码位于 `domains/Coda-classic/`，修改后必须运行 `pnpm build:classic-runtime` 同步 `assets/skills/Coda/scripts/Coda-runtime.mjs`
- launcher 必须保持薄封装，只 import `./Coda-runtime.mjs` 并调用对应命令；不要把业务逻辑写回 launcher
- 不再新增 `.sh` runtime；测试 fixture `test/fixtures/classic-0.3.9/` 是冻结参考实现，只用于差分兼容
- 新增 launcher 或 runtime 文件必须加入 `test/domains/Coda-classic/Coda-scripts.test.ts` 的 `beforeEach` 拷贝列表和 `assets/manifest.json`

## 脚本依赖关系

```
Coda-runtime.mjs ← domains/Coda-classic/*
Coda-state.mjs ← Coda-runtime.mjs
Coda-guard.mjs ← Coda-runtime.mjs
Coda-handoff.mjs ← Coda-runtime.mjs (写入 handoff_context/handoff_hash)
Coda-archive.mjs ← Coda-runtime.mjs
Coda-yaml-validate.mjs ← Coda-runtime.mjs
Coda-hook-guard.mjs ← Coda-runtime.mjs
```

新增共享工具函数时（如 archive 目录解析、change name 校验、hash、yaml 解析），优先放在 `domains/Coda-classic/` 的共享模块中，再重新生成 runtime，避免多个命令漂移。

## .Coda.yaml 状态机

每个 change 的状态文件，字段变更需要同步三处：

1. `domains/Coda-classic/classic-state-command.ts` — `set` 白名单 + enum 验证
2. `domains/Coda-classic/classic-validate-command.ts` — schema 校验 + KNOWN_KEYS
