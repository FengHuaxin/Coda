...
const PACKAGE_NAME = '@fenghuaxin/coda';
...
/**
 * Get the current installed Coda version from package.json.
 */
 ...
 * Used by `coda init` and `coda update` at the start of command output.
 */
export async function printVersionInfo(
  ...
  log(`  Coda v${result.currentVersion}`);
