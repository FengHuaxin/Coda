...
async function hasOpenCodeCodaCommands(baseDir: string, skillsDir: string, entries: string[]) {
  const codaEntries = entries.filter((entry) => entry.startsWith('Coda'));
  if (codaEntries.length === 0) return false;

  const commandsDir = path.join(baseDir, skillsDir, 'commands');
  const commandEntries = await readDir(commandsDir);
  return codaEntries.every((entry) => commandEntries.includes(`${entry}.md`));
}
...
async function hasSkills(
  baseDir: string,
  platform: Platform,
  component: 'openspec' | 'superpowers' | 'coda',
    ...
    case 'coda':
      if (OPENCODE_STYLE_PLATFORM_IDS.has(platform.id)) {
        for (const dir of skillDirEntries) {
          if (await hasOpenCodeCodaCommands(baseDir, dir.skillsDir, dir.entries)) return true;
        }
        break;
      }
      if (entries.some((e) => e.startsWith('Coda'))) return true;
      break;
  }
      ...
      case 'coda':
        if (OPENCODE_STYLE_PLATFORM_IDS.has(platform.id)) {
          for (const dir of globalSkillDirEntries) {
            if (await hasOpenCodeCodaCommands(os.homedir(), dir.skillsDir, dir.entries)) {
              return true;
            }
          }
          break;
        }
        if (globalEntries.some((e) => e.startsWith('Coda'))) return true;
        break;
    }
  }
