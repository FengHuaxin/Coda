...
describe('context execution benchmark runner', () => {
  ...
  it('creates a deterministic dry-run report for both phases', async () => {
    ...
    const tmpDir = path.join(
      os.tmpdir(),
      `Coda-execution-benchmark-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    try {
      ...
      expect(report).toContain('# Coda Execution Benchmark 报告');
      ...
      await expect(
        fs.stat(
          path.join(tmpDir, '.Coda', 'benchmark', 'execution', 'l1', 'small', 'beta', 'openspec'),
        ),
      ).resolves.toBeDefined();
    } finally {
  ...
  it('runs L3 dry-run with correct fixture structure', async () => {
    ...
    const tmpDir = path.join(
      os.tmpdir(),
      `Coda-execution-l3-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('runs all phases (l1+l2+l3) together in dry-run', async () => {
    ...
    const tmpDir = path.join(
      os.tmpdir(),
      `Coda-execution-all-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
