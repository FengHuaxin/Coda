...
describe('Classic baseline benchmark', () => {
  it('reports perfect deterministic migration and recovery rates', async () => {
    ...
    expect(report.transitionAccuracy).toBe(1);
    expect(report.migrationSuccessRate).toBe(1);
    expect(report.idempotencyRate).toBe(1);
    expect(report.contractMatchRate).toBe(1);
