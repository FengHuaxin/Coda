...
describe('context compression benchmark runner', () => {
  ...
  it('normalizes Windows paths before passing script paths to Bash', async () => {
    ...
    expect(toBashPath('D:\\Project\\Coda\\assets\\skills\\Coda\\scripts\\Coda-handoff.sh')).toBe(
      '/d/Project/Coda/assets/skills/Coda/scripts/Coda-handoff.sh',
    );
    expect(
      toBashPath('D:\\Project\\Coda\\assets\\skills\\Coda\\scripts\\Coda-handoff.sh', 'wsl'),
    ).toBe('/mnt/d/Project/Coda/assets/skills/Coda/scripts/Coda-handoff.sh');
    expect(toBashPath('/tmp/Coda-handoff.sh')).toBe('/tmp/Coda-handoff.sh');
  });
  ...
  it('places Codex approval flags before the exec subcommand', async () => {
    ...
    const args = buildCodexArgs({
      cwd: 'D:\\Project\\Coda\\.Coda\\fixture',
      model: 'gpt-test',
    });
  ...
  it('creates a deterministic dry-run report without invoking Codex', async () => {
    ...
    const tmpDir = path.join(
      os.tmpdir(),
      `Coda-context-benchmark-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    try {
      ...
      expect(report).toContain('# Coda 上下文压缩 Benchmark 报告');
