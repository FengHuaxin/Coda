...
function mockExternalSuccess() {
  mockedExecFileSync.mockImplementation((command: unknown, args?: unknown, opts?: unknown) => {
    ...
    if (
      ...
      const stagedSkillsDir = path.join(cwd, '.claude', 'skills', 'Coda');
      mkdirSync(stagedSkillsDir, { recursive: true });
      writeFileSync(path.join(stagedSkillsDir, 'SKILL.md'), '# Lingma Coda\n');
...
describe('Coda init E2E', () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `Coda-init-e2e-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it(
    'installs Coda skills at project scope with --yes --json',
    async () => {
      mockExternalSuccess();
      ...
      const claudeResult = (result.results as { platform: string; Coda: string }[]).find(
        (r) => r.platform === 'claude',
      );
      expect(claudeResult?.Coda).toBe('installed');
  ...
  it(
    'installs Coda skills at global scope',
    async () => {
      mockExternalSuccess();
  ...
  it(
    'skips already-installed Coda skills with --yes',
    async () => {
      mockExternalSuccess();
      await fs.mkdir(path.join(tmpDir, '.claude'), { recursive: true });
      ...
      const claude1 = (result1.results as { platform: string; Coda: string }[]).find(
        (r) => r.platform === 'claude',
      );
      expect(claude1?.Coda).toBe('installed');
      ...
      const claude2 = (result2.results as { platform: string; Coda: string }[]).find(
        (r) => r.platform === 'claude',
      );
      expect(claude2?.Coda).toBe('skipped');
    },
    INIT_E2E_TIMEOUT_MS,
  );

  it(
    'overwrites existing Coda skills with --overwrite',
    async () => {
      mockExternalSuccess();
      await fs.mkdir(path.join(tmpDir, '.claude'), { recursive: true });
      ...
      const claude = (result.results as { platform: string; Coda: string }[]).find(
        (r) => r.platform === 'claude',
      );
      expect(claude?.Coda).toBe('installed');
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await expect(
          fs.access(path.join(tmpDir, '.opencode', 'commands', 'Coda-open.md')),
        ).resolves.toBeUndefined();
        await expect(
          fs.access(path.join(tmpDir, '.mimocode', 'commands', 'Coda-open.md')),
        ).resolves.toBeUndefined();
        await expect(
          fs.access(path.join(tmpDir, '.pi', 'extensions', 'Coda-commands.ts')),
        ).resolves.toBeUndefined();
      } finally {
  ...
  it(
    'installs Antigravity Coda skills to the Gemini global skills directory',
    async () => {
      mockExternalSuccess();
  ...
  it(
    'installs OpenCode global Coda skills and commands to the OpenCode config directory',
    async () => {
      mockExternalSuccess();
      ...
      await expect(
        fs.access(path.join(fakeHome, '.config', 'opencode', 'commands', 'Coda.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(fakeHome, '.config', 'opencode', 'commands', 'Coda-open.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(fakeHome, '.opencode', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    'installs MimoCode global Coda skills and commands to the MimoCode config directory',
    async () => {
      mockExternalSuccess();
      ...
      await expect(
        fs.access(path.join(fakeHome, '.config', 'mimocode', 'commands', 'Coda.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(fakeHome, '.config', 'mimocode', 'commands', 'Coda-open.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(fakeHome, '.mimocode', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    ...
    async () => {
      ...
      await expect(
        fs.access(path.join(fakeHome, '.pi', 'agent', 'skills', 'Coda', 'SKILL.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(fakeHome, '.pi', 'agent', 'extensions', 'Coda-commands.ts')),
      ).resolves.toBeUndefined();
      await expect(
        fs.readFile(path.join(fakeHome, '.pi', 'agent', 'settings.json'), 'utf-8'),
      ).resolves.toContain('"enableSkillCommands": true');
      await expect(
        fs.access(path.join(fakeHome, '.pi', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    'installs Lingma global Coda skills to the user Lingma skills directory',
    async () => {
      mockExternalSuccess();
      ...
      await expect(
        fs.access(path.join(tmpDir, '.lingma', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    'installs Kimi Code global Coda skills to the user Kimi Code skills directory',
    async () => {
      mockExternalSuccess();
      ...
      await expect(
        fs.access(path.join(tmpDir, '.kimi-code', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
  ...
  it(
    'installs ZCode global Coda skills to the user ZCode skills directory',
    async () => {
      mockExternalSuccess();
      ...
      // Coda rules are also distributed to the platform rules directory
      ...
      await expect(
        fs.access(path.join(tmpDir, '.zcode', 'skills', 'Coda', 'SKILL.md')),
      ).rejects.toThrow();
    },
    INIT_E2E_TIMEOUT_MS,
  );
