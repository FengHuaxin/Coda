...
describe('init command helpers', () => {
  ...
  it('creates a project coda config with context compression disabled by default', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-init-config-'));

    try {
      await createWorkingDirs(tmpDir);

      const config = await fs.readFile(path.join(tmpDir, '.coda', 'config.yaml'), 'utf-8');
  ...
  it('preserves existing user config values and fills missing managed fields', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-init-merge-'));
  ...
  it('installs manifest-driven Pi slash commands and preserves existing settings', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-init-pi-'));
    ...
    try {
      ...
      await copyCodaSkillsForPlatform(tmpDir, piPlatform, false, 'skills', 'project');

      const extension = await fs.readFile(
        path.join(tmpDir, '.pi', 'extensions', 'coda-commands.ts'),
        'utf-8',
      );
  ...
  it('rejects invalid Pi settings without writing a command extension', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-init-pi-invalid-'));
    const piPlatform = PLATFORMS.find((platform) => platform.id === 'pi')!;
    const settingsPath = path.join(tmpDir, '.pi', 'settings.json');
    const extensionPath = path.join(tmpDir, '.pi', 'extensions', 'coda-commands.ts');
    ...
    try {
      ...
      await expect(
        copyCodaSkillsForPlatform(tmpDir, piPlatform, true, 'skills', 'project'),
  ...
  it('overwrites a stale Pi command extension while preserving unrelated settings', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-init-pi-overwrite-'));
    const piPlatform = PLATFORMS.find((platform) => platform.id === 'pi')!;
    const settingsPath = path.join(tmpDir, '.pi', 'settings.json');
    const extensionPath = path.join(tmpDir, '.pi', 'extensions', 'coda-commands.ts');
