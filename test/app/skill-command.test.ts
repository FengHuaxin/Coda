...
describe('skill validate and inspect commands', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-skill-command-'));
    projectRoot = path.join(root, 'project');
    skillRoot = path.join(projectRoot, '.coda', 'skills', 'demo');
  ...
  it('installs an explicit Skill into a different project', async () => {
    ...
    expect(result).toMatchObject({
      name: 'demo',
      version: '1',
      destination: path.join(targetProject, '.coda', 'skills', 'demo'),
    });
  });
...
`,
    ...
    try {
      ...
      expect(runOutput).toContain('Next: complete the pending action, then run coda skill continue');
      ...
      expect(evalOutput).toContain(
        'Next: record the missing artifact/state and rerun coda skill check',
      );
    } finally {
      log.mockRestore();
    }
...
`,
  ...
  it('inspects orchestration, dependencies and guardrails without modifying the Skill', async () => {
    const before = await fs.readFile(path.join(skillRoot, 'coda', 'skill.yaml'), 'utf8');
    ...
    expect(await fs.readFile(path.join(skillRoot, 'coda', 'skill.yaml'), 'utf8')).toBe(before);
  });
});
