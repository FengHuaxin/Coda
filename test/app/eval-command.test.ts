...
describe('eval command', () => {
  ...
  it('runs a manifest-backed quick eval from the repo root', async () => {
    ...
    try {
      ...
      await evalRunCommand({
        project: 'D:/Project/coda',
        manifest: 'D:/tmp/demo/coda/eval.yaml',
        quick: true,
      });
    } finally {
      log.mockRestore();
    ...
    expect(execFileSync).toHaveBeenCalledWith(
      ...
      [
        ...
        `--eval-manifest=${'D:\\tmp\\demo\\coda\\eval.yaml'}`,
        '-v',
      ],
      {
        cwd: 'D:\\Project\\coda\\eval',
        stdio: 'inherit',
      },
    );
  });
  ...
  it('collects a manifest target directly with --collect', async () => {
    ...
    try {
      const { evalCommand } = await import('../../app/commands/eval.js');
      await evalCommand('D:/tmp/demo/coda/eval.yaml', {
        project: 'D:/Project/coda',
        collect: true,
      });
    } finally {
  ...
  it('prints eval execution details and report path for manifest runs', async () => {
    ...
    expect(output).toContain('Eval root: D:\\Project\\coda\\eval');
