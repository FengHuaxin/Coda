...
import {
  removeCodaSkillsForPlatform,
  removeCodaRulesForPlatform,
  removeCodaHooksForPlatform,
  removeWorkingDirs,
...
describe('uninstall', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `coda-uninstall-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe('removeCodaSkillsForPlatform', () => {
    const claudePlatform: Platform = PLATFORMS.find((p) => p.id === 'claude')!;

    it('removes installed coda skills', async () => {
      await copyCodaSkillsForPlatform(tmpDir, claudePlatform, true, 'skills', 'project');

      const skillsDir = path.join(tmpDir, '.claude', 'skills');
      const entriesBefore = await fs.readdir(skillsDir);
      const CODAEntries = entriesBefore.filter((e) => e.startsWith('Coda'));
      expect(CODAEntries.length).toBeGreaterThan(0);

      const result = await removeCodaSkillsForPlatform(tmpDir, claudePlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);

      for (const entry of CODAEntries) {
    ...
    it('removes OpenCode commands', async () => {
      ...
      await copyCodaSkillsForPlatform(tmpDir, opencodePlatform, true, 'skills', 'project');

      const commandsDir = path.join(tmpDir, '.opencode', 'commands');
      expect(await fileExists(commandsDir)).toBe(true);

      const result = await removeCodaSkillsForPlatform(tmpDir, opencodePlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);
    });
    ...
    it('removes only the managed Pi extension and preserves shared settings', async () => {
      ...
      const CODAExtension = path.join(extensionsDir, 'coda-commands.ts');
      ...
      await copyCodaSkillsForPlatform(tmpDir, piPlatform, true, 'skills', 'project');
      await fs.writeFile(unrelatedExtension, 'export default function custom() {}', 'utf-8');

      const result = await removeCodaSkillsForPlatform(tmpDir, piPlatform, 'project');
      const settings = JSON.parse(await fs.readFile(settingsPath, 'utf-8'));

      expect(result.removed).toBeGreaterThan(0);
      expect(await fileExists(CODAExtension)).toBe(false);
      expect(await fileExists(unrelatedExtension)).toBe(true);
      expect(settings).toEqual({ theme: 'dark', enableSkillCommands: true });
    });

    it('removes coda skills from the legacy global Pi directory', async () => {
      const piPlatform: Platform = PLATFORMS.find((p) => p.id === 'pi')!;
      const legacySkill = path.join(tmpDir, '.pi', 'skills', 'coda', 'SKILL.md');

      await fs.mkdir(path.dirname(legacySkill), { recursive: true });
      await fs.writeFile(legacySkill, '# coda', 'utf-8');

      const result = await removeCodaSkillsForPlatform(tmpDir, piPlatform, 'global');
  ...
  describe('removeCodaRulesForPlatform', () => {
    it('removes rules for a platform that supports them', async () => {
      const claudePlatform: Platform = PLATFORMS.find((p) => p.id === 'claude')!;

      await copyCodaRulesForPlatform(tmpDir, claudePlatform, true, 'project');

      const rulePath = path.join(tmpDir, '.claude', 'rules', 'coda-phase-guard.md');
      expect(await fileExists(rulePath)).toBe(true);

      const result = await removeCodaRulesForPlatform(tmpDir, claudePlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);
    ...
    it('removes Cursor MDC format rules', async () => {
      ...
      await copyCodaRulesForPlatform(tmpDir, cursorPlatform, true, 'project');

      const rulePath = path.join(tmpDir, '.cursor', 'rules', 'coda-phase-guard.mdc');
      expect(await fileExists(rulePath)).toBe(true);

      const result = await removeCodaRulesForPlatform(tmpDir, cursorPlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);
      expect(await fileExists(rulePath)).toBe(false);
    });
    ...
    it('removes GitHub Copilot instructions format', async () => {
      ...
      await copyCodaRulesForPlatform(tmpDir, copilotPlatform, true, 'project');

      const rulePath = path.join(
        tmpDir,
        '.github',
        'instructions',
        'coda-phase-guard.instructions.md',
      );
      expect(await fileExists(rulePath)).toBe(true);

      const result = await removeCodaRulesForPlatform(tmpDir, copilotPlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);
      expect(await fileExists(rulePath)).toBe(false);
    });
    ...
    it('skips platforms without rules support', async () => {
      ...
      const result = await removeCodaRulesForPlatform(tmpDir, geminiPlatform, 'project');
      expect(result.removed).toBe(0);
    });
  });

  describe('removeCodaHooksForPlatform', () => {
    it('removes Claude Code hooks while preserving non-coda hooks', async () => {
      const claudePlatform: Platform = PLATFORMS.find((p) => p.id === 'claude')!;
          ...
          PreToolUse: [
            {
              ...
              hooks: [
                {
                  type: 'command',
                  command: 'bash .claude/skills/Coda/scripts/Coda-hook-guard.sh',
                },
      ...
      await installCodaHooksForPlatform(tmpDir, claudePlatform, 'project');

      const result = await removeCodaHooksForPlatform(tmpDir, claudePlatform, 'project');
      expect(result.removed).toBeGreaterThan(0);
      ...
      expect(allCommands.some((c: string) => c.includes('coda-hook-guard'))).toBe(false);
    });
    ...
    it('removes Copilot hook file', async () => {
      ...
      const hookFilePath = path.join(hooksDir, 'coda-guard.json');
      await fs.writeFile(hookFilePath, JSON.stringify({ version: 1 }), 'utf-8');

      expect(await fileExists(hookFilePath)).toBe(true);

      const result = await removeCodaHooksForPlatform(tmpDir, copilotPlatform, 'project');
      expect(result.removed).toBe(1);
    ...
    it('removes Kiro hook files', async () => {
      ...
      await fs.mkdir(hooksDir, { recursive: true });
      const hookFilePath = path.join(hooksDir, 'Coda-hook-guard.kiro.hook');
      await fs.writeFile(hookFilePath, JSON.stringify({ enabled: true }), 'utf-8');

      expect(await fileExists(hookFilePath)).toBe(true);

      const result = await removeCodaHooksForPlatform(tmpDir, kiroPlatform, 'project');
      expect(result.removed).toBe(1);
      expect(await fileExists(hookFilePath)).toBe(false);
    });
    ...
    it('skips platforms without hooks support', async () => {
      const cursorPlatform: Platform = PLATFORMS.find((p) => p.id === 'cursor')!;
      const result = await removeCodaHooksForPlatform(tmpDir, cursorPlatform, 'project');
      expect(result.removed).toBe(0);
    });
  ...
  describe('removeWorkingDirs', () => {
    it('removes .coda directory', async () => {
      const CODADir = path.join(tmpDir, '.coda');
      await fs.mkdir(CODADir, { recursive: true });
      await fs.writeFile(path.join(CODADir, 'config.yaml'), 'test: true', 'utf-8');

      const result = await removeWorkingDirs(tmpDir);
      expect(result.removed).toBeGreaterThan(0);
      expect(await fileExists(CODADir)).toBe(false);
    });
  ...
  describe('full uninstall cycle', () => {
    it('installs and then completely removes coda for Claude Code', async () => {
      const claudePlatform: Platform = PLATFORMS.find((p) => p.id === 'claude')!;
      ...
      // Verify installation
      const skillsDir = path.join(tmpDir, '.claude', 'skills');
      const skillEntries = (await fs.readdir(skillsDir)).filter((e) => e.startsWith('Coda'));
      ...
      const skillsResult = await removeCodaSkillsForPlatform(tmpDir, claudePlatform, 'project');
      expect(skillsResult.removed).toBeGreaterThan(0);

      const rulesResult = await removeCodaRulesForPlatform(tmpDir, claudePlatform, 'project');
      expect(rulesResult.removed).toBeGreaterThan(0);

      const hooksResult = await removeCodaHooksForPlatform(tmpDir, claudePlatform, 'project');
...
describe('uninstallCommand interactive selection', () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(
      os.tmpdir(),
      `coda-uninstall-cmd-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    await fs.mkdir(tmpDir, { recursive: true });
  });
  ...
  it('auto-selects single target and uninstalls on confirmation', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills');
    const entries = (await fs.readdir(skillsDir)).filter((e) => e.startsWith('Coda'));
  ...
  it('shows checkbox when multiple targets detected', async () => {
    ...
    const codexDir = path.join(tmpDir, '.codex', 'skills', 'Coda');
    await fs.mkdir(codexDir, { recursive: true });
    await fs.writeFile(path.join(codexDir, 'SKILL.md'), '# Coda', 'utf-8');
    ...
    const claudeEntries = (await fs.readdir(claudeSkillsDir)).filter((e) => e.startsWith('Coda'));
  ...
  it('prints message when no targets found', async () => {
    ...
    expect(output).toContain('No Coda installations found');
    expect(mockedSelect).not.toHaveBeenCalled();
  });
});
