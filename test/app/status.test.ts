...
describe('status command', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `coda-status-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('prints the next command for active changes', async () => {
    ...
    expect(output).toContain('next: /coda-build');
  ...
  it('silently migrates legacy state and includes the Run step in JSON output', async () => {
    ...
    expect(change.nextCommand).toBe('/coda-verify');
  ...
  it('reports invalid state without modifying it', async () => {
    ...
    await fs.appendFile(path.join(changeDir, '.coda.yaml'), 'unknown_root_field: true\n');
    const before = await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8');
  ...
  it('keeps invalid errors visible and only prints the invalid recovery hint for invalid changes', async () => {
    ...
    expect(output).toContain('next: inspect .coda.yaml and rerun coda doctor');
    expect(output).toContain('runtime-eval-fail [phase: open]');
    expect(output.match(/next: inspect \.coda\.yaml and rerun coda doctor/g)).toHaveLength(1);
  });
