...
function referencedCommands(doc: string): string[] {
  const refs: string[] = [];
  for (const match of doc.matchAll(/coda\s+(bundle|publish|skill|eval)\s+([\w-]+)/gu)) {
    refs.push(match[2]!);
  }
  return refs;
}

describe('coda-any skill contract', () => {
  it('never references the non-existent find-skill command (en + zh)', async () => {
    for (const localeRoot of ['assets/skills/coda-any', 'assets/skills-zh/coda-any']) {
  ...
  it('only references commands that are registered in the CLI', async () => {
    ...
    const docs = await readTree(path.resolve(REPO_ROOT, 'assets/skills/coda-any'));
    ...
    expect(
      unregistered,
      `coda-any references unregistered commands: ${unregistered.join(', ')}`,
    ).toEqual([]);
  });

  it('documents the full six-script generated package', async () => {
    const bundleAuthoring = await readText('assets/skills/coda-any/reference/bundle-authoring.md');
    for (const script of [
      'workflow-state.mjs',
      'workflow-guard.mjs',
