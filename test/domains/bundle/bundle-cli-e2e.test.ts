...
const cli = path.join(repositoryRoot, 'bin', 'coda.js');
...
describe('coda bundle CLI end to end', () => {
  let root: string;
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-cli-'));
  ...
  it('optimizes, evaluates, approves, publishes, and distributes a Bundle', async () => {
    ...
    await expect(
      fs.access(path.join(projectRoot, '.claude', 'skills', 'alpha', '.coda')),
  ...
  it('prints a JSON Factory proposal before draft creation', async () => {
    ...
    await fs.mkdir(path.join(projectRoot, '.coda', 'skills', 'factory-alpha'), {
      recursive: true,
    });
    await fs.writeFile(
...
`,
    ...
    await fs.writeFile(
      path.join(projectRoot, '.coda', 'skills', 'factory-alpha', 'SKILL.md'),
...
`,
    ...
    expect(bundleStatus.stdout).toContain(
      'Benchmark: missing; run coda bundle benchmark-plan and coda bundle benchmark-record',
    );
    expect(bundleStatus.stdout).toContain(
      'Review: missing; run coda bundle review-summary before approval',
    );
    expect(bundleStatus.stdout).toContain('Next action: choose-benchmark-level');
    expect(bundleStatus.stdout).toContain('Suggested user command: coda eval ');
    expect(bundleStatus.stdout).toContain(
      'Backend command: coda bundle benchmark-plan factory-status-text-mode --level quick',
    );
  });
...
`,
    ...
    expect(status).toMatchObject({
      nextAction: {
        action: 'choose-benchmark-level',
        backendCommand: 'coda bundle benchmark-plan factory-next-action --level quick',
...
`,
    ...
    expect(status.stdout).toContain(
      'Suggested user command: Ask /coda-any to resolve missing-skill',
    );
    expect(status.stdout).toContain(
      'Backend command: coda bundle factory-resolve factory-resolve-action --candidate missing-skill',
    );
  });
});
