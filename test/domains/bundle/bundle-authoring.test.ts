...
describe('Bundle authoring lifecycle', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-authoring-'));
  ...
  it('creates a draft directory and atomically persists authoring choices', async () => {
    ...
    expect(await fs.readdir(path.join(projectRoot, '.coda', 'bundle-authoring'))).toEqual([
      'demo-bundle.json',
    ]);
  });
  ...
  it('persists factory metadata with ordered preferences and deviation reasons', async () => {
    ...
    const state = await createBundleDraft({
      ...
      engineEnabled: true,
      factory: {
        goal: 'Create a coda-native workflow authoring Skill.',
  ...
  it('never overwrites an existing draft directory', async () => {
    const existing = path.join(projectRoot, '.coda', 'bundle-drafts', 'existing');
  ...
  it('replaces state as complete JSON without leaving temporary files', async () => {
    ...
    await writeBundleAuthoringState(projectRoot, { ...state, creator: 'coda-fallback' });
  ...
  async function preparedReadyState(name: string): Promise<BundleAuthoringState> {
    ...
    const readyPath = path.join(projectRoot, '.coda', 'bundles', name);
    ...
    const ready: BundleAuthoringState = {
      ...
      eval: {
        ...
        resultPath: path.join(projectRoot, '.coda', 'bundle-evals', `${name}.json`),
        passed: true,
      },
      review: {
        hash: state.currentHash!,
