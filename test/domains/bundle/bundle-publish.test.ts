...
describe('Bundle review and publish', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-publish-'));
  ...
  it('publishes atomically, marks ready, and copies only the draft Bundle', async () => {
    ...
    expect(ready).toMatchObject({
      ...
      ready: {
        ...
        path: path.join(projectRoot, '.coda', 'bundles', 'publish-bundle'),
    ...
    await expect(
      fs.access(path.join(ready.ready!.path, '.coda', 'bundle-authoring')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
      fs.access(path.join(ready.ready!.path, '.coda', 'bundle-evals')),
    ).rejects.toMatchObject({ code: 'ENOENT' });
  });
  ...
  it('refuses an existing published target unless overwrite is explicit', async () => {
    ...
    const destination = path.join(projectRoot, '.coda', 'bundles', 'publish-bundle');
  ...
  it('blocks publishing an evaluated factory Bundle with missing control-plane files', async () => {
    ...
    ).rejects.toThrow(/control plane.*scripts\/coda-check\.mjs/iu);
  });
