...
describe('Bundle benchmark planning and evidence', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-eval-'));
    ...
    expect(state).toMatchObject({
      ...
      eval: {
        ...
        resultPath: expect.stringContaining(
          path.join('.coda', 'bundle-evals', 'eval-bundle', stateHash, 'result.json'),
        ),
      },
    });
  ...
  it('retains stale-hash evidence without allowing a lifecycle transition', async () => {
    ...
    await expect(
      fs.access(
        path.join(projectRoot, '.coda', 'bundle-evals', 'eval-bundle', stateHash, 'result.json'),
      ),
