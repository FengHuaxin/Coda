...
describe('bundle factory plan normalization', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-factory-plan-'));
  });
  ...
  it('normalizes workflow contracts as the primary factory plan input', () => {
    const normalized = normalizeBundleFactoryPlan({
      plan: {
        goal: 'Customize existing coda Skills with team execution requirements.',
        skillMakerIntent: 'customize-coda',
        workflow: {
          kind: 'coda-five-phase-overlay',
          name: 'team-coda',
          goal: 'Require component and whitebox Skills without replacing coda control nodes.',
          nodes: {
            execute: {
              requiredSkillCalls: [
                {
    ...
    expect(normalized.workflowProtocol.kind).toBe('coda-five-phase-overlay');
    ...
    expect(normalized.preferredSkills).toEqual(
      expect.arrayContaining(['elementui', 'whitebox-code-standard', 'coda-build']),
    );
