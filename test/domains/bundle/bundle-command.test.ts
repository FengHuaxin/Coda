...
async function writeFactorySkill(
  ...
  const skillRoot = path.join(projectRoot, '.coda', 'skills', name);
  ...
  if (options.flow) {
    ...
    await fs.writeFile(path.join(skillRoot, 'coda', 'flow.yaml'), options.flow, 'utf8');
  }
  return skillRoot;
}
...
describe('bundle commands', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-bundle-command-'));
...
`,
  ...
  it('creates and optimizes drafts, then reports status', async () => {
    ...
    expect(text).toContain(
      'Backend command: coda bundle benchmark-plan optimized-bundle --level quick',
    );
        ...
        expect.objectContaining({
          resumeSummary: expect.objectContaining({
            ...
            recommendedNextStep: expect.objectContaining({
              userCommand: expect.stringContaining('coda eval '),
            }),
          }),
        }),
        expect.objectContaining({
...
`,
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          goal: 'Create a review-oriented Coda-native Skill.',
    ...
    await expect(
      fs.readFile(
        path.join(projectRoot, '.coda', 'bundle-factory-plans', 'factory-bundle', 'plan.json'),
        'utf8',
      ),
...
`,
  ...
  it('generates a draft bundle source from stored factory metadata', async () => {
    ...
    const draftRoot = path.join(projectRoot, '.coda', 'bundle-drafts', 'factory-bundle');
    ...
    expect(bundle.manifest.resources.scripts).toEqual([
      {
        id: 'coda-plan',
        path: 'skills/factory-bundle/scripts/coda-plan.mjs',
        sideEffect: 'write',
        runtime: 'node',
      },
      {
        id: 'coda-check',
        path: 'skills/factory-bundle/scripts/coda-check.mjs',
        sideEffect: 'read',
        runtime: 'node',
      },
      {
        id: 'coda-hook-guard',
        path: 'skills/factory-bundle/scripts/coda-hook-guard.mjs',
        sideEffect: 'read',
        runtime: 'node',
      },
      {
    ...
    await expect(
      fs.access(path.join(draftRoot, 'skills', 'factory-bundle', 'scripts', 'coda-plan.mjs')),
    ).resolves.toBeUndefined();
    await expect(
      fs.readFile(path.join(draftRoot, 'skills', 'factory-bundle', 'coda', 'skill.yaml'), 'utf8'),
...
`,
  ...
  it('keeps composition blocking after resolving an ambiguous candidate to a flow with unresolved choice', async () => {
    ...
    await fs.mkdir(path.join(secondRoot, 'coda'), { recursive: true });
    await fs.writeFile(
      path.join(secondRoot, 'coda', 'flow.yaml'),
      `steps:
  - choose:
      id: review
      options:
    ...
    await createBundleDraft({
      ...
      factory: {
        ...
        generatedSkillPackage: {
          ...
          internalSkills: [],
          packageRoot: path.join(projectRoot, '.coda', 'bundle-drafts', 'resolvable-factory'),
          enginePath: null,
...
`,
  ...
  it('points composition-blocked factory states away from factory-generate as the next action', async () => {
            ...
            sources: [
              {
                ...
                root: path.join(projectRoot, '.coda', 'skills', 'task3-choice-flow'),
                description: 'Choice flow.',
                skillMd: '# Choice flow\n',
                hash: 'c'.repeat(64),
              },
    ...
    expect(status.nextAction).toMatchObject({
      ...
      backendCommand:
        'coda bundle review-summary factory-composition-action --platform <reference-platform>',
    });
