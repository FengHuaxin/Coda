...
describe('Bundle next action', () => {
  it('uses the generated eval manifest path in the user-facing eval command', () => {
    ...
    expect(action).toMatchObject({
      ...
      category: 'benchmark',
      userCommand:
        'coda eval /project/.coda/bundle-drafts/demo-skill/skills/demo-skill/coda/eval.yaml --quick --html',
    });
    expect(action.backendCommand).toBe('coda bundle benchmark-plan demo-skill --level quick');
  });
  ...
  it('requests review again when the current-hash review was rejected', () => {
    const reviewedState = state({
      ...
      eval: {
        hash: 'a'.repeat(64),
        level: 'quick',
        resultPath: '/project/.coda/bundle-drafts/demo-skill/eval-result.json',
        passed: true,
      },
      review: {
        hash: 'a'.repeat(64),
