...
describe('symlink install mode', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(path.join(os.tmpdir(), 'coda-symlink-test-'));
  });
  ...
  describe('getCentralSkillsDir', () => {
    it('returns .coda/skills for project scope', () => {
      const result = getCentralSkillsDir(tmpDir, 'project');
      expect(result).toBe(path.join(tmpDir, '.coda', 'skills'));
    });

    it('returns .coda/skills for global scope', () => {
      const result = getCentralSkillsDir(tmpDir, 'global');
      expect(result).toBe(path.join(tmpDir, '.coda', 'skills'));
    });
  });

  describe('copyCodaSkillsForPlatform with symlink mode', () => {
    it('copies skills to central store and creates symlink', async () => {
      const result = await copyCodaSkillsForPlatform(
        tmpDir,
        mockPlatform,
        false,
        'skills',
      ...
      const centralSkillPath = path.join(tmpDir, '.coda', 'skills', 'skills', 'coda', 'SKILL.md');
      ...
      expect(linkTarget).toBe(path.join(tmpDir, '.coda', 'skills', 'skills'));
    });

    it('skips existing files in central store when overwrite is false', async () => {
      // First install
      await copyCodaSkillsForPlatform(tmpDir, mockPlatform, false, 'skills', 'project', 'symlink');
    ...
    it('uses copy behavior when mode is copy (default)', async () => {
      ...
      const skillPath = path.join(tmpDir, '.claude', 'skills', 'coda', 'SKILL.md');
