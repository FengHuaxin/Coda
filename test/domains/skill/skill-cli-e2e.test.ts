...
`,
  ...
  await fs.writeFile(
    path.join(root, 'coda', 'checks.yaml'),
    `runtime:
  - id: report
    scope: completion
...
describe('coda skill CLI end to end', () => {
  let root: string;
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-skill-cli-'));
    projectRoot = path.join(root, 'project');
    changeDir = path.join(root, 'change');
    await writeSkill(path.join(projectRoot, '.coda', 'skills', 'demo'));
  });
  ...
  it('runs, resumes, upgrades, and evaluates a project Skill through bin/coda.js', async () => {
    ...
    const skillRoot = path.join(projectRoot, '.coda', 'skills', 'demo');
  ...
  it('runs a project Skill through a standalone .coda/runs/<run-id> directory', async () => {
    const started = runCli(
      'skill',
      'run',
      'demo',
    ...
    await expect(
      fs.access(
        path.join(projectRoot, '.coda', 'runs', 'standalone-demo', '.coda', 'run-state.json'),
      ),
