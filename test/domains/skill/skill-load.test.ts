...
const skillDefinition = `apiVersion: coda/v1alpha1
kind: Skill
metadata:
  name: demo
  version: 1.0.0
...
describe('loadSkillPackage', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-skill-load-'));
    ...
    await fs.writeFile(path.join(skillRoot, 'coda', 'skill.yaml'), skillDefinition);
  });
  ...
  it('loads explicit guardrails and runtime checks', async () => {
    await fs.writeFile(
      path.join(skillRoot, 'coda', 'guardrails.yaml'),
      `allowedSkills:
  - writing-plans
allowedAgents: []
allowedTools: []
...
`,
    ...
    await fs.writeFile(
      path.join(skillRoot, 'coda', 'checks.yaml'),
      `runtime:
  - id: report
    scope: completion
...
`,
  ...
  it('rejects legacy coda/evals.yaml instead of loading runtime checks from it', async () => {
    await fs.writeFile(
      path.join(skillRoot, 'coda', 'evals.yaml'),
      `runtime:
  - id: legacy-completed
    scope: completion
    type: state_equals
...
`,
  ...
  it('rejects packages that still include evals.yaml even when checks.yaml exists', async () => {
    await fs.writeFile(path.join(skillRoot, 'coda', 'checks.yaml'), 'runtime: []\n');
    await fs.writeFile(path.join(skillRoot, 'coda', 'evals.yaml'), 'runtime: []\n');
  ...
  it.each([
    ...
    await fs.writeFile(path.join(skillRoot, 'coda', 'skill.yaml'), yaml);

    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(/coda[\\/]skill\.yaml/);
  });
...
`,
    ...
    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(/coda[\\/]guardrails\.yaml/);
  });
  ...
  it.each([
    ...
    await fs.writeFile(path.join(skillRoot, 'coda', 'checks.yaml'), yaml);

    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(/coda[\\/]checks\.yaml/);
  });

  it('rejects checks.yaml with its file path and field path', async () => {
    await fs.writeFile(path.join(skillRoot, 'coda', 'checks.yaml'), `runtime:\n  id: report\n`);

    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(/coda[\\/]checks\.yaml.*runtime/);
  });

  it('loads a structurally complete nested skill definition', async () => {
    await fs.writeFile(
      path.join(skillRoot, 'coda', 'skill.yaml'),
...
`,
  ...
  steps:
    ...
    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(
      new RegExp(`coda[\\\\/]skill\\.yaml.*${field}`),
    );
  });
  ...
  it.each([
    ...
    await fs.writeFile(path.join(skillRoot, 'coda', 'guardrails.yaml'), yaml);

    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(
      new RegExp(`coda[\\\\/]guardrails\\.yaml.*${field}`),
    );
  });
  ...
  it.each([
    ...
    await expect(loadSkillPackage(skillRoot)).rejects.toThrow(
      new RegExp(`coda[\\\\/]checks\\.yaml.*${field}`),
    );
  });
});
