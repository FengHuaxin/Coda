...
import {
  ...
  getManifestSkills,
  createWorkingDirs,
  copyCodaSkillsForPlatform,
  installCodaHooksForPlatform,
...
describe('skills', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `Coda-skills-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe('getManifestSkills', () => {
    it('returns the skills array from manifest', async () => {
      ...
      expect(skills.some((s) => s.includes('Coda/SKILL.md'))).toBe(true);
    });
  });
  ...
  describe('copyCodaSkillsForPlatform', () => {
    ...
    it('copies skill files from assets to platform skills directory', async () => {
      const result = await copyCodaSkillsForPlatform(tmpDir, mockPlatform, false);
      expect(result.copied).toBeGreaterThan(0);
      expect(result.skipped).toBe(0);

      // Verify a key file was copied
      const CodaSkillPath = path.join(tmpDir, '.claude', 'skills', 'Coda', 'SKILL.md');
      expect(await fileExists(CodaSkillPath)).toBe(true);
    });

    it('skips existing files when overwrite is false', async () => {
      // First copy
      await copyCodaSkillsForPlatform(tmpDir, mockPlatform, false);
    ...
    it('overwrites existing files when overwrite is true', async () => {
      await copyCodaSkillsForPlatform(tmpDir, mockPlatform, false);
      const result = await copyCodaSkillsForPlatform(tmpDir, mockPlatform, true);
      expect(result.copied).toBeGreaterThan(0);
    });

    it('copies to Chinese skills directory when language is zh', async () => {
      const result = await copyCodaSkillsForPlatform(tmpDir, mockPlatform, false, 'skills-zh');
      expect(result.copied).toBeGreaterThan(0);
    ...
    it('creates OpenCode slash commands for copied Coda skills', async () => {
      ...
      const result = await copyCodaSkillsForPlatform(tmpDir, opencodePlatform, false);

      expect(result.copied).toBeGreaterThan(0);
      const commandPath = path.join(tmpDir, '.opencode', 'commands', 'Coda-open.md');
      const command = await fs.readFile(commandPath, 'utf-8');

      expect(command).toContain('description: Run the Coda-open Coda workflow');
      expect(command).toContain('Equivalent Coda skill: `Coda-open`');
      expect(command).toContain(
        'Use the invocation arguments below as the user input for this workflow:',
      );
      expect(command).toContain('$ARGUMENTS');
      expect(command).toContain('# Coda Phase 1: Open');
      expect(command).toContain('## Steps');
      expect(command).toContain('node "$Coda_STATE" init <name> full');
      expect(command).not.toContain('Immediately load the `Coda-open` skill with the skill tool');
      expect(path.basename(commandPath)).toBe('Coda-open.md');
    });
    ...
    it('creates OpenCode slash commands from the selected language skill content', async () => {
      ...
      await copyCodaSkillsForPlatform(tmpDir, opencodePlatform, false, 'skills-zh');
      ...
      expect(command).toContain('description: Run the Coda-open Coda workflow');
      expect(command).toContain('Equivalent Coda skill: `Coda-open`');
      expect(command).toContain('# Coda 阶段 1：开启（Open）');
      expect(command).toContain('## 步骤');
      expect(command).not.toContain('# Coda Phase 1: Open');
      expect(path.basename(commandPath)).toBe('Coda-open.md');
    });
    ...
    it('creates OpenCode slash commands in the global OpenCode config directory', async () => {
      ...
      await copyCodaSkillsForPlatform(tmpDir, opencodePlatform, false, 'skills', 'global');

      await expect(
        fs.access(path.join(tmpDir, '.config', 'opencode', 'commands', 'Coda.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(tmpDir, '.opencode', 'commands', 'Coda.md')),
      ).rejects.toThrow();
    });
    ...
    it('creates MimoCode slash commands in project and global config directories', async () => {
      ...
      await copyCodaSkillsForPlatform(tmpDir, mimocodePlatform, false, 'skills', 'project');
      await expect(
        fs.access(path.join(tmpDir, '.mimocode', 'commands', 'Coda-open.md')),
      ).resolves.toBeUndefined();

      const globalRoot = path.join(tmpDir, 'global-root');
      await fs.mkdir(globalRoot, { recursive: true });
      await copyCodaSkillsForPlatform(globalRoot, mimocodePlatform, false, 'skills', 'global');
      await expect(
        fs.access(path.join(globalRoot, '.config', 'mimocode', 'commands', 'Coda.md')),
      ).resolves.toBeUndefined();
      await expect(
        fs.access(path.join(globalRoot, '.mimocode', 'commands', 'Coda.md')),
      ).rejects.toThrow();
    });
  });

  describe('installCodaHooksForPlatform', () => {
    const staleCodaCommand = 'bash .legacy/skills/Coda/scripts/Coda-hook-guard.sh';
    const currentCodaScript = 'Coda/scripts/Coda-hook-guard.mjs';
    const normalized = (value: string) => value.replace(/\\/g, '/');
    const expectedHookCommand = (skillsDir: string) =>
      `node "${normalized(path.join(tmpDir, skillsDir, 'skills', ...currentCodaScript.split('/')))}" --project-root "${normalized(tmpDir)}"`;
    ...
    it('merges Claude-style hooks into an existing matcher group without replacing user hooks', async () => {
          ...
          PreToolUse: [
            {
              ...
              hooks: [
                ...
                { type: 'command', command: staleCodaCommand },
              ],
            },
            {
              matcher: 'Bash',
      ...
      await installCodaHooksForPlatform(tmpDir, platform);
      ...
      expect(normalized(command)).toContain(`/.claude/skills/${currentCodaScript}`);
    ...
    it('does not throw when an existing hook group is malformed (non-array)', async () => {
      ...
      await expect(installCodaHooksForPlatform(tmpDir, platform)).resolves.toEqual({
        installed: true,
      });
      ...
      async ({ id, skillsDir, hookFormat }) => {
            ...
            PreToolUse: [
              {
                ...
                hooks: [
                  ...
                  {
                    type: 'command',
                    command: staleCodaCommand,
                    description: 'Old Coda hook',
                  },
                ],
              },
            ],
        ...
        expect(firstInstall.hooks.PreToolUse[0].hooks).toEqual([
          ...
          },
          {
            ...
            description: 'Block code writes in wrong Coda phase (open/design/archive)',
          },
        ]);
    ...
    it('merges Gemini hooks into the existing matcher group idempotently', async () => {
      ...
      expect(firstInstall.hooks.BeforeTool[0].hooks).toEqual([
        ...
        },
        {
          ...
          name: 'Block code writes in wrong Coda phase (open/design/archive)',
        },
      ]);
      ...
      const initialHooks = {
        ...
        hooks: {
          ...
          pre_write_code: [
            ...
            { command: staleCodaCommand, show_output: true },
          ],
        },
      };
  ...
  describe('Chinese Coda workflow safeguards', () => {
    ...
    it('requires OpenSpec instructions for each standard open artifact (English)', async () => {
      const enOpen = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-open', 'SKILL.md'),
        'utf-8',
      );
    ...
    it('routes Chinese tweak build through OpenSpec apply without changing full workflow', async () => {
      const zhTweak = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-tweak', 'SKILL.md'),
        'utf-8',
      );
      const zhBuild = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-build', 'SKILL.md'),
        'utf-8',
      );
      ...
      expect(zhTweak).toContain(
        '完整 `/Coda` 或 `workflow: full` 不得套用 tweak 的 `openspec-apply-change` 构建路径',
      );
    ...
    it('requires explicit user confirmation at full-workflow decision points', async () => {
      const zhCoda = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda', 'SKILL.md'),
        'utf-8',
      );
      const zhOpen = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-open', 'SKILL.md'),
      ...
      const zhDesign = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-design', 'SKILL.md'),
        'utf-8',
      );
      const zhBuild = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-build', 'SKILL.md'),
      ...
      const zhVerify = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-verify', 'SKILL.md'),
        'utf-8',
      );
      const zhArchive = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-archive', 'SKILL.md'),
        'utf-8',
      );
      const zhHotfix = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-hotfix', 'SKILL.md'),
        'utf-8',
      );
      const zhTweak = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-tweak', 'SKILL.md'),
      ...
      const zhCodaRule = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'rules', 'Coda-phase-guard.md'),
        'utf-8',
      );
      const zhDecisionPoint = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda', 'reference', 'decision-point.md'),
        'utf-8',
      );
      const zhDebugGate = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda', 'reference', 'debug-gate.md'),
        'utf-8',
      );

      expect(zhCoda).toContain('决策点是阻塞点');
      expect(zhCoda).toContain('`Coda/reference/decision-point.md`');
      ...
      expect(zhOpen).toContain('`Coda/reference/decision-point.md`');
      expect(zhOpen).toContain(
        '完整 `/Coda` 流程默认不得使用 Skill 工具加载 `openspec-propose` 技能',
      );
      expect(zhOpen).toContain(
      ...
      expect(zhDesign).toContain(
        '必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户明确确认设计方案',
      );
      expect(zhDesign).toContain(
      ...
      expect(zhBuild).toContain('`Coda/reference/decision-point.md`');
      expect(zhVerify).toContain(
        '验证不通过时**必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户决定修复或接受偏差',
      );
      expect(zhVerify).toContain(
        '必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户选择分支处理方式',
      );
      expect(zhVerify).toContain(
        '只有在用户完成选择且对应操作完成后，才允许写入 `branch_status: handled`',
      );
      expect(zhArchive).toContain('### 1. 归档前最终确认（阻塞点）');
      expect(zhArchive).toContain('不得在用户确认前运行 `node "$Coda_ARCHIVE" "<change-name>"`');
      expect(zhArchive).toContain('`Coda/reference/decision-point.md`');
      expect(zhArchive).toContain('「确认归档」');
      expect(zhArchive).toContain('「需要调整或重新验证」');
      expect(zhArchive).toContain('「暂不归档」');
      expect(zhArchive).toContain('`node "$Coda_STATE" transition <change-name> archive-reopen`');
      expect(zhVerify).toContain('不得因为验证已通过就自动归档');
      expect(zhHotfix).toContain(
        '命中质变信号或文件数 tripwire 时，**必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户明确选择**',
      );
      expect(zhHotfix).toContain('不得直接进入 `/Coda-design`');
      expect(zhTweak).toContain(
        '命中质变信号或文件数 tripwire 时，**必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户明确选择**',
      );
      expect(zhTweak).toContain('不得直接进入 `/Coda-design`');
      expect(zhCoda).toContain('`verify_result: fail` → 进入验证失败决策阻塞点');
      expect(zhCoda).not.toContain(
        '`verify_result: fail` → `node "$Coda_STATE" transition <name> verify-fail` 后 `/Coda-build`',
      );
      expect(zhHotfix).toContain(
        '若 hotfix 创建了 delta spec，则根据 Coda-verify 的规模评估规则进入完整验证路径',
      );
      expect(zhHotfix).not.toContain('停止 hotfix，升级为 `/Coda`');
      expect(zhTweak).toContain('带 delta spec 的验证分流');

      // HIGH: hotfix/tweak IMPORTANT blocks must acknowledge verify decision points
      expect(zhHotfix).toContain('验证阶段（Coda-verify）的验证失败决策和分支处理决策');
      expect(zhTweak).toContain('验证阶段（Coda-verify）的验证失败决策和分支处理决策');
      expect(zhHotfix).toContain('归档前最终确认');
      expect(zhTweak).toContain('归档前最终确认');

      // MEDIUM: Coda-design brainstorming does not write Design Doc before confirmation
      ...
      // MEDIUM: Coda-verify Spec drift requires user choice
      expect(zhVerify).toContain(
        '必须使用当前平台可用的用户输入/确认机制以单选题形式暂停并等待用户选择处理方式',
      );

      // MEDIUM: Coda/SKILL.md build phase resume recognizes plan-ready pause before build decisions
      expect(zhCoda).toContain('先检查 `build_pause`、`plan`、`build_mode` 和 `isolation`');
      expect(zhCoda).toContain('`build_pause: plan-ready` 且 plan 文件存在');
      expect(zhCoda).toContain('`build_pause` 不是执行方式，不得写入 `build_mode`');
      expect(zhCoda).toContain(
      ...
      // MEDIUM: Coda-verify Step 1b treats CRITICAL/IMPORTANT as blocking
      ...
      // MEDIUM: hotfix IMPORTANT covers >3-tasks Coda-build decision points
      expect(zhHotfix).toContain('任务超过 3 个转入 `/Coda-build` 时的工作区隔离和执行方式选择');

      // LOW: Coda-build "中" level requires user confirmation before brainstorming
      expect(zhBuild).toContain(
        '使用当前平台可用的用户输入/确认机制暂停并等待用户确认后**，必须使用 Skill 工具加载 Superpowers `brainstorming`',
      );

      // LOW: Coda-build 50% threshold is a hard decision point
      expect(zhBuild).toContain(
        '必须按 `Coda/reference/decision-point.md` 的协议暂停并等待用户决定是否拆分为新 change',
      );

      // LOW: Coda-verify Step 2b disambiguates design.md vs Design Doc
      expect(zhVerify).toContain('实现符合 `openspec/changes/<name>/design.md` 高层设计决策');
      expect(zhTweak).not.toContain('停止 tweak，升级为完整 `/Coda`');

      // IMPORTANT: main /Coda preset detection must match the current tweak positioning.
      expect(zhCoda).toContain('用户明确描述为可收敛为单一 OpenSpec change 的轻量/中等变更');
      expect(zhCoda).toContain('通过 OpenSpec apply 执行');
      expect(zhCoda).not.toContain('用户明确描述为文案/配置/文档/prompt 小调整');

      // CRITICAL: build scope split must not bypass Coda state initialization
      expect(zhBuild).toContain('通过 `/Coda-open` 创建独立 change');
      ...
      expect(zhOpen).toContain('每个被接受的拆分项都必须通过 `/Coda-open` 创建独立 change');
      ...
      expect(zhOpen).toContain(
        '批量拆分模式下，单个拆分项完成 open 阶段后不得自动流转到 `/Coda-design`',
      );
      ...
      expect(zhCoda).toContain('build 阶段范围扩张需重新设计或拆分新 change');
      expect(zhCoda).toContain('archive 阶段执行归档脚本前的最终确认');
      expect(zhCoda).toContain('open 阶段大型 PRD 需确认拆分为多个 change');
      ...
      expect(zhBuild).toContain('`node "$Coda_STATE" set <name> subagent_dispatch confirmed`');
      expect(zhBuild).toContain(
        '用户选择改用主窗口执行后，必须先运行 `node "$Coda_STATE" set <name> build_mode executing-plans`',
      );
      expect(zhBuild).not.toContain('使用 Skill 工具加载对应技能');
      expect(zhBuild).toContain('tdd_mode');
      expect(zhBuild).toContain('`node "$Coda_STATE" set <name> tdd_mode <tdd|direct>`');
      expect(zhBuild).toContain('若 `tdd_mode: tdd`');
      expect(zhBuild).toContain(
        'TDD 约束和证据门槛已在 `Coda/reference/subagent-dispatch.md` 中定义',
      );
      expect(zhCoda).toContain('`tdd_mode`');
      expect(zhCoda).toContain('full workflow 离开 build 阶段前 `tdd_mode` 必须已选择');
      expect(zhHotfix).toContain('立即使用 Skill 工具加载 `Coda-design` skill');
      expect(zhTweak).toContain('立即使用 Skill 工具加载 `Coda-design` skill');
      expect(zhVerify).toContain(
        '用户选择 B 后，运行 `node "$Coda_STATE" transition <change-name> verify-fail`，然后调用 `/Coda-build`',
      );
      ...
      expect(zhBuild).toContain('`Coda/reference/debug-gate.md`');
      expect(zhBuild).toContain(
        '运行程序、测试、构建或手动验证时出现崩溃、异常行为、测试失败或构建失败',
      );
      expect(zhHotfix).toContain('必须使用 Skill 工具加载 Superpowers `systematic-debugging` 技能');
      expect(zhHotfix).toContain('`Coda/reference/debug-gate.md`');
      expect(zhTweak).toContain('`Coda/reference/debug-gate.md`');
      ...
      expect(
        [zhCoda, zhDesign, zhBuild, zhVerify, zhArchive, zhHotfix, zhTweak].join('\n'),
      ).not.toContain('AskUserQuestion');
      expect(zhCoda).toContain('`auto_transition`');
      expect(zhCoda).toContain('不影响 phase 推进');
      expect(zhCodaRule).toContain(
        'brainstorming in progress: incrementally update brainstorm-summary.md',
      );
      expect(zhCodaRule).toContain('active compaction gate');
      expect(zhCodaRule).toContain(
        '使用 Skill 工具重新加载 Superpowers `subagent-driven-development` 技能',
      );
      expect(zhCodaRule).toContain(
        '读取 `Coda/reference/subagent-dispatch.md` 获取 Coda 专属扩展',
      );
      expect(zhCodaRule).toContain('禁止在主会话中直接执行 task');
      for (const [content] of [
        [zhOpen, '/Coda-design'],
        [zhDesign, '/Coda-build'],
        [zhBuild, '/Coda-verify'],
        [zhVerify, '/Coda-archive'],
      ] as const) {
        expect(content).toContain('自动衔接下一阶段');
        expect(content).toContain('node "$Coda_STATE" next <change-name>');
      ...
      expect(zhHotfix).toContain('node "$Coda_STATE" next <name>');
      expect(zhHotfix).toContain('`NEXT: auto`');
      expect(zhHotfix).toContain(
        '`phase: build` 返回 `Coda-hotfix`，`verify` 返回 `Coda-verify`，`archive` 返回 `Coda-archive`',
      );
      expect(zhTweak).toContain('自动衔接下一阶段');
      expect(zhTweak).toContain('node "$Coda_STATE" next <name>');
      expect(zhTweak).toContain('`NEXT: auto`');
      expect(zhTweak).toContain(
        '`phase: build` 返回 `Coda-tweak`，`verify` 返回 `Coda-verify`，`archive` 返回 `Coda-archive`',
      );
    });
  });

  describe('English Coda workflow safeguards', () => {
    it('matches the Chinese workflow decision-point requirements', async () => {
      const enCoda = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'SKILL.md'),
        'utf-8',
      );
      const enOpen = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-open', 'SKILL.md'),
      ...
      const enDesign = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-design', 'SKILL.md'),
        'utf-8',
      );
      const enBuild = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-build', 'SKILL.md'),
        'utf-8',
      );
      const enVerify = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-verify', 'SKILL.md'),
        'utf-8',
      );
      const enArchive = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-archive', 'SKILL.md'),
        'utf-8',
      );
      const enHotfix = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-hotfix', 'SKILL.md'),
        'utf-8',
      );
      const enTweak = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda-tweak', 'SKILL.md'),
        'utf-8',
      );
      const enCodaRule = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'rules', 'Coda-phase-guard.md'),
        'utf-8',
      );
      const enDecisionPoint = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'reference', 'decision-point.md'),
        'utf-8',
      );
      const enDebugGate = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'reference', 'debug-gate.md'),
        'utf-8',
      );

      expect(enCoda).toContain('Decision points are blocking points');
      ...
      expect(enOpen).toContain(
        'Full `/Coda` workflow must not use the Skill tool to load the `openspec-propose` skill',
      );
      expect(enOpen).toContain('`Coda/reference/decision-point.md`');
      expect(enOpen).toContain(
      ...
      expect(enDesign).toContain(
        'must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to explicitly confirm',
      );
      expect(enDesign).toContain(
      ...
      expect(enBuild).toContain('`Coda/reference/decision-point.md`');
      expect(enVerify).toContain(
        'must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to decide whether to fix or accept the deviation',
      );
      expect(enVerify).toContain(
        'Must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to choose branch handling method',
      );
      expect(enVerify).toContain(
      ...
      expect(enTweak).toContain(
        "Full `/Coda` or `workflow: full` must not use tweak's `openspec-apply-change` build path",
      );
      ...
      expect(enArchive).toContain(
        'Must not run `node "$Coda_ARCHIVE" "<change-name>"` before user confirmation',
      );
      expect(enArchive).toContain('`Coda/reference/decision-point.md`');
      ...
      expect(enArchive).toContain('`node "$Coda_STATE" transition <change-name> archive-reopen`');
      expect(enVerify).toContain('Must not automatically archive just because verification passed');
      expect(enHotfix).toContain(
        'must pause per `Coda/reference/decision-point.md` and delegate the decision to the user',
      );
      expect(enHotfix).toContain('Do not directly enter `/Coda-design`');
      expect(enTweak).toContain(
        'must pause per `Coda/reference/decision-point.md` and delegate the decision to the user',
      );
      expect(enTweak).toContain('Do not directly enter `/Coda-design`');
      expect(enTweak).toContain('`Coda/reference/debug-gate.md`');
      expect(enCoda).toContain(
        '`verify_result: fail` → Enter verification failure decision blocking point',
      );
      expect(enCoda).not.toContain(
        '`verify_result: fail` → `node "$Coda_STATE" transition <name> verify-fail` then `/Coda-build`',
      );
      ...
      expect(enHotfix).toContain(
        'verify phase (Coda-verify) verification-failure and branch-handling decisions',
      );
      expect(enTweak).toContain(
        'verify phase (Coda-verify) verification-failure and branch-handling decisions',
      );
      ...
      expect(enCoda).toContain('first check `build_pause`, `plan`, `build_mode`, and `isolation`');
      expect(enCoda).toContain('`build_pause: plan-ready` and the plan file exists');
      expect(enCoda).toContain(
      ...
      expect(enHotfix).toContain(
        'workspace isolation and execution-method selection when tasks exceed 3 and transfer to `/Coda-build`',
      );
      expect(enBuild).toContain(
        'Must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to explicitly choose',
      );
      expect(enBuild).toContain(
        'must follow the `Coda/reference/decision-point.md` protocol to pause and wait for the user to decide whether to split into a new change',
      );
      expect(enVerify).toContain(
        'Implementation matches `openspec/changes/<name>/design.md` high-level design decisions',
      );
      expect(enBuild).toContain('create independent change through `/Coda-open`');
      ...
      expect(enOpen).toContain(
        'Every accepted split item must be created as an independent change through `/Coda-open`',
      );
      ...
      expect(enOpen).toContain(
        'In batch split mode, a single split item must not auto-advance to `/Coda-design` after completing the open phase',
      );
      expect(enOpen).toContain(
      ...
      expect(enDesign).toContain(
        'The script reads the change `.coda.yaml` `context_compression` snapshot',
      );
      ...
      expect(enDesign).toContain('openspec/changes/<name>/.coda/handoff/spec-context.md');
      ...
      expect(enHotfix).toContain('Immediately use the Skill tool to load the `Coda-design` skill');
      expect(enTweak).toContain('Immediately use the Skill tool to load the `Coda-design` skill');
      expect(enVerify).toContain(
        'After user selects B, run `node "$Coda_STATE" transition <change-name> verify-fail`, then invoke `/Coda-build`',
      );

      expect(enCoda).toContain(
        'User explicitly describes a lightweight/medium change that can fit in a single OpenSpec change',
      );
      expect(enCoda).toContain('executed through OpenSpec apply');
      expect(enCoda).not.toContain(
      ...
      expect(enBuild).toContain('`Coda/reference/debug-gate.md`');
      expect(enBuild).toContain(
      ...
      expect(enHotfix).toContain('`Coda/reference/debug-gate.md`');
      expect(enDebugGate).toContain(
        'do not replace the current change verification loop by starting a separate “write test cases” change',
      );

      expect(
        [enCoda, enOpen, enDesign, enBuild, enVerify, enArchive, enHotfix, enTweak].join('\n'),
      ).not.toContain('AskUserQuestion');
      expect(enCoda).toContain('`Coda/reference/decision-point.md`');
      expect(enCoda).toContain('`auto_transition`');
      expect(enCoda).toContain('does not block phase updates');
      expect(enCodaRule).toContain(
        'brainstorming in progress: incrementally update brainstorm-summary.md',
      );
      expect(enCodaRule).toContain('active compaction gate');
      expect(enCodaRule).toContain(
        'Use the Skill tool to reload the Superpowers `subagent-driven-development` skill',
      );
      expect(enCodaRule).toContain(
        're-read `Coda/reference/subagent-dispatch.md` for Coda-specific extensions',
      );
      expect(enCodaRule).toContain('Do not execute the pending task directly in the main window');
      for (const [content] of [
        [enOpen, '/Coda-design'],
        [enDesign, '/Coda-build'],
        [enBuild, '/Coda-verify'],
        [enVerify, '/Coda-archive'],
      ] as const) {
      ...
      expect(enHotfix).toContain('node "$Coda_STATE" next <name>');
      expect(enHotfix).toContain('`NEXT: auto`');
      expect(enHotfix).toContain(
        '`phase: build` returns `Coda-hotfix`, `verify` returns `Coda-verify`, `archive` returns `Coda-archive`',
      );
      expect(enTweak).toContain('Automatic Handoff to Next Phase');
      expect(enTweak).toContain('node "$Coda_STATE" next <name>');
      expect(enTweak).toContain('`NEXT: auto`');
      expect(enTweak).toContain(
        '`phase: build` returns `Coda-tweak`, `verify` returns `Coda-verify`, `archive` returns `Coda-archive`',
      );
    });
  });

  describe('Coda output language safeguards', () => {
    it('requires OpenSpec and Superpowers outputs to follow the user request language', async () => {
      ...
      expect(zhSkills.Coda).toContain('输出语言规则');
      expect(zhSkills.Coda).toContain('以触发本次工作流的用户请求语言作为默认输出语言');
      expect(zhSkills['Coda-open']).toContain(
        '传递给 OpenSpec 的所有提问和产物要求都必须包含输出语言约束',
      );
      expect(zhSkills['Coda-design']).toContain('Language: 使用触发本次工作流的用户请求语言输出');
      expect(zhSkills['Coda-build']).toContain(
        '计划文件和执行反馈必须使用触发本次工作流的用户请求语言',
      );
      expect(zhSkills['Coda-build']).toContain('ARGUMENTS 必须包含与 Step 1 相同的 Language 约束');
      expect(zhSkills['Coda-verify']).toContain(
        '验证报告和分支处理说明必须使用触发本次工作流的用户请求语言',
      );
      expect(zhSkills['Coda-archive']).toContain(
        '归档摘要和生命周期闭环说明必须使用触发本次工作流的用户请求语言',
      );
      expect(zhSkills['Coda-hotfix']).toContain(
        '精简版 OpenSpec 产物必须使用触发本次工作流的用户请求语言',
      );
      expect(zhSkills['Coda-tweak']).toContain(
        '精简版 OpenSpec 产物必须使用触发本次工作流的用户请求语言',
      );

      expect(enSkills.Coda).toContain('Output Language Rule');
      expect(enSkills.Coda).toContain(
        'Use the language of the user request that triggered this workflow as the default output language',
      );
      expect(enSkills['Coda-open']).toContain(
        'Every prompt and artifact request passed to OpenSpec must include the output-language constraint',
      );
      expect(enSkills['Coda-design']).toContain(
        'Language: Use the language of the user request that triggered this workflow',
      );
      expect(enSkills['Coda-build']).toContain(
      ...
      expect(enSkills['Coda-verify']).toContain(
        'Verification reports and branch-handling notes must use the language of the user request that triggered this workflow',
      );
      expect(enSkills['Coda-archive']).toContain(
        'Archive summaries and lifecycle closure notes must use the language of the user request that triggered this workflow',
      );
      expect(enSkills['Coda-hotfix']).toContain(
        'Streamlined OpenSpec artifacts must use the language of the user request that triggered this workflow',
      );
      expect(enSkills['Coda-tweak']).toContain(
        'Streamlined OpenSpec artifacts must use the language of the user request that triggered this workflow',
      );
    });
  });

  describe('Coda build subagent dispatch safeguards', () => {
    it('composes the Superpowers loop with the Chinese Coda dispatch contract', async () => {
      const zhBuild = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda-build', 'SKILL.md'),
        'utf-8',
      );
      const zhDispatch = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda', 'reference', 'subagent-dispatch.md'),
        'utf-8',
      );
      const zhRecovery = await fs.readFile(
        path.resolve('assets', 'skills-zh', 'Coda', 'reference', 'context-recovery.md'),
        'utf-8',
      );
      ...
      expect(zhBuild).toContain('读取 `Coda/reference/subagent-dispatch.md` 获取 Coda 专属扩展');
      expect(zhBuild).not.toContain('#### Subagent 调度协议');
      expect(zhDispatch).toContain('发生冲突时，以本文档中更具体的 Coda 约束为准');
      ...
      expect(zhDispatch).toContain('openspec/changes/<name>/.Coda/subagent-progress.md');
      ...
      expect(zhDispatch).toContain(
        'node "$Coda_STATE" task-checkoff "$PLAN_FILE" "$PLAN_TASK_TEXT"',
      );
      ...
      expect(zhDispatch).toContain('返回 `Coda-build` 继续执行退出条件、阶段守卫和后续阶段衔接');
      expect(zhRecovery).toContain('重新加载 Superpowers `subagent-driven-development` 技能');
      expect(zhRecovery).toContain('重新阅读 `Coda/reference/subagent-dispatch.md`');
      expect(zhRecovery).toContain('读取 `openspec/changes/<name>/.Coda/subagent-progress.md`');
      expect(zhGuard).toContain('重新加载 Superpowers `subagent-driven-development` 技能');
      expect(zhGuard).toContain('读取 `Coda/reference/subagent-dispatch.md` 获取 Coda 专属扩展');
      expect(zhGuard).toContain('读取 `openspec/changes/<name>/.Coda/subagent-progress.md`');
    });
    ...
    it('keeps the English dispatch contract behaviorally aligned', async () => {
      ...
      const enDispatch = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'reference', 'subagent-dispatch.md'),
        'utf-8',
      );
      const enRecovery = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'reference', 'context-recovery.md'),
        'utf-8',
      );
      const enGuard = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'rules', 'Coda-phase-guard.en.md'),
        'utf-8',
      );
      ...
      expect(enBuild).toContain(
        'read `Coda/reference/subagent-dispatch.md` for Coda-specific extensions',
      );
      expect(enBuild).toContain(
        'TDD constraints and evidence thresholds are defined in `Coda/reference/subagent-dispatch.md`',
      );
      ...
      expect(enDispatch).toContain(
        'If the Superpowers skill conflicts with this document, the more specific Coda constraints here take precedence',
      );
      ...
      expect(enDispatch).toContain('openspec/changes/<name>/.Coda/subagent-progress.md');
      ...
      expect(enDispatch).toContain(
        'return control to `Coda-build` for exit checks, the phase guard, and phase handoff',
      );
      expect(enRecovery).toContain('reload the Superpowers `subagent-driven-development` skill');
      expect(enRecovery).toContain('Re-read `Coda/reference/subagent-dispatch.md`');
      expect(enRecovery).toContain('Read `openspec/changes/<name>/.Coda/subagent-progress.md`');
      expect(enGuard).toContain('reload the Superpowers `subagent-driven-development` skill');
      expect(enGuard).toContain(
        'Re-read `Coda/reference/subagent-dispatch.md` for Coda-specific extensions',
      );
      expect(enGuard).toContain('Read `openspec/changes/<name>/.Coda/subagent-progress.md`');
  ...
  describe('Coda phase guard rules', () => {
    ...
    it('delegates post-guard handoff to Coda-state next so auto_transition is honored', async () => {
      const zhGuard = await fs.readFile(
        path.resolve('assets', 'skills', 'Coda', 'rules', 'Coda-phase-guard.md'),
        'utf-8',
      );
      ...
      expect(zhSection).toContain('Coda-state next <change-name>');
      ...
      expect(zhSection).not.toContain('open → `Coda-design`');

      const enSection = section(enGuard, '## Automatic Transition After Phase Exit');
      expect(enSection).toContain('Coda-state next <change-name>');
      ...
      expect(enSection).not.toContain('open → `Coda-design`');
    });
  });
  ...
  describe('Coda script discovery helper', () => {
    it('ships a shared script locator helper', async () => {
      const manifest = await readManifest();
      expect(manifest.skills).toContain('Coda/scripts/Coda-env.mjs');
    });

    it('keeps review_mode wired through state and schema scripts', async () => {
      const stateScript = await fs.readFile(
        path.resolve('domains', 'Coda-classic', 'classic-state-command.ts'),
        'utf-8',
      );
      const guardScript = await fs.readFile(
        path.resolve('domains', 'Coda-classic', 'classic-guard.ts'),
        'utf-8',
      );
      const validateScript = await fs.readFile(
        path.resolve('domains', 'Coda-classic', 'classic-validate-command.ts'),
        'utf-8',
      );
    ...
    it('keeps platform search roots out of English and Chinese skill prose', async () => {
      ...
      const skillPaths = manifest.skills.filter(
        (skillPath) =>
          ...
          (skillPath === 'Coda/SKILL.md' ||
            skillPath.startsWith('Coda-') ||
            skillPath.startsWith('Coda-any/')),
      );
      ...
      for (const languageDir of ['skills', 'skills-zh']) {
        for (const skillPath of skillPaths) {
          ...
          if (!content.includes('Coda_STATE') && !content.includes('Coda_GUARD')) continue;
          ...
          const isMainEntry = skillPath === 'Coda/SKILL.md';

          if (!isMainEntry) {
            expect(content, `${languageDir}/${skillPath} should use Coda-env.mjs`).toContain(
              'Coda-env.mjs',
            );
            expect(
              content,
              `${languageDir}/${skillPath} should resolve Coda_ENV via node`,
            ).toContain('node "$Coda_ENV"');
            expect(
              content,
            ...
            ).toContain('Coda/reference/scripts.md');
          }
    ...
    it('uses node (not bash) in shipped Coda command examples', async () => {
      const manifest = await readManifest();
      const skillPaths = manifest.skills.filter(
        (skillPath) =>
          skillPath.endsWith('SKILL.md') &&
          (skillPath === 'Coda/SKILL.md' || skillPath.startsWith('Coda-')),
      );
      ...
      for (const languageDir of ['skills', 'skills-zh']) {
        for (const skillPath of skillPaths) {
          ...
          expect(
            content,
            `${languageDir}/${skillPath} should avoid raw bash for Coda scripts`,
          ).not.toMatch(/(^|[` \t])bash[ \t]+"?\$Coda_/m);
        }
      }
    });

    it('keeps the Coda_ENV locator block identical across shipped skills', async () => {
      const manifest = await readManifest();
      const skillPaths = manifest.skills.filter(
        (skillPath) =>
          skillPath.endsWith('SKILL.md') &&
      ...
      const extractLocatorBlock = (content: string) => {
        const start = content.indexOf('Coda_ENV="${Coda_ENV:-$(find .');
        const end = content.indexOf('node "$Coda_ENV"');

        expect(start).toBeGreaterThanOrEqual(0);
        expect(end).toBeGreaterThan(start);

        return content.slice(start, end + 'node "$Coda_ENV"'.length);
      };
      ...
      for (const languageDir of ['skills', 'skills-zh']) {
        ...
        for (const skillPath of skillPaths) {
          ...
          if (!content.includes('Coda_ENV="${Coda_ENV:-$(find .')) continue;
    ...
    it('ships every Coda reference doc that skill prose points to', async () => {
      const manifest = await readManifest();
      ...
      for (const languageDir of ['skills', 'skills-zh']) {
        for (const skillPath of skillPaths) {
          ...
          const references =
            content.match(/(?:Coda|Coda-any)\/reference\/(?:subagents\/)?[a-z-]+\.md/g) ?? [];
  ...
  describe('mergeProjectConfig', () => {
    it('creates config with defaults when no file exists', async () => {
      ...
      const content = await fs.readFile(path.join(tmpDir, '.Coda', 'config.yaml'), 'utf-8');
    ...
    it('preserves existing user values and fills missing managed fields', async () => {
      const configDir = path.join(tmpDir, '.Coda');
  ...
  describe('createWorkingDirs with config merge', () => {
    it('merges config on second call instead of skipping', async () => {
      await createWorkingDirs(tmpDir);
      const configPath = path.join(tmpDir, '.Coda', 'config.yaml');
