...
describe('engine state projection', () => {
  ...
  it('creates .coda/run-state.json when the state file is absent', async () => {
    await writeRunState(changeDir, state());

    expect(await readRunState(changeDir)).toEqual(state());
  });

  it('does not replace .coda/run-state.json when writing fails', async () => {
  ...
  it('rejects incomplete Run projections', async () => {
    await fs.mkdir(path.join(changeDir, '.coda'), { recursive: true });
    await fs.writeFile(
