...
describe('Skill Engine schema compatibility', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-engine-'));
    ...
    stateScript = path.join(root, 'assets', 'coda-state.mjs');
    validateScript = path.join(root, 'assets', 'coda-yaml-validate.mjs');
  });
  ...
  async function setYamlField(field: string, value: string): Promise<void> {
    const yamlPath = path.join(root, 'openspec', 'changes', 'demo', '.coda.yaml');
