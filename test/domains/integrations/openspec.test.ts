...
describe('openspec', () => {
  ...
  describe('installOpenSpec', () => {
    it('accepts the Kimi Code OpenSpec tool id from platform definitions', async () => {
      ...
      const result = await installOpenSpec('/tmp/test', ['kimicode'], 'project');
    ...
    it('copies OpenSpec opencode output into MimoCode project paths', async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-mimocode-openspec-'));
      try {
    ...
    it('writes the default OpenSpec config under XDG_CONFIG_HOME on non-Windows platforms', async () => {
      ...
      const xdgConfigHome = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-openspec-xdg-'));
    ...
    it('removes a default OpenSpec config backup when writing the replacement config fails', async () => {
      ...
      mockedExecFileSync.mockReturnValueOnce(Buffer.from('ok'));
      vi.spyOn(os, 'platform').mockReturnValue('linux');
      const xdgConfigHome = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-openspec-backup-'));
      vi.stubEnv('XDG_CONFIG_HOME', xdgConfigHome);
    ...
    it('cleans up the temporary OpenSpec profile directory if config creation fails', async () => {
      ...
      mockedExecFileSync.mockReturnValueOnce(Buffer.from('upgraded'));
      mockedExecFileSync.mockReturnValueOnce(Buffer.from('/usr/bin/openspec'));
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-openspec-test-'));
    ...
    it('merges with existing content in ~/.config/opencode/ without overwrite errors', async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-migrate-test-'));
      ...
      fs.mkdirSync(path.join(correctSkillsDir, 'coda'), { recursive: true });
      fs.writeFileSync(path.join(correctSkillsDir, 'coda', 'SKILL.md'), 'coda skill');
      ...
      expect(fs.readFileSync(path.join(correctSkillsDir, 'coda', 'SKILL.md'), 'utf-8')).toBe(
        'coda skill',
      );
      expect(
    ...
    it('integrates with installOpenSpec for global scope with opencode tool', async () => {
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coda-install-test-'));
