...
async function seedArchiveChange(dir: string): Promise<string> {
  ...
  run(dir, ['state', 'set', 'demo', 'phase', 'archive'], { CODA_FORCE_PHASE: '1' });
...
describe('Classic archive command', () => {
  ...
  it('archives a verified change and completes its Run transaction', async () => {
    ...
    const result = run(dir, ['archive', 'demo'], { CODA_OPENSPEC: fake.command });
    ...
    const state = parse(await fs.readFile(path.join(archiveDir, '.coda.yaml'), 'utf8')) as Record<
      string,
      unknown
    >;
  ...
  it('treats a completed archive retry as an idempotent no-op', async () => {
    ...
    await seedArchiveChange(dir);
    const fake = await fakeOpenSpec(dir, 'success');
    expect(run(dir, ['archive', 'demo'], { CODA_OPENSPEC: fake.command }).status).toBe(0);
    const archiveDir = path.join(
      dir,
      'openspec',
      'changes',
    ...
    const stateFile = path.join(archiveDir, '.coda.yaml');
  ...
  it('keeps a recoverable pending marker when OpenSpec fails before moving files', async () => {
    ...
    const state = parse(await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8')) as Record<
      string,
      unknown
    >;
  ...
  it('reconciles an archive that moved before the external process was interrupted', async () => {
    ...
    expect(run(dir, ['archive', 'demo'], { CODA_OPENSPEC: interrupted.command }).status).toBe(9);
    const logBeforeRetry = await fs.readFile(interrupted.log, 'utf8');
    const retry = await fakeOpenSpec(dir, 'success');

    const result = run(dir, ['archive', 'demo'], { CODA_OPENSPEC: retry.command });
