...
import {
  readClassicState,
  writeClassicState,
} from '../../../domains/coda-classic/classic-store.js';
...
function classicState(): ClassicState {
  return {
    ...
    handoffContext: '.coda/handoff/context.json',
...
describe('Classic state projection', () => {
  ...
  beforeEach(async () => {
    changeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-classic-state-'));
    stateFile = path.join(changeDir, '.coda.yaml');
  });
