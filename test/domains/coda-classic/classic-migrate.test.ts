...
describe('Classic legacy migration', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-classic-migrate-'));
  ...
  it('imports handoff context and delegated progress into Run storage', async () => {
    const handoff = 'openspec/changes/demo/.coda/handoff/context.json';
  ...
  it('is byte-idempotent and does not duplicate migration events', async () => {
    ...
    const yamlBefore = await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8');
    ...
    expect(await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8')).toBe(yamlBefore);
  ...
  it('leaves state unchanged when the Skill snapshot cannot be created', async () => {
    ...
    expect(await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8')).toBe(before);
    await expect(fs.access(path.join(changeDir, '.coda'))).rejects.toMatchObject({
      code: 'ENOENT',
    });
  });
