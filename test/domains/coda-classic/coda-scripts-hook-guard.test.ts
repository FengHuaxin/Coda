/**
 * Hook guard tests — split from coda-scripts.test.ts for maintainability.
 *
 * Tests the PreToolUse hook guard (coda-hook-guard.mjs) that blocks or allows
...
describe('hook guard', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-hook-guard-'));
    hookGuardScript = path.resolve(scriptsDir, 'coda-hook-guard.mjs');
