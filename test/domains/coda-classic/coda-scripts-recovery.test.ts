/**
 * Recovery tests — split from coda-scripts.test.ts for maintainability.
 *
...
describe('check --recover', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-recovery-'));
  ...
  it('outputs recovery context for archive phase', async () => {
    ...
    expect(result.stdout).toContain('Recovery action: Run /Coda-archive to complete archiving.');
  });
});
