...
const chinesePackageRoot = path.resolve('assets', 'skills-zh', 'Coda-classic');
const englishPackageRoot = path.resolve('assets', 'skills', 'Coda-classic');
const stableSteps = [
  'full.open',
  'full.design.handoff',
...
describe('Chinese Coda-classic package', () => {
  ...
  it('declares every invoked public Skill and never invokes itself', async () => {
    ...
    expect(invoked).not.toContain('Coda-classic');
  });
  ...
  it('documents its internal-only and fail-closed contract in Chinese', async () => {
    ...
    expect(source).toContain('name: Coda-classic');
...
describe('English Coda-classic package', () => {
