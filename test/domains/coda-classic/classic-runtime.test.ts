...
describe('Classic runtime bundle', () => {
  it('runs without dist or node_modules and exposes JSON diagnostics', async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-runtime-'));
    temporaryDirectories.push(directory);
    const isolatedRuntime = path.join(directory, 'coda-runtime.mjs');
  ...
  it('is fresh and listed in the shipped manifest', async () => {
    ...
    expect(manifest.skills).toContain('Coda/scripts/Coda-runtime.mjs');
  });

  it('executes state and validation commands from a standalone project', async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-runtime-state-'));
    temporaryDirectories.push(directory);
    ...
    expect(next.stdout).toContain('SKILL: Coda-design');
  ...
  it('keeps task-checkoff validation in the TypeScript state command', async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-runtime-task-'));
    temporaryDirectories.push(directory);
  ...
  it('rejects direct writes to machine-owned Run fields', async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-runtime-owned-'));
    temporaryDirectories.push(directory);
  ...
  it('re-resolves the Run step when migrated Classic configuration changes', async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-runtime-sync-'));
    temporaryDirectories.push(directory);
    spawnSync(process.execPath, [runtime, 'state', 'init', 'demo', 'full'], {
      cwd: directory,
      encoding: 'utf8',
    ...
    spawnSync(process.execPath, [runtime, 'state', 'set', 'demo', 'phase', 'build'], {
      cwd: directory,
      encoding: 'utf8',
      env: { ...process.env, CODA_FORCE_PHASE: '1' },
    });
