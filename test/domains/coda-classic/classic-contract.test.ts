...
// behavior (stdout/stderr/exit/.coda.yaml), not the invocation mechanism.
...
const activeScripts = path.resolve('assets', 'skills', 'coda', 'scripts');
...
const FROZEN_VARIANT: ScriptVariant = {
  ...
  state: 'coda-state.sh',
  guard: 'coda-guard.sh',
  handoff: 'coda-handoff.sh',
  hookGuard: 'coda-hook-guard.sh',
  executor: 'bash',
};

const ACTIVE_VARIANT: ScriptVariant = {
  names: activeScriptNames,
  state: 'coda-state.mjs',
  guard: 'coda-guard.mjs',
  handoff: 'coda-handoff.mjs',
  hookGuard: 'coda-hook-guard.mjs',
  executor: 'node',
};
...
async function copyScripts(source: string, destination: string, names: string[]): Promise<void> {
  ...
  try {
    await fs.copyFile(
      path.join(source, 'coda-runtime.mjs'),
      path.join(destination, 'coda-runtime.mjs'),
    );
  } catch (error) {
...
function runScript(
  ...
  const env = {
    ...process.env,
    CODA_CLASSIC_SKILL_ROOT: path.resolve('assets', 'skills', 'coda-classic'),
  };
...
async function observeState(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `coda-classic-${profile}-`));
  ...
  const result = followUp
    ...
    : init;
  const yamlPath = path.join(root, 'openspec', 'changes', name, '.coda.yaml');
...
async function observeGuard(
  ...
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `coda-guard-${profile}-`));
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
...
async function observeHandoff(
  ...
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `coda-handoff-${profile}-`));
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
  ...
  const yaml = parse(await fs.readFile(path.join(changeDir, '.coda.yaml'), 'utf8')) as Record<
    string,
    unknown
  >;
...
async function observeHook(sourceScripts: string): Promise<GuardObservation> {
  const variant = variantOf(sourceScripts);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-hook-'));
  temporaryRoots.push(root);
  const scripts = path.join(root, 'scripts');
  await copyScripts(sourceScripts, scripts, variant.names);
...
describe('frozen Classic 0.3.9 reference', () => {
  it('records the source commit and checksums for every compatibility script', async () => {
    ...
    const checksums = JSON.parse(
      (await fs.readFile(path.join(referenceRoot, 'checksums.json'), 'utf8')).replace(
        /^\uFEFF/u,
        '',
      ),
