...
import {
  ...
  evidenceSatisfied,
} from '../../../domains/coda-classic/classic-evidence.js';
...
describe('Classic evidence collection', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-classic-evidence-'));
    ...
    projection = {
      classic: {
        ...
        buildCommand: null,
        verifyCommand: null,
        handoffContext: 'openspec/changes/demo/.coda/handoff/context.json',
  ...
  it('collects structured evidence with stable codes and source paths', async () => {
    await Promise.all([
      ...
      writeProjectFile('openspec/changes/demo/.coda/handoff/context.json', '{"context":true}\n'),
      fs
        .mkdir(path.join(changeDir, '.coda'), { recursive: true })
        .then(() => fs.writeFile(path.join(changeDir, '.coda', 'checkpoint.json'), '{}\n')),
    ]);
