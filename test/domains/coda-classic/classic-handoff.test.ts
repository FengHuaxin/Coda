...
describe('Classic handoff command', () => {
  it('writes a compact design handoff and records the context fields', async () => {
    ...
    expect(result.stderr).toContain(
      '[HANDOFF] wrote openspec/changes/demo/.Coda/handoff/design-context.json',
    );
    expect(result.stderr).toMatch(/\[HANDOFF\] handoff_hash=[a-f0-9]{64}/);

    const md = await fs.readFile(
      path.join(changeDir, '.Coda', 'handoff', 'design-context.md'),
      'utf8',
    );
    expect(md).toContain('Generated-by: Coda-handoff.mjs');
    ...
    expect(run(dir, 'state', 'get', 'demo', 'handoff_context').stdout.trim()).toBe(
      'openspec/changes/demo/.Coda/handoff/design-context.json',
    );
    ...
    expect(artifacts).toMatchObject({
      handoff_context: 'openspec/changes/demo/.Coda/handoff/design-context.json',
      handoff_markdown: 'openspec/changes/demo/.Coda/handoff/design-context.md',
    });
  ...
  it('computes and prints the hash without writing files in --hash-only mode', async () => {
    ...
    expect(existsSync(path.join(changeDir, '.coda', 'handoff'))).toBe(false);
  });
