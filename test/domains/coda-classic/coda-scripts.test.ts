...
const scriptsDir = path.resolve('assets', 'skills', 'Coda', 'scripts');
const classicSkillRoot = path.resolve('assets', 'skills', 'Coda-classic');

// Forward-slash absolute path — used for the fake OpenSpec shim (a bash script)
// and the Coda_OPENSPEC env var it is passed through, both of which need a
...
function runNode(
  ...
  return spawnSync(process.execPath, [script, ...args], {
    cwd,
    encoding: 'utf-8',
    env: { ...process.env, Coda_CLASSIC_SKILL_ROOT: classicSkillRoot, ...env },
    timeout,
  });
}
...
async function createChange(tmpDir: string, name: string, yaml: string, tasks = '- [x] done\n') {
  ...
  await writeFile(path.join(changeDir, '.Coda.yaml'), yaml);
...
async function createFakeOpenSpecArchive(
  ...
  // to assign to Coda_OPENSPEC — already shaped so the runtime's
...
describe('Coda script contracts', () => {
  it('keeps all Classic launchers as runtime-only Node facades', async () => {
    const sources: Record<string, string> = {
      state: await fs.readFile(path.join(scriptsDir, 'Coda-state.mjs'), 'utf-8'),
      validate: await fs.readFile(path.join(scriptsDir, 'Coda-yaml-validate.mjs'), 'utf-8'),
      guard: await fs.readFile(path.join(scriptsDir, 'Coda-guard.mjs'), 'utf-8'),
      handoff: await fs.readFile(path.join(scriptsDir, 'Coda-handoff.mjs'), 'utf-8'),
      archive: await fs.readFile(path.join(scriptsDir, 'Coda-archive.mjs'), 'utf-8'),
      'hook-guard': await fs.readFile(path.join(scriptsDir, 'Coda-hook-guard.mjs'), 'utf-8'),
    };

    for (const [command, source] of Object.entries(sources)) {
      expect(source).toContain("import { main } from './Coda-runtime.mjs'");
  ...
  it('keeps Coda-hook-guard blocked messages in English', async () => {
    const hookSource = await fs.readFile(
      path.resolve('domains', 'Coda-classic', 'classic-hook-guard.ts'),
      'utf-8',
    );
...
describe('Coda scripts', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `Coda-scripts-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    guardScript = path.join(tmpScriptsDir, 'Coda-guard.mjs');
    stateScript = path.join(tmpScriptsDir, 'Coda-state.mjs');
    validateScript = path.join(tmpScriptsDir, 'Coda-yaml-validate.mjs');
    hookGuardScript = path.join(tmpScriptsDir, 'Coda-hook-guard.mjs');
  });
  ...
  it('initializes a new change directory with workflow defaults', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'new-full-change', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes build_pause as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'pause-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes subagent_dispatch as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'subagent-dispatch-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as null for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as standard for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes review_mode as off for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-hotfix', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as direct for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-hotfix', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes context_compression as off by default', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('snapshots beta context compression from .Coda/config.yaml when initializing a change', async () => {
    await writeFile(path.join(tmpDir, '.Coda', 'config.yaml'), 'context_compression: beta\n');

    const result = runNode(tmpDir, stateScript, ['init', 'context-beta', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-beta', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('snapshots review_mode from .Coda/config.yaml when initializing a full change', async () => {
    await writeFile(path.join(tmpDir, '.Coda', 'config.yaml'), 'review_mode: standard\n');

    const result = runNode(tmpDir, stateScript, ['init', 'review-standard', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'review-standard', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('rejects invalid review_mode from .Coda/config.yaml when initializing a change', async () => {
    await writeFile(path.join(tmpDir, '.Coda', 'config.yaml'), 'review_mode: noisy\n');
  ...
  it('lets Coda_CONTEXT_COMPRESSION override the project context compression default', async () => {
    await writeFile(path.join(tmpDir, '.Coda', 'config.yaml'), 'context_compression: beta\n');

    const result = runNode(tmpDir, stateScript, ['init', 'context-env', 'full'], {
      Coda_CONTEXT_COMPRESSION: 'off',
    });
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-env', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition as true when openspec Coda config is absent', async () => {
    const result = runNode(tmpDir, stateScript, ['init', 'auto-transition-defaults', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-defaults', '.Coda.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition from .Coda/config.yaml when set to false', async () => {
    await fs.mkdir(path.join(tmpDir, '.Coda'), { recursive: true });
    await writeFile(
      path.join(tmpDir, '.Coda', 'config.yaml'),
      'context_compression: off\nauto_transition: false\n',
    );

    const result = runNode(tmpDir, stateScript, ['init', 'auto-transition-config-false', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-config-false', '.Coda.yaml'),
      'utf-8',
    );
    const get = runNode(tmpDir, stateScript, [
      'get',
  ...
  it('next resolves auto for full workflow when auto_transition is true', async () => {
    ...
    expect(result.stdout).toContain('SKILL: Coda-verify');
  }, 20_000);
  ...
  it('next resolves manual with hint when auto_transition is false', async () => {
    ...
    expect(result.stdout).toContain('SKILL: Coda-build');
  ...
  it('next maps hotfix and tweak workflows to their preset skills in build phase', async () => {
    ...
    expect(hotfix.stdout).toContain('SKILL: Coda-hotfix');
    expect(tweak.stdout).toContain('SKILL: Coda-tweak');
  }, 20_000);
  ...
  it('next maps each non-build phase to the owning skill', async () => {
    ...
    expect(design.stdout).toContain('SKILL: Coda-design');
    expect(archive.stdout).toContain('SKILL: Coda-archive');
  }, 20_000);

  it('next exits non-zero when .Coda.yaml is missing', async () => {
    const result = runNode(tmpDir, stateScript, ['next', 'next-missing']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('.Coda.yaml not found');
  }, 20_000);
  ...
  it('Coda-env.mjs prints the scripts directory it lives in', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'Coda-env.mjs');
    const result = runNode(tmpDir, envScript);

    expect(result.status).toBe(0);
    const printedDir = result.stdout.trim();
    // Coda-env.mjs prints its own directory — the scripts dir holding every
    // sibling launcher plus the bundled runtime.
    expect(printedDir.endsWith('scripts')).toBe(true);
    for (const sibling of ['Coda-state.mjs', 'Coda-runtime.mjs', 'Coda-hook-guard.mjs']) {
  ...
  it('generates a design handoff and requires minimal design doc linkage before leaving design', async () => {
    const handoffScript = path.join(tmpDir, 'scripts', 'Coda-handoff.mjs');
    await createChange(
      tmpDir,
      'handoff-change',
      [
    ...
    expect(contextPath).toBe('openspec/changes/handoff-change/.Coda/handoff/design-context.json');
  ...
  it('generates a beta spec projection handoff with verbatim spec content', async () => {
    ...
    expect(handoff.status).toBe(0);
    expect(contextPath).toBe('openspec/changes/beta-context/.Coda/handoff/spec-context.json');
    ...
    expect(contextMarkdown).toContain('Generated-by: Coda-handoff.mjs');
  ...
  it('blocks beta design exit when spec-context.json is malformed JSON', async () => {
    ...
    await writeFile(
      ...
      [
        '---',
        'Coda_change: beta-malformed-json',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('reads Coda yaml fields without including trailing comments', async () => {
    const handoffScript = path.join(tmpDir, 'scripts', 'Coda-handoff.mjs');
    const validateScript = path.join(tmpDir, 'scripts', 'Coda-yaml-validate.mjs');
    await createChange(
      tmpDir,
      'commented-yaml',
      [
  ...
  it('accepts design doc frontmatter after a BOM and leading blank lines', async () => {
    ...
    await writeFile(
      ...
      [
        ...
        '',
        '---',
        'Coda_change: frontmatter-prefix',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('warns when --full is passed in beta mode', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/beta-full-warn/.Coda/handoff/spec-context.json');
  }, 20_000);
  ...
  it('blocks design exit when design doc frontmatter is missing required fields', async () => {
    ...
    // Design doc with wrong Coda_change
    await writeFile(
      path.join(tmpDir, 'docs', 'superpowers', 'specs', 'bad-design.md'),
      [
        '---',
        'Coda_change: wrong-change',
        'role: technical-design',
        'canonical_spec: openspec',
        '---',
        '',
  ...
  it('rejects invalid build_pause values during schema validation', async () => {
    ...
    expect(result.stderr).toContain('FATAL: .Coda.yaml schema validation failed');
  }, 20_000);
  ...
  it('uses root-level build command config before inferred build commands', async () => {
    ...
    await writeFile(path.join(tmpDir, 'Coda.yaml'), 'build_command: node root-build-check.js\n');
    await writeFile(
  ...
  it('resolves OpenSpec date-prefixed archive directories from the original change name', async () => {
    ...
    expect(validate.stderr).toContain(
      '[VALIDATE] openspec/changes/archive/2026-05-21-resolved-archive/.Coda.yaml',
    );
  }, 20_000);

  it('reports accurate archive step counts when syncing and annotating', async () => {
    const archiveScript = path.join(tmpDir, 'scripts', 'Coda-archive.mjs');
  ...
  it('scale uses base_ref from .Coda.yaml when plan has no base-ref header', async () => {
    execFileSync('git', ['init'], { cwd: tmpDir, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: tmpDir });
    execFileSync('git', ['config', 'user.name', 'Test User'], { cwd: tmpDir });
    await writeFile(path.join(tmpDir, 'README.md'), 'base\n');
  ...
  it('blocks direct phase writes but allows the Coda_FORCE_PHASE escape hatch', async () => {
    await createChange(
      tmpDir,
      'phase-jump',
      ['workflow: full', 'phase: open', 'design_doc: null', 'archived: false', ''].join('\n'),
  ...
  it('reports error for malformed .Coda.yaml on get', async () => {
    const changeDir = path.join(tmpDir, 'openspec', 'changes', 'bad-yaml');
    await fs.mkdir(changeDir, { recursive: true });
    await writeFile(path.join(changeDir, '.Coda.yaml'), 'workflow: full\nphase: [\n  broken\n');

    const result = runNode(tmpDir, stateScript, ['get', 'bad-yaml', 'phase']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('Invalid .Coda.yaml');
  });
  ...
  describe('check --recover', () => {
    ...
    it('outputs recovery context for design phase with handoff but no design doc', async () => {
      await createChange(
        ...
        [
          ...
          'plan: null',
          'verify_result: pending',
          'handoff_context: openspec/changes/recover-design/.Coda/handoff/design-context.json',
    ...
    it('outputs recovery context for archive phase', async () => {
      ...
      expect(result.stdout).toContain('/Coda-archive');
      expect(result.stdout).toContain('End Recovery Context');
    });
  ...
  describe('review fix: build-complete conditional reset', () => {
    it('preserves verification_report on re-verify cycle (H1)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'reverify-test', '.Coda.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: verify-fail preserves branch_status', () => {
    it('does not reset branch_status on verify-fail (H6)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'branch-preserve', '.Coda.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: command injection prevention', () => {
    it('rejects build_command with shell metacharacters (C3)', async () => {
      ...
      // No Coda_SKIP_BUILD — run_command_string should reject before executing
  ...
  describe('Coda-hook-guard.mjs — phase write guard', () => {
    it('allows all writes when no active Coda change exists', async () => {
    ...
    it('allows new change artifact writes while another change is in archive phase (no state file yet)', async () => {
      ...
      // Brand-new change being created: artifacts written before .Coda.yaml exists
