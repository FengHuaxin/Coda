...
async function makeRepo(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-git-'));
  git(dir, ['init', '-q', '-b', 'main']);
  git(dir, ['config', 'user.email', 'coda@test.local']);
  git(dir, ['config', 'user.name', 'coda Test']);
...
describe('collectGitSnapshot', () => {
  ...
  it('returns null fields and empty lists for a non-git directory', async () => {
    const notRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-notgit-'));
    try {
