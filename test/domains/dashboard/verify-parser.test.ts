...
describe('resolveVerify', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-verify-'));
  });
  ...
  it('detects a verify report at .coda/verify-result.md and extracts a summary', async () => {
    const reportPath = path.join(tmpDir, '.coda', 'verify-result.md');
  ...
  it('rejects traversal paths in verification_report and falls back to the default location', async () => {
    ...
    const secretsDir = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-secret-'));
  ...
  it('rejects absolute paths in verification_report', async () => {
    const elsewhere = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-abs-'));
