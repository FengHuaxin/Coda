...
function packagePlan(options: {
  ...
  return {
    ...
    skillMaker: {
      intent:
        options.workflow.protocol.kind === 'coda-five-phase-overlay'
          ? 'customize-coda'
          : 'new-skill',
    },
    resolvedSkills: [],
...
describe('Factory skill package generation', () => {
  ...
  it('generates workflow contract packages from Nodes and Output Schemas', async () => {
    const workflow = normalizeWorkflowDefinition({
      ...builtinCodaFivePhaseWorkflow({
        name: 'team-coda',
        goal: 'Require component and review Skills in the coda workflow.',
      }),
      nodes: {
        execute: {
          requiredSkillCalls: [
    ...
    const output = await generateFactorySkillPackage(
      packagePlan({ root, name: 'team-coda', workflow }),
    );
    ...
    const skillYaml = parse(
      await fs.readFile(path.join(output.packageRoot, 'coda', 'skill.yaml'), 'utf8'),
    ) as { orchestration?: { steps?: Array<{ action?: { ref?: string } }> } };

    expect(protocol.kind).toBe('coda-five-phase-overlay');
    ...
    expect(skillYaml.orchestration?.steps?.[0]?.action?.ref).toBe('team-coda-open');

    const runRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-workflow-contract-run-'));
    const env = { ...process.env, CODA_RUN_ROOT: runRoot };
    ...
    const hookGuardScript = path.join(output.packageRoot, 'scripts', 'coda-hook-guard.mjs');
    try {
      await expect(
      ...
      await fs.writeFile(path.join(changeRoot, '.coda.yaml'), 'phase: open\n', 'utf8');
      await fs.writeFile(
  ...
  it('does not generate engine manifests when engine mode is none', async () => {
    ...
    await expect(
      fs.access(path.join(output.packageRoot, 'coda', 'skill.yaml')),
