...
function overlayPlan(root: string): FactorySkillPackagePlan {
  const workflow = normalizeWorkflowDefinition(
    builtinCodaFivePhaseWorkflow({ name: 'team-coda', goal: 'Team coda overlay.' }),
  );
  return {
    root,
    name: 'team-coda',
    version: '1.0.0',
    description: 'team-coda overlay.',
    goal: workflow.protocol.goal,
    defaultLocale: 'en',
    callChain: [],
    workflowDefinition: workflow.input,
...
describe('Factory content-merge (Auto + Authored zones)', () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'coda-content-merge-'));
  });
  afterEach(async () => {
  ...
  it('treats overlay (coda-five-phase-overlay) nodes as delegates, not pending', async () => {
    const output = await generateFactorySkillPackage(overlayPlan(root));
    const openSkill = await fs.readFile(
      path.join(root, 'skills', 'team-coda-open', 'SKILL.md'),
      'utf8',
    );
