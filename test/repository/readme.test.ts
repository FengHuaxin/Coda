...
describe('README assets', () => {
  it.each(readmes)('uses npm-friendly absolute image URLs in %s', async (readmePath) => {
    ...
    expect(content).toContain('https://github.com/rpamis/Coda/blob/master/img/');
  });
  ...
  it('documents task-first paths for Coda-any and eval without making Bundle CLI the default user path', async () => {
    const readmeEn = await fs.readFile('README.md', 'utf-8');
    const readmeZh = await fs.readFile('README-zh.md', 'utf-8');

    expect(readmeEn).toContain('Create or optimize a reusable Skill');
    expect(readmeEn).toContain('`/Coda-any` is the main user path');
    expect(readmeEn).toContain('`Coda eval`');
    expect(readmeEn).toContain('`Coda publish`');
    expect(readmeEn).toContain('`Coda publish distribute --preview`');
    ...
    expect(readmeZh).toContain('`/Coda-any` 是普通用户主路径');
    expect(readmeZh).toContain('`Coda eval`');
    expect(readmeZh).toContain('`Coda publish`');
    expect(readmeZh).toContain('`Coda publish distribute --preview`');
