...
describe('CI workflows', () => {
  it('validates init e2e through owned files and installer status', async () => {
    ...
    const projectVerify = workflow.slice(
      workflow.indexOf('- name: Verify Coda skills installed (project)'),
      workflow.indexOf('- name: Verify external installer status (project)'),
    );
    const globalVerify = workflow.slice(
      workflow.indexOf('- name: Verify Coda skills installed (global)'),
      workflow.indexOf('- name: Verify external installer status (global)'),
    );

    expect(workflow).toContain('coda-init-project.json');
    expect(workflow).toContain('coda-init-global.json');
    expect(workflow).toContain('pnpm run lint:architecture');
    expect(workflow).toContain('pnpm test -- test/domains/coda-classic/coda-scripts.test.ts');
    expect(workflow).not.toContain('test/ts/coda-scripts.test.ts');
    expect(workflow).toContain('export USERPROFILE="$RUNNER_TEMP/coda-e2e-global"');
    expect(workflow).toContain('check_file "$PROJ/$sd/coda/SKILL.md"');
    expect(workflow).toContain('check_file "$HOME_DIR/$sd/coda/SKILL.md"');
    ...
    expect(workflow).toContain('All 31 platforms project Coda skills: OK');
    expect(workflow).toContain('All 31 platforms global Coda skills: OK');
  });

  it('defines PR title linting with coda-specific semantic scopes', async () => {
