#!/bin/bash
# Coda State — unified interface for .coda.yaml state management
# Usage: coda-state.sh <subcommand> <change-name> [args...]
#
# Subcommands:
#   init <change-name> <workflow>  — Initialize .coda.yaml with workflow defaults
#   get <change-name> <field>       — Read a field value from .coda.yaml
...
yaml_file_for() {
  ...
  echo "$change_dir/.coda.yaml"
}

project_context_compression() {
  local value="off"
  local source="default"
  if [ -n "${CODA_CONTEXT_COMPRESSION:-}" ]; then
    value="$CODA_CONTEXT_COMPRESSION"
    source="CODA_CONTEXT_COMPRESSION"
  elif [ -f ".coda/config.yaml" ]; then
    value=$(yaml_field "context_compression" ".coda/config.yaml")
    value="${value:-off}"
    source=".coda/config.yaml"
  fi
...
project_auto_transition_default() {
  local value="true"
  local source="default"
  if [ -n "${CODA_AUTO_TRANSITION:-}" ]; then
    value="$CODA_AUTO_TRANSITION"
    source="CODA_AUTO_TRANSITION"
  elif [ -f ".coda/config.yaml" ]; then
    local raw
    raw=$(yaml_field "auto_transition" ".coda/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".coda/config.yaml"
    fi
...
project_review_mode_default() {
  ...
  local source="default"
  if [ -n "${CODA_REVIEW_MODE:-}" ]; then
    value="$CODA_REVIEW_MODE"
    source="CODA_REVIEW_MODE"
  elif [ -f ".coda/config.yaml" ]; then
    local raw
    raw=$(yaml_field "review_mode" ".coda/config.yaml" 2>/dev/null || true)
    if [ -n "$raw" ]; then
      value="$raw"
      source=".coda/config.yaml"
    fi
...
cmd_init() {
  ...
  # Check if .coda.yaml already exists
  if [ -f "$yaml_file" ]; then
    red "ERROR: .coda.yaml already exists at $yaml_file"
    exit 1
  fi
...
cmd_get() {
  ...
  # Check if .coda.yaml exists
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .coda.yaml not found at $yaml_file"
    exit 1
  fi
...
cmd_set() {
  ...
  case "$field" in
    phase)
      ...
      if [ "${_coda_IN_TRANSITION:-}" != "1" ] && [ "${CODA_FORCE_PHASE:-}" != "1" ]; then
        red "ERROR: Setting 'phase' directly is not allowed; it bypasses state machine evidence checks." >&2
        red "  Use: coda-state.sh transition <change-name> <event>" >&2
        red "  Repair-only escape hatch: CODA_FORCE_PHASE=1 coda-state.sh set <change-name> phase <value>" >&2
        exit 1
      fi
...
cmd_check() {
  ...
  local yaml_file="$change_dir/.coda.yaml"
  local proposal_file="$change_dir/proposal.md"
  local design_file="$change_dir/design.md"
  local tasks_file="$change_dir/tasks.md"

  echo "=== Entry Check: coda-${phase} ==="

  # .coda.yaml must exist for all phases (state machine core)
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .coda.yaml not found at $yaml_file"
    exit 1
  fi
  ...
  case "$phase" in
    open)
      ...
      if [ "$artifacts_done" -eq 3 ]; then
        echo "Recovery action: All artifacts complete. Run /coda-open user confirmation, then guard to transition."
      elif [ "$artifacts_done" -eq 0 ]; then
        echo "Recovery action: No artifacts created yet. Start from /coda-open Step 1 (explore and clarify)."
      else
        echo "Recovery action: Some artifacts incomplete. Resume /coda-open from the first missing artifact."
      fi
      ;;
    design)
      echo "  Artifacts:"
    ...
    verify)
      ...
      if [ "$verify_result" = "pass" ] && [ "$branch_status" = "handled" ]; then
        ...
        echo "Recovery action: Verification failed and rolled back to build. Resume from /coda-build."
      else
    ...
    archive)
      ...
      echo ""
      echo "Recovery action: Run /coda-archive to complete archiving."
      ;;
    *)
...
cmd_scale() {
  ...
  # 3. Changed files: prefer plan base-ref, then .coda.yaml base_ref, fall back to worktree diff
  ...
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    # Fallback to base_ref stored in .coda.yaml (set during init)
  ...
  # Update verify_mode in .coda.yaml
...
cmd_next() {
  ...
  local yaml_file="$change_dir/.coda.yaml"
  if [ ! -f "$yaml_file" ]; then
    red "ERROR: .coda.yaml not found at $yaml_file" >&2
    exit 1
  fi
  ...
  case "$phase" in
    ...
    build)
      case "$workflow" in
        hotfix) skill="coda-hotfix" ;;
        tweak)  skill="coda-tweak" ;;
        *)      skill="coda-build" ;;
      esac
      ;;
    verify)
...
case "$SUBCOMMAND" in
  init)
    if [ $# -lt 2 ]; then
      red "Usage: coda-state.sh init <change-name> <workflow>" >&2
  ...
  get)
    if [ $# -lt 2 ]; then
      red "Usage: coda-state.sh get <change-name> <field>" >&2
      exit 1
    fi
    cmd_get "$@"
    ;;
  set)
    if [ $# -lt 3 ]; then
      red "Usage: coda-state.sh set <change-name> <field> <value>" >&2
      exit 1
    fi
    cmd_set "$@"
    ;;
  transition)
    if [ $# -lt 2 ]; then
      red "Usage: coda-state.sh transition <change-name> <event>" >&2
  ...
  check)
    if [ $# -lt 2 ]; then
      red "Usage: coda-state.sh check <change-name> <phase> [--recover]" >&2
  ...
  scale)
    if [ $# -lt 1 ]; then
      red "Usage: coda-state.sh scale <change-name>" >&2
      exit 1
    fi
    cmd_scale "$@"
    ;;
  task-checkoff)
    if [ $# -lt 2 ]; then
      red "Usage: coda-state.sh task-checkoff <file> <task-text>" >&2
      exit 1
    fi
    cmd_task_checkoff "$@"
    ;;
  next)
    if [ $# -lt 1 ]; then
      red "Usage: coda-state.sh next <change-name>" >&2
      exit 1
    fi
    cmd_next "$@"
    ;;
  *)
    ...
    echo "" >&2
    echo "Usage: coda-state.sh <subcommand> <change-name> [args...]" >&2
    echo "" >&2
    echo "Subcommands:" >&2
    echo "  init <change-name> <workflow>  — Initialize .coda.yaml with workflow defaults" >&2
    echo "  get <change-name> <field>       — Read a field value from .coda.yaml" >&2
    echo "  set <change-name> <field> <val> — Update a field value in .coda.yaml" >&2
