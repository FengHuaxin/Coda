#!/bin/bash
# Coda Archive — automates the archive phase in one command
# Usage: coda-archive.sh <change-name> [--dry-run]
# Exit 0 = archive complete, exit 1 = fatal error

set -euo pipefail

CODA_BASH="${CODA_BASH:-${BASH:-bash}}"
CODA_OPENSPEC="${CODA_OPENSPEC:-openspec}"
...
YAML="$CHANGE_DIR/.coda.yaml"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd -P)"
STATE_SH="$SCRIPT_DIR/coda-state.sh"
TODAY=$(date -u +%Y-%m-%d)
...
echo "=== Coda Archive: $CHANGE ===" >&2

# --- Step 1: Read .coda.yaml, extract paths ---

yaml_field() {
  local field="$1"
  if [ -f "$STATE_SH" ]; then
    "$CODA_BASH" "$STATE_SH" get "$CHANGE" "$field" 2>/dev/null
  else
    if [ -f "$YAML" ]; then
      local value
...
if [ "$VERIFY_VAL" != "pass" ]; then
  red "FATAL: verify_result is '$VERIFY_VAL', expected 'pass'. Run coda-verify first."
  exit 1
fi
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  if ! command -v "$CODA_OPENSPEC" >/dev/null 2>&1; then
    red "FATAL: OpenSpec CLI not found: $CODA_OPENSPEC"
    red "Install OpenSpec or set CODA_OPENSPEC to the openspec executable."
    exit 1
  fi

  "$CODA_OPENSPEC" archive "$CHANGE" --yes >&2
...
# --- Step 7: Mark archived via coda-state transition ---

ARCHIVE_YAML="$ARCHIVE_DIR/.coda.yaml"

if [ "$DRY_RUN" -eq 1 ]; then
  step_dry_run "Would set archived: true in $ARCHIVE_YAML"
else
  if [ -f "$ARCHIVE_YAML" ]; then
    "$CODA_BASH" "$STATE_SH" transition "$ARCHIVE_NAME" archived >/dev/null
    step_ok "archived: true"
  else
    step_fail "archived: true (.coda.yaml not found after move)"
  fi
fi
