#!/bin/bash
# Coda-hook-guard.sh — PreToolUse hook for Coda phase enforcement
#
# Blocks file writes (Write/Edit) when the active Coda change is in
...
if [ -z "$TARGET" ]; then
  echo "[Coda-HOOK] allowed: no file path in tool input" >&2
  exit 0
fi
...
# ── Helpers to read .Coda.yaml fields ───────────────────────────
...
# ── Determine the governing Coda change + phase ─────────────────
#
...
PHASE=""
# Path to the .Coda.yaml that governs this write (used for deeper invariant checks)
GOV_YAML=""
...
case "$RELPATH" in
  openspec/changes/*/*)
    ...
    if [ -n "$_own_change" ] && [ "$_own_change" != "archive" ]; then
      _own_yaml="openspec/changes/${_own_change}/.Coda.yaml"
      if [ -f "$_own_yaml" ]; then
        if [ "$(is_archived "$_own_yaml")" = "true" ]; then
          # This change is already archived — its own writes are unrestricted
          echo "[Coda-HOOK] allowed: $RELPATH (own change archived)" >&2
          exit 0
        fi
        ...
        # (artifacts are created before .Coda.yaml during /Coda-open).
...
if [ -z "$PHASE" ]; then
  ...
  if [ -d "openspec/changes" ]; then
    for dir in openspec/changes/*/; do
      ...
      if [ -f "${dir}.Coda.yaml" ]; then
        # Skip changes already marked as archived
        if [ "$(is_archived "${dir}.Coda.yaml")" = "true" ]; then
          continue
        fi
        YAML_FILE="${dir}.Coda.yaml"
        break
      fi
    done
  fi
  ...
  if [ -z "$YAML_FILE" ]; then
    echo "[Coda-HOOK] allowed: no active Coda change" >&2
    exit 0
  fi
...
if [ -z "$PHASE" ]; then
  echo "[Coda-HOOK] allowed: no phase in .Coda.yaml" >&2
  exit 0
fi
...
case "$RELPATH" in
  openspec/*)
    ...
    case "$PHASE" in
      open)
        ...
        case "$RELPATH" in
          */proposal.md|*/design.md|*/tasks.md|*/.openspec.yaml|*/.Coda.yaml|*/.Coda/*|*/specs/*)
            echo "[Coda-HOOK] allowed: $RELPATH (phase: open, openspec artifacts)" >&2
            exit 0
            ;;
        esac
        ;;
      design)
        # design: allow handoff, delta spec (Spec Patch), proposal/design/tasks (minor refinements), .Coda.yaml
        case "$RELPATH" in
          */proposal.md|*/design.md|*/tasks.md|*/.Coda/*|*/specs/*|*/.Coda.yaml|*/.openspec.yaml)
            echo "[Coda-HOOK] allowed: $RELPATH (phase: design, handoff/spec)" >&2
            exit 0
            ;;
        esac
        ;;
      build)
        # build: allow delta spec (incremental update), tasks, .Coda.yaml
        case "$RELPATH" in
          */specs/*|*/tasks.md|*/.Coda.yaml|*/.openspec.yaml)
            echo "[Coda-HOOK] allowed: $RELPATH (phase: build, spec/tasks)" >&2
            exit 0
            ;;
        esac
        ;;
      verify)
        # verify: allow tasks (post-check), .Coda.yaml
        case "$RELPATH" in
          */tasks.md|*/.Coda.yaml|*/.openspec.yaml)
            echo "[Coda-HOOK] allowed: $RELPATH (phase: verify, tasks/state)" >&2
            exit 0
            ;;
        esac
        ;;
      archive)
        # archive: allow .Coda.yaml state updates only
        case "$RELPATH" in
          */.Coda.yaml|*/.openspec.yaml)
            echo "[Coda-HOOK] allowed: $RELPATH (phase: archive, state)" >&2
            exit 0
            ;;
        esac
        ;;
  ...
  docs/superpowers/*)
    ...
    case "$PHASE" in
      design)
        echo "[Coda-HOOK] allowed: $RELPATH (phase: design, superpowers)" >&2
        exit 0
        ;;
      build)
        echo "[Coda-HOOK] allowed: $RELPATH (phase: build, superpowers)" >&2
        exit 0
        ;;
      verify)
        echo "[Coda-HOOK] allowed: $RELPATH (phase: verify, superpowers)" >&2
        exit 0
        ;;
    esac
  ...
  .Coda/*|*/.Coda/*)
    # Coda config
    echo "[Coda-HOOK] allowed: $RELPATH (whitelist: Coda config)" >&2
    exit 0
    ;;
  .claude/*)
    # Claude settings/rules
    echo "[Coda-HOOK] allowed: $RELPATH (whitelist: claude config)" >&2
    exit 0
    ;;
  CLAUDE.md|CHANGELOG.md|README.md|*.md)
    ...
    case "$RELPATH" in
      ...
      *)
        echo "[Coda-HOOK] allowed: $RELPATH (whitelist: root markdown)" >&2
        exit 0
        ;;
    esac
    ;;
  .Coda.yaml|Coda.yaml|.Coda.yml|Coda.yml)
    # Project-level Coda config
    echo "[Coda-HOOK] allowed: $RELPATH (whitelist: Coda config)" >&2
    exit 0
    ;;
esac
  ...
  build|verify)
    ...
    if [ -n "$GOV_YAML" ]; then
      ...
      if [ "$_wf" = "full" ] && { [ -z "$_dd" ] || [ "$_dd" = "null" ]; }; then
        echo "" >&2
        echo "╔══════════════════════════════════════════╗" >&2
        echo "║     Coda PHASE GUARD — WRITE BLOCKED    ║" >&2
        echo "╚══════════════════════════════════════════╝" >&2
        echo "" >&2
        ...
        echo "  ✅ Correct flow: create the Design Doc in design phase, then run Coda-guard design --apply" >&2
        echo "  💡 Run /Coda-design to fill the missing design; for repair, set design_doc with Coda-state" >&2
        echo "" >&2
        exit 2
      fi
    fi
    # Code writes allowed in build and verify
    echo "[Coda-HOOK] allowed: $RELPATH (phase: $PHASE)" >&2
    exit 0
    ;;
  open|design|archive)
    echo "" >&2
    ...
    case "$PHASE" in
      ...
      design)
        ...
        echo "  💡 After the Design Doc is ready, run Coda-guard design --apply to enter build" >&2
        ;;
      archive)
