...
describe('detect', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `coda-detect-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe('platform global skills directories', () => {
    it('declares Kimi Code global skills under the user .kimi-code directory', () => {
      ...
      expect(kimicode?.openspecToolId).toBe('kimicode');
    });
  ...
  describe('hasSkills', () => {
    ...
    it('detects coda skills', async () => {
      await fs.mkdir(path.join(tmpDir, '.claude', 'skills', 'Coda'), { recursive: true });
      expect(await hasSkills(tmpDir, mockPlatform, 'coda')).toBe(true);
    });

    it('treats OpenCode coda skills without slash commands as incomplete', async () => {
      const opencode = PLATFORMS.find((platform) => platform.id === 'opencode');
      expect(opencode).toBeDefined();
      if (!opencode) return;

      await fs.mkdir(path.join(tmpDir, '.opencode', 'skills', 'Coda'), { recursive: true });
      await fs.mkdir(path.join(tmpDir, '.opencode', 'skills', 'Coda-open'), { recursive: true });

      expect(await hasSkills(tmpDir, opencode, 'coda')).toBe(false);
    });

    it('detects OpenCode coda skills when matching slash commands exist', async () => {
      const opencode = PLATFORMS.find((platform) => platform.id === 'opencode');
      expect(opencode).toBeDefined();
      if (!opencode) return;
      ...
      await fs.writeFile(path.join(tmpDir, '.opencode', 'commands', 'Coda.md'), '');
      await fs.writeFile(path.join(tmpDir, '.opencode', 'commands', 'Coda-open.md'), '');

      expect(await hasSkills(tmpDir, opencode, 'coda')).toBe(true);
    });

    it('requires MimoCode slash commands before treating coda as installed', async () => {
      const mimocode = PLATFORMS.find((platform) => platform.id === 'mimocode');
      expect(mimocode).toBeDefined();
      if (!mimocode) return;

      await fs.mkdir(path.join(tmpDir, '.mimocode', 'skills', 'Coda'), { recursive: true });
      await fs.mkdir(path.join(tmpDir, '.mimocode', 'skills', 'Coda-open'), { recursive: true });

      expect(await hasSkills(tmpDir, mimocode, 'coda')).toBe(false);

      await fs.mkdir(path.join(tmpDir, '.mimocode', 'commands'), { recursive: true });
      await fs.writeFile(path.join(tmpDir, '.mimocode', 'commands', 'Coda.md'), '');
      await fs.writeFile(path.join(tmpDir, '.mimocode', 'commands', 'Coda-open.md'), '');

      expect(await hasSkills(tmpDir, mimocode, 'coda')).toBe(true);
    });
    ...
    it('detects Antigravity global skills in the Gemini Antigravity directory', async () => {
      ...
      await fs.mkdir(path.join(tmpDir, '.gemini', 'antigravity', 'skills', 'Coda'), {
        recursive: true,
      });

      expect(await hasSkills(tmpDir, antigravity, 'coda', [], 'global')).toBe(true);
    });

    it('returns false for missing skills', async () => {
      await fs.mkdir(path.join(tmpDir, '.claude', 'skills'), { recursive: true });
      expect(await hasSkills(tmpDir, mockPlatform, 'coda')).toBe(false);
    });
