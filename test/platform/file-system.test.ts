...
describe('file-system utils', () => {
  ...
  beforeEach(async () => {
    tmpDir = path.join(
      os.tmpdir(),
      `coda-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe('copyFile', () => {
    ...
    it('follows broken symlinks by resolving readlink target', async () => {
      ...
      // Simulate: ~/.claude/skills/coda -> ../../.agents/skills/coda
      const agentDir = path.join(tmpDir, '.agents', 'skills', 'coda');
      await fs.mkdir(agentDir, { recursive: true });

      const claudeSkillsDir = path.join(tmpDir, '.claude', 'skills');
      await fs.mkdir(claudeSkillsDir, { recursive: true });
      const symlinkPath = path.join(claudeSkillsDir, 'coda');
      await fs.symlink(path.join('..', '..', '.agents', 'skills', 'coda'), symlinkPath);
